...
static LPTSTR szBrowseNotSupported                    = TEXT("Browse Connect is not supported with Connection Pooling on");
...
_inline int PASCAL SearchForError(HENV henv, HDBC hdbc,HSTMT hstmt,LPTSTR szFindState)
{
	...
	{
		found = _tcscmp(szState, szFindState) == 0;
	}
...
VOID PASCAL GetDiagRecs(SWORD fHandleType,SQLHANDLE hHandle)
{
	...
	SWORD           cchBufferLength=sizeof(szMessageText)/sizeof(TCHAR),
				cbTextLength=0;
	SDWORD  sdNumRecs=0;
	...
	for(irecRecNumber=1;(irecRecNumber <= sdNumRecs) && RC_SUCCESSFUL(rc);irecRecNumber++)
	{
		rc = SQLGetDiagRec(fHandleType, hHandle, irecRecNumber,szSQLState,&fNativeError,
			szMessageText,cchBufferLength,&cbTextLength);
...
LPTSTR  PASCAL RetcodeToTCHAR(RETCODE retcode, LPTSTR buf)
{
	switch (retcode) 
		{
		case SQL_SUCCESS:
			_tcscpy (buf,TEXT("SQL_SUCCESS"));
			break;
		case SQL_ERROR:
			_tcscpy (buf,TEXT("SQL_ERROR"));
			break;
		case SQL_SUCCESS_WITH_INFO:
			_tcscpy (buf,TEXT("SQL_SUCCESS_WITH_INFO"));
			break;
		case SQL_NO_DATA_FOUND:
			_tcscpy (buf,TEXT("SQL_NO_DATA_FOUND"));
			break;
		case SQL_NEED_DATA:
			_tcscpy (buf,TEXT("SQL_NEED_DATA"));
			break;
		case SQL_INVALID_HANDLE:
			_tcscpy (buf,TEXT("SQL_INVALID_HANDLE"));
			break;
		case SQL_STILL_EXECUTING:
			_tcscpy (buf,TEXT("SQL_STILL_EXECUTING"));
			break;
		default:
			_tcscpy(buf,TEXT("UNKNOWN rc"));
		}
...
BOOL PASCAL FindError(SWORD fHandleType, LPTSTR szFindState)
{
	...
	for(irec = 1
		...
		; irec++)
	{
		if(_tcscmp(szState, szFindState) == 0)
			return TRUE;
	}
	return FALSE;
	...
	if(pch) 
	{
		...
		else
		{
			...
			switch(rgFields[icol-1].wSQLType)
			{
				...
				case SQL_DECIMAL:
				case SQL_REAL:
					bCompare=(fabs(_tcstod(rgbValue, &stopstr) - _tcstod(pstr, &stopstr)) <= fabs((1/pow(10,7))*_tcstod(rgbValue, &stopstr)));
					break;
				case SQL_FLOAT:
				case SQL_DOUBLE:
				...
				case SQL_TYPE_TIMESTAMP:
					pstr=szExpBuf+_tcslen(TEXT("{ts '"));
					// Replace second ' with '\0'                                                                   '
					_tcstok(pstr, TEXT("'"));
					if (_tcslen(rgbValue) > _tcslen(TEXT("1994-10-10 10:00:00")))
						_tcscat(pstr, TEXT(".000"));
					bCompare=(!_tcsncmp(rgbValue, pstr, rgFields[icol-1].precision));
					break;
				default:
					bCompare=(!_tcsncmp(rgbValue, pstr, _tcslen(pstr)));
			}
...
void ResetHstmt(SQLHSTMT * phstmt)
{
	...
	hdesc=NULL;     // Global hdesc needs to be NULL or attempt to read freed mem
}
...
_inline BOOL PASCAL SetStatementAttributes(UWORD fAttr, PTR rgbValue)
{
	...
	if (Supported(SQL_API_SQLSETSTMTATTR))
	{
		...
		_tcscpy(szAPI,szSQLSETSTMTATTR);
	}
	else
	{
		rc = SQLSetStmtOption(hstmt,fAttr,(UDWORD)rgbValue);
		_tcscpy(szAPI,szSQLSETSTMTOPTION);
	}
...
_inline BOOL PASCAL SetConnectionAttributes(HDBC lhdbc,UWORD fAttr, UDWORD rgbValue)
{
	...
	if (Supported(SQL_API_SQLSETCONNECTATTR))
	{
		...
		_tcscpy(szAPI,szSQLSETCONNECTATTR);
	}
	else
	{
		rc = SQLSetConnectOption(lhdbc,fAttr,rgbValue);
		_tcscpy(szAPI,szSQLSETCONNECTOPTION);
	}
...
VOID PASCAL ClearErrorQueue()
{
	...
	while (RC_SUCCESSFUL(SQLError(NULL, hdbc,NULL, szState, NULL, 
		szErrMsg, sizeof(szErrMsg)/sizeof(TCHAR), NULL)))
			;
}
...
BOOL PASCAL Is3XDriver(HDBC hdbc,QTSTRUCT *lpqt)
{
	...
	rc = SQLGetInfo(hdbc, SQL_DRIVER_ODBC_VER, 
...
/* The signal for test to use it's
	...
	rc = SQLDriverConnect(hdbc, lpSI->hwnd, lpqt->sz,
		SQL_NTS, lpqt->buf, MAX_STRING_SIZE, NULL, SQL_DRIVER_NOPROMPT);
	...
	{
		_tcscpy(szHY009, TEXT("S1009"));
		_tcscpy(szHYC00, TEXT("S1C00"));
	}
...
*/
	...
	if (!_tcscmp(szType,TEXT("LONG RAW")) ||
		 !_tcscmp(szType,TEXT("LONG VARTCHAR")) ||
		 !_tcscmp(szType,TEXT("LONG VARTCHAR FOR BIT DATA")))
		{
		fIgnore=TRUE;
		}
...
void  PASCAL TestConnectionOptions()
{
	...
	if (Supported(SQL_API_SQLGETCONNECTATTR))
	{
		...
		_tcscpy(szAPI,szSQLGETCONNECTATTR);
	}
	else
	{
...
VOID PASCAL CheckData(SWORD fHandleType,UWORD uwDescField, PTR rgbValue,LPTSTR szDataSourceName)
{
	...
	switch(uwDescField)
	{
		...
		case SQL_DIAG_SQLSTATE:
			{
				if (_tcscmp((LPTSTR)rgbValue,pszExpState))
				{
		...
		case SQL_DIAG_CLASS_ORIGIN:
			{
				if (_tcscmp((LPTSTR)rgbValue,szISO))
	...
	if (*szBuff)
		DISPLAYERROR(szSQLGETDIAGFIELD,szBuff);
...
void PASCAL TestDiagRec()
{
	...
	rcFunction = SQLGetInfo(hdbc, SQL_DATA_SOURCE_NAME, 
		szDataSourceName, sizeof(szDataSourceName), NULL);
	...
	for (iHandle=0; iHandle < cHandles; iHandle++)
	{
		...
		if(fDiagRecSupported)
		{
			...
			if (RC_SUCCESSFUL(rc) && _tcscmp(szState,pszExpState))
			{
...
{
	SQLWCHAR wszConvBuf[MAX_STRING_SIZE];
	SQLTCHAR tszAval[MAX_STRING_SIZE], tszWval[MAX_STRING_SIZE];
	SQLINTEGER cbT;
	BOOL bCompare=TRUE;
	...
	if (cbA != SQL_NULL_DATA)
	{
		...
		(void)(MultiByteToWideChar(CP_ACP, 0, aszBuf, strlen(aszBuf), wszConvBuf,
			MAX_STRING_SIZE));
		wszConvBuf[strlen(aszBuf)]=L'\0';

		if (wcscmp(wszConvBuf, wszBuf))
		{

#ifdef UNICODE
			wcscpy(tszAval,wszConvBuf);
			wcscpy(tszWval,wszBuf);
#else
			WideCharToMultiByte(CP_ACP, 0, wszConvBuf, wcslen(wszConvBuf), tszAval,
				MAX_STRING_SIZE, NULL, NULL);
			tszAval[wcslen(wszConvBuf)] = '\0';

			WideCharToMultiByte(CP_ACP, 0, wszBuf, wcslen(wszBuf), tszWval,
				MAX_STRING_SIZE, NULL, NULL);
			tszAval[wcslen(wszBuf)] = '\0';

#endif

			_stprintf(buf, TEXT("Unicode string did not match ANSI string.\r\n")\
				TEXT("\t\t\tUnicode: '%s', ANSI: '%s'"), tszWval, tszAval);
			DISPLAYERROR(szAPI, buf);
			bCompare=FALSE;
		}
	}
...
void PASCAL TestMixedAnsiUnicode(QTSTRUCT * lpqt)
{
	...
	_tcscpy(wszServer, lpSI->szValidServer0);
	_tcscpy(wszLogin, lpSI->szValidLogin0);
	_tcscpy(wszPassword, lpSI->szValidPassword0);
	_tcscpy(wszTable, lpqt->szTableName);
	...
	*/
	...
	(void)(WideCharToMultiByte(CP_ACP,
						  0,
						  lpSI->szValidServer0,
						  _tcslen(lpSI->szValidServer0),
						  aszServer,
						  sizeof(aszServer),
						  NULL,
						  NULL));
	aszServer[_tcslen(lpSI->szValidServer0)]=TEXT('\0');

	// UID
	(void)(WideCharToMultiByte(CP_ACP,
						  0,
						  lpSI->szValidLogin0,
						  _tcslen(lpSI->szValidLogin0),
						  aszLogin,
						  sizeof(aszLogin),
						  NULL,
						  NULL));
	aszLogin[_tcslen(lpSI->szValidLogin0)]=TEXT('\0');

	// PWD
	(void)(WideCharToMultiByte(CP_ACP,
						  0,
						  lpSI->szValidPassword0,
						  _tcslen(lpSI->szValidPassword0),
						  aszPassword,
						  sizeof(aszPassword),
						  NULL,
						  NULL));
	aszPassword[_tcslen(lpSI->szValidPassword0)]=TEXT('\0');
	...
	(void)(WideCharToMultiByte(CP_ACP,
						  ...
						  NULL,
						  NULL));
	aszQuery[_tcslen(wszQuery)]=TEXT('\0');
	...
	(void)(WideCharToMultiByte(CP_ACP,
						  ...
						  NULL,
						  NULL));
	aszTable[_tcslen(wszTable)]=TEXT('\0');

#else
	// Copy the ANSI strings
	_tcscpy(aszServer, lpSI->szValidServer0);
	_tcscpy(aszLogin, lpSI->szValidLogin0);
	_tcscpy(aszPassword, lpSI->szValidPassword0);
	_tcscpy(aszTable, lpqt->szTableName);
	sprintf(aszQuery,szSELECTSTAR,lpqt->szTableName);
	...
	*/
	...
	wszServer[_tcslen(lpSI->szValidServer0)]=TEXT('\0');
	...
	wszLogin[_tcslen(lpSI->szValidLogin0)]=TEXT('\0');
	...
	wszPassword[_tcslen(lpSI->szValidPassword0)]=TEXT('\0');
	...
	wszQuery[_tcslen(aszQuery)]=TEXT('\0');
	...
	wszTable[_tcslen(aszTable)]=TEXT('\0');
	...
	rc=SQLNativeSqlA(hdbcANSI, aszQuery, strlen(aszQuery), aszBuf, 
		sizeof(aszBuf), &cbANSI);
	ERRSHANDLE(SQL_HANDLE_DBC, hdbcANSI, SQL_SUCCESS, rc,szSQLNATIVESQL);
	rc=SQLNativeSqlW(hdbcANSI, wszQuery, wcslen(wszQuery), wszBuf, 
		sizeof(wszBuf)/sizeof(WCHAR), &cchUnicode);
	ERRSHANDLE(SQL_HANDLE_DBC, hdbcANSI, SQL_SUCCESS, rc,szSQLNATIVESQL);

	CompareAnsiToUnicode(aszBuf, cbANSI, wszBuf, cchUnicode*sizeof(WCHAR), szSQLNATIVESQLW);
	...
	if (uDriverODBCVer >= 3)
		...
		rc = SQLGetInfo(hdbcANSI, SQL_ACTIVE_STATEMENTS, &fStmts, sizeof(fStmts), NULL);
...
void  PASCAL TestStmtOptions()
{
	...
	if (Supported(SQL_API_SQLGETSTMTATTR))
	{
		...
		_tcscpy(szAPI,szSQLGETSTMTATTR);
	}
	else
	{
		rc = SQLGetStmtOption(hstmt, SQL_MAX_LENGTH, &dwLen);
		_tcscpy(szAPI,szSQLGETSTMTOPTION);
	}
	...
	//Reset the MAX_LENGTH
	if(fConnectOption)
		SetStatementAttributes(SQL_MAX_LENGTH, (PTR)0);
	...
	for(i = 0; i < MAX_TYPES_SUPPORTED; i++)
	{
		...
		*rgFields[i].szType = *rgFields[i].szLength = *rgFields[i].szParams = TEXT('\0');
		...
		rc = SQLGetData(hstmt, 1, SQL_C_TCHAR,
			rgFields[i].szType, sizeof(rgFields[i].szType), &sdw);
		...
		rc = SQLGetData(hstmt, 3, SQL_C_TCHAR,
			rgFields[i].szLength, sizeof(rgFields[i].szLength), &sdw);
		if(!RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA))
			goto TypeErrorRet;
		...
		rc = SQLGetData(hstmt, 4, SQL_C_TCHAR,
			rgFields[i].szPrefix, sizeof(rgFields[i].szPrefix), &sdw);
		if(!RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA))
			goto TypeErrorRet;

		/* suffix */
		rc = SQLGetData(hstmt, 5, SQL_C_TCHAR,
			rgFields[i].szSuffix, sizeof(rgFields[i].szSuffix), &sdw);
		if(!RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA))
			goto TypeErrorRet;

		/* create params */
		rc = SQLGetData(hstmt, 6, SQL_C_TCHAR,
			rgFields[i].szParams, sizeof(rgFields[i].szParams), &sdw);
		if(!RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA))
			goto TypeErrorRet;
...
BOOL  PASCAL BuildCreateStmt(QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes)
{
	...
	*lpqt->sz = TEXT('\0');
	_tcscpy(lpqt->sz,TEXT("Create Table "));
	_tcscat(lpqt->sz, lpqt->szTableName);
	_tcscat(lpqt->sz,TEXT(" ("));
	for(i = 0; i < cTypes; i++) {
		...
		if(rgFields[i].szParams == NULL || _tcslen(rgFields[i].szParams) == 0)
			*lpqt->szParam = TEXT('\0');
		else if(_tcschr(rgFields[i].szParams, TEXT(',')) == NULL)
			wsprintf(lpqt->szParam,TEXT("(%s)"), rgFields[i].szLength);
		else
			{
			_tcscpy(szNum, rgFields[i].szLength);
			wsprintf(lpqt->szParam,TEXT("(%s, %d)"), rgFields[i].szLength,
				_ttoi(szNum) - SCALEDIFF);
			}

		_tcscpy(szParamType, rgFields[i].szType);
		if(pch = _tcschr((LPTSTR)szParamType, TEXT('('))) 
			{
			*pch = TEXT('\0');
			_tcscat(szParamType, lpqt->szParam);
			_tcscat(szParamType, (LPTSTR)_tcschr(rgFields[i].szType, TEXT(')')) + 1);
			}
		else
			{
			_tcscat(szParamType, lpqt->szParam);
		...
		if((rgFields[i].nullable == SQL_NO_NULLS) && (wNum != SQL_NNC_NULL))
			_tcscpy(szNotNull,TEXT("NOT NULL"));
		else
			szNotNull[0] = TEXT('\0');
		
		wsprintf(&lpqt->sz[_tcslen(lpqt->sz)],TEXT(" %s %s %s ,"),
			rgFields[i].szFieldName, (LPTSTR)szParamType, (LPTSTR)szNotNull);
	}

	/* remove the last comma and space */
	lpqt->sz[_tcslen(lpqt->sz) - 2] = TEXT('\0');
	_tcscat(lpqt->sz,TEXT(")"));
	...
	_tcscpy(lpqt->sz,TEXT("select "));
	for(i = 0; i < cTypes; i ++) {
		if(i)
			_tcscat(lpqt->sz,TEXT(","));
		_tcscat(lpqt->sz, rgFields[i].szFieldName);
	}

	_tcscat(lpqt->sz,TEXT(" from "));
	_tcscat(lpqt->sz, lpqt->szTableName);
	...
	DSTRUCT  *lpd,SDWORD  *cColsSelected,BOOL fBindParameter)
{
	...
	_tcscpy(lpqt->szColNames,TEXT("\0"));
	_tcscpy(lpqt->szValues,TEXT("\0"));
	...
	for(i = 0; i < cTypes; i++) {
		...
		for(ind = 0, w = 0; ind < cTypes; ind++) {
			...
			if(fColNames) 
				...
				_tcscat(lpqt->szColNames, rgFields[ind].szFieldName);
				_tcscat(lpqt->szColNames,TEXT(", "));
				_tcscat(lpqt->szValues,TEXT(" ?, "));
		...
		if(fColNames) 
			...
			lpqt->szColNames[_tcslen(lpqt->szColNames) - 2] = TEXT('\0');
			lpqt->szValues[_tcslen(lpqt->szValues) - 2] = TEXT('\0');
			...
			//_ASSERTE(_tcslen(lpqt->sz) < MAX_STRING_SIZE);
			_tcscpy(szInsertStmt,lpqt->sz);
	...
	rc = SQLGetInfo(hdbc, SQL_NEED_LONG_DATA_LEN, 
		lpqt->buf, (SWORD)sizeof(lpqt->buf), NULL);
	if(rc == SQL_SUCCESS)
...
{               
	...
	_tcscpy(szBuff,TEXT("drop table "));
	_tcscat(szBuff, szTableName);
		...
		switch(rgFields[i].wSQLType) 
			...
			case SQL_DECIMAL:
				rc = SQLBindCol(hstmt, w, SQL_C_TCHAR,
							lpd[i].data, MAX_STRING_SIZE*sizeof(SQLTCHAR), &lpd[i].cb);
				break;
			default:
				rc = SQLBindCol(hstmt, w, SQL_C_DEFAULT,
 ...
 *  Binds Binary data types as SQL_C_TCHAR
 *----------------------------------------------------------------------- */
	...
	DSTRUCT  *lpdFetch)
{
	...
	for(i = 0, w = 0; i < cTypes; i++) 
		...
		switch(rgFields[i].wSQLType) 
			...
			case SQL_LONGVARBINARY:
				rc = SQLBindCol(hstmt, w, SQL_C_TCHAR,
							lpdFetch[i].data, sizeof(lpdFetch[i].data), &lpdFetch[i].cb);
				break;
			case SQL_CHAR:
			case SQL_WCHAR:
			case SQL_VARCHAR:
...
void  PASCAL TestMetaData(lpSERVERINFO lpSI, QTSTRUCT  *lpqt,FIELDINFO  *rgFields,UWORD cTypes)
{
	...
	for(i = 0, w = 0; i < cTypes; i++) 
		...
		rc = SQLDescribeCol(hstmt, w, lpqt->buf,
			sizeof(lpqt->buf)/sizeof(TCHAR), NULL, &wType, &dwLen, &wNum, &wNull);
		RETCHECK(SQL_SUCCESS, rc,szSQLDESCRIBECOL);

		/* verify that column name returned is column name created */

		if(0 != _tcsicmp(rgFields[i].szFieldName, lpqt->buf))
	...
	for(ind = 0; ;ind++) 
		...
		for(i = 0; i < cTypes; i++) 
			...
			switch(rgFields[i].wSQLType) 
				...
				case SQL_TINYINT:
					if(lpd[i].cb != sizeof(SCHAR))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);

					_tcscpy(szNum, pch);
					nSame = (SCHAR)_ttoi(szNum) == (SCHAR)lpd[i].data[0];
					break;

				case SQL_DECIMAL:
				case SQL_NUMERIC:
					if(lpd[i].cb > MAX_STRING_SIZE*sizeof(TCHAR))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);

					// Chg _tcslen to strlen below pending spec decision
					if((lpd[i].cb / sizeof(TCHAR)) > (UDWORD)_tcslen(lpd[i].data))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);
				...
				case SQL_WLONGVARCHAR:
					...
					if(lpd[i].cb / sizeof(SQLTCHAR) > _tcslen(lpd[i].data))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);
					nSame = (SDWORD)_tcsstr(lpd[i].data, pch);
				...
				case SQL_LONGVARBINARY:
					...
					if(lpd[i].cb * 2 != (SWORD)_tcslen(pch))
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);
				...
				case SQL_CHAR:
				case SQL_WCHAR:
					if(lpd[i].cb > MAX_STRING_SIZE*sizeof(TCHAR) && rc == SQL_SUCCESS)
						DISPLAYERROR(szRETDATA,TEXT("incorrect return code for outlen"));

					if(((lpd[i].cb / sizeof(TCHAR)) != (UDWORD)rgFields[i].precision) && rc == SQL_SUCCESS)
						DISPLAYERROR(szRETDATA,szINCORRECTOUTLEN);

					nSame = !_tcsnccmp(lpd[i].data, pch, _tcslen(pch));
...
{       
	...
	static SDWORD  sdw = SQL_DATA_AT_EXEC;
	UWORD           w,
				ind;
	...
	for(i = 0, w = 0,ind=0; i < cTypes; i++) 
			...
			pch && *pch) 
		{
			...
			switch(rgFields[i].wSQLType)
			{
				...
				case SQL_FLOAT:
				case SQL_DOUBLE:
					wsprintf(&szQuery[_tcslen(szQuery)],TEXT("%s < ? + 1 and "),
						rgFields[i].szFieldName);
					break;
				default:
					wsprintf(&szQuery[_tcslen(szQuery)],TEXT("%s = ? and "),
						rgFields[i].szFieldName);
			}
	...
	if (_tcsstr(szQuery,TEXT("and")))
	{
		...
		szQuery[_tcslen(szQuery) - 5] = TEXT('\0');
	}

	_tcscpy(lpqt->szParamQuery, szQuery);
	...
	UWORD cTypes,DSTRUCT  *lpd,BOOL fBindParameter)
{
	...
	for(i = 0, w = 0,ind=0; i < cTypes; i++) 
		...
		if ((rgFields[i].fSearchable == SQL_SEARCHABLE ||
			...
			switch(rgFields[i].wSQLType) {
				...
				case SQL_FLOAT:
				case SQL_DOUBLE:
					wsprintf(&lpqt->sz[_tcslen(lpqt->sz)], TEXT("%s < ? + 1 and "),
						rgFields[i].szFieldName);
					break;
				default:
					wsprintf(&lpqt->sz[_tcslen(lpqt->sz)], TEXT("%s = ? and "),
					rgFields[i].szFieldName);
			}
	...
	/* remove the final "and " */
	lpqt->sz[_tcslen(lpqt->sz) - 5] = TEXT('\0');
	_tcscpy(lpqt->szParamQuery, lpqt->sz);
	...
	for(i = 0; ; i++) 
		...
		if(*pch) 
			{
			udw = _tcslen(pch);
			rc = SQLPutData(hstmt, pch, (SDWORD)_tcslen(pch)*sizeof(SQLTCHAR));
			RETCHECK(SQL_SUCCESS, rc, szSQLPUTDATA);
			} 
		else
			{
	...
	UWORD cTypes,DSTRUCT  *lpd)
{
	...
	while(_tcslen(szQuery) < 1024L) 
		{
		int li=_tcslen(szQuery);
		switch(rgFields[i].wSQLType) 
			{
			case SQL_REAL:
			case SQL_FLOAT:
			case SQL_DOUBLE:
				wsprintf(&szQuery[_tcslen(szQuery)],TEXT("%s < %s + 1 and "),
					rgFields[i].szFieldName, pch);
				break;
			default:
				wsprintf(&szQuery[_tcslen(szQuery)],TEXT("%s = %s%s%s and "),
...
void  PASCAL TestSQLTables(QTSTRUCT  *lpqt)
{
	...
	for(i = 0;; i++) 
		...
		rc = SQLGetData(hstmt, 3, SQL_C_TCHAR, lpqt->sz, sizeof(lpqt->sz), NULL);
		RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA); /* should not overflow
			...
			table name */
		fFoundTable += 0 == _tcsicmp(lpqt->sz, lpqt->szTableName);
		}
...
void  PASCAL TestSQLSpecialCols(QTSTRUCT  *lpqt)
{
	...
	rc = SQLSpecialColumns(hstmt, SQL_BEST_ROWID, NULL, 0, NULL, 0,
		lpqt->szTableName, (SWORD)(_tcslen(lpqt->szTableName)), SQL_SCOPE_TRANSACTION,
		SQL_NULLABLE);
...
void  PASCAL VerifyIndex(QTSTRUCT  *lpqt,UWORD fIndex)
{
	...
	for(i = 0;; i++) 
		...
		rc = SQLGetData(hstmt, 3, SQL_C_TCHAR, lpqt->sz, sizeof(lpqt->sz),
			&sdw);
		RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
		if (_tcsicmp(lpqt->sz, lpqt->szTableName) == 0)
			{
			rc = SQLGetData(hstmt, 6, SQL_C_TCHAR, lpqt->sz, sizeof(lpqt->sz),
				&sdw);
			RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
			fFoundTable += 0 == _tcsicmp(lpqt->sz, lpqt->szDataItem);
			}
		}
	...
	UWORD cTypes,DSTRUCT  *lpd,UWORD fIndex)
{
	...
	_tcscpy(lpqt->szDataItem, lpqt->szTableName);
	...
	_tcscpy(lpqt->buf, rgFields[i].szFieldName);
	...
	UWORD cTypes,DSTRUCT  *lpd)
{
	...
	if(i < cTypes) 
		...
		for(i = 0;; i++) 
			...
			rc = SQLGetData(hstmt, 1, SQL_C_TCHAR, lpqt->sz, sizeof(lpqt->sz),
				&cbValue);
			if(rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
				RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
			}
...
void  PASCAL TestOJCap(QTSTRUCT  *lpqt)
{
	...
	rc = SQLGetInfo(hdbc, SQL_OUTER_JOINS, lpqt->buf, sizeof(lpqt->buf), NULL);
	if(RETCHECK(SQL_SUCCESS, rc,szSQLGETINFO)){
...
void  PASCAL TestExtendedFetch(QTSTRUCT  *lpqt, FIELDINFO  *rgFields, UWORD cTypes)
{
	...
	if (!(rgbValue = AllocateMemory(MAX_STRING_SIZE*sizeof(TCHAR)))){
		DISPLAYERROR(szSQLEXTENDEDFETCH,TEXT("Insufficient Memory Available"));
	...
	for (row=0; row<uwRowSetSize; row++)
	{
		...
		for (iTableCol=0; iTableCol<cTypes; iTableCol++)
		{
			...
			iResultSetCol++;
			rc = SQLGetData(hstmt, iResultSetCol, SQL_C_TCHAR, rgbValue, MAX_STRING_SIZE*sizeof(TCHAR),
				&cbValue);
			if(rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
				RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
									  ...
									  DSTRUCT  *lpd)
{
	...
	/* Allocate an area for test data */
	if (!(rgbValue = AllocateMemory(MAX_STRING_SIZE * sizeof(SQLTCHAR)))){
		DISPLAYERROR(szSQLEXTENDEDFETCH,TEXT("Insufficient Memory Available"));
		goto abortQuikRebind;
	}
	...
	for (iRowset=0; iRowset<=1; iRowset++)
	{
		...
		for (row=0; row<(uwRowsetSize*2)  ;row++,iRowsetRow++)
		{
			...
			for(iTableCol=0; iTableCol<cTypes; iTableCol++)
			{
				...
				iResultSetCol++;
				rc = SQLGetData(hstmt, iResultSetCol, SQL_C_TCHAR, rgbValue, MAX_STRING_SIZE * sizeof(SQLTCHAR),
					&cbValue);
				if(rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO)
					RETCHECK(SQL_SUCCESS, rc,szSQLGETDATA);
...
void  PASCAL TestSQLBrowseConnect(lpSERVERINFO lpSI)
{
	...
	rc = SQLGetInfo(hdbc, SQL_ACTIVE_CONNECTIONS, &cMaxConnections, sizeof(cMaxConnections), NULL);
	RETCHECK(SQL_SUCCESS, rc,szSQLGETINFO);

	rc = SQLGetEnvAttr(henv, SQL_ATTR_CONNECTION_POOLING, &sdwCP, SQL_IS_INTEGER, NULL);
	RETCHECK(SQL_SUCCESS, rc,szSQLGETENVATTR);
	if ( SQL_CP_OFF	!= sdwCP)
		{
			DISPLAYERROR(szSQLBROWSECONNECT, szBrowseNotSupported);
			return;
		}

	if(cMaxConnections != 1) 
		{
		_tcscpy(szBCString,TEXT("DSN="));
		_tcscat(szBCString, lpSI->szValidServer0);
		
		if(lpSI->szValidServer0[0] == 0)
			{
			rc = SQLGetInfo(hdbc, SQL_DATA_SOURCE_NAME, szDSN, sizeof(szDSN), NULL);
			_tcscat(szBCString, szDSN);
			}
...
void  PASCAL TestSQLSetScrollOptions()
{
	...
	// Set the Cursor back to READ_ONLY, FORWARD_ONLY, 1
	rc = SQLSetScrollOptions(hstmt,SQL_CONCUR_READ_ONLY,SQL_SCROLL_FORWARD_ONLY,1);
...
void TestThreading(lpSERVERINFO lpSI,QTSTRUCT  *lpqt,FIELDINFO *rgFieldInfo)
{
   ...
   HANDLE hThread[NUM_THREAD];
	...
	for (i=0; i < NUM_THREAD; i++ ) 
	{
		...
		rc=SQLDriverConnect(hdbc1, lpSI->hwnd, lpqt->sz,
		   SQL_NTS, lpqt->buf, MAX_STRING_SIZE, NULL, SQL_DRIVER_NOPROMPT);

		if (!RC_SUCCESSFUL(rc))
			DISPLAYERROR(TEXT("TestThread failed:"),TEXT("SQLDriverConnect failed"));

		// initialize thread structure
		pArg[i].henv = henv1;
		pArg[i].hdbc = hdbc1;
		_tcscpy(pArg[i].szTableName, lpqt->szTableName);
	...
	if (SQL_NULL_HDESC == hdesc2) {
		...
		if (rgDescInfo[uwDescField].fSettable & DescTypes[uwDescIndex].uwTypeMask) {
			...
			if (!rgDescInfo[uwDescField].size)
			{
				...
				cbValue2        = (_tcslen(pNewvalue)+1) * sizeof(TCHAR); 
			}
			...
			for (iDescField=iStartField; iDescField < sizeof(rgDescInfo)/sizeof(rgDescInfo[0]); iDescField++) {
				...
				if (DESC_UPDATE_MODE_READ == swUpdateMode)
				{
					...
					if (RC_SUCCESSFUL(rc))
					{
						...
						if (!rgDescInfo[iDescField].size)
							cbValue2 = _tcslen((LPTSTR)DescBuf) * sizeof(SQLTCHAR);  // For string fields
				...
				else
				{
					...
					rc = SQLSetDescField(hdesc, swDescRec, rgDescInfo[iDescField].uwDescField,
						...
						_tcslen((LPTSTR)rgDescInfo[iDescField].NewValue)*sizeof(TCHAR));
	   ...
	   SQLPOINTER pData, SQLINTEGER *pcbValue, SQLINTEGER *pIndicator, SQLUSMALLINT iDescType)
{
	...
	{
		// Read it again
		rc = SQLGetDescRec(hdesc, swDescRec, szName, sizeof(szName)/sizeof(TCHAR),
			&cbName, &fType2, &fSubType2, &Length, &Precision, &Scale, &Nullable);
...
void  PASCAL TestGetDescRec(QTSTRUCT * lpqt)
{
	...
	SQLHDESC hdesc1=NULL, hdesc2=NULL;
	PTR rgbValue=NULL;
	SQLSMALLINT swDescRec, swDescRecCount, iRecord, cbName, fType, fSubType,
	...
	if (!Supported(SQL_API_SQLGETDESCREC))
	{
		...
		if (RC_SUCCESSFUL(rc))
		{
			rc = SQLGetDescRec(hdesc, iRecord, szName, MAX_DESC_BUF, &cbName, &fType,
...
void  PASCAL TestUseDesc(QTSTRUCT * lpqt, FIELDINFO  *rgFields)
{
	...
	if (Supported(SQL_API_SQLSETDESCFIELD))
	{
		...
		for (icol=1, rgbValue=rgbMemBuf, pcbValue=pcbValueBuf;
			icol <= ccol;
			icol++, rgbValue+=MAX_STRING_SIZE, pcbValue++)
		{
			...
			if(pch) 
			{
				...
				paramno++;
//                              rc=SQLBindParameter(hstmt, paramno, SQL_PARAM_INPUT, SQL_C_TCHAR, 
				rc=DescBindParameter(hstmt, paramno, SQL_PARAM_INPUT, SQL_C_TCHAR, 
		...
		{
			// Bind to next column
			rc = DescBindCol(hstmt, icol, SQL_C_TCHAR, rgbValue, MAX_STRING_SIZE*sizeof(TCHAR), pcbValue);
//                      rc = SQLBindCol(hstmt, icol, SQL_C_TCHAR, rgbValue, MAX_STRING_SIZE*sizeof(TCHAR), pcbValue);
		...
		// Close the cursor and retry using SQLFetchScroll
		rc=SQLFreeStmt(hstmt, SQL_CLOSE); 
		RETCHECK(SQL_SUCCESS, rc,szSQLFREESTMT);

		// Make sure we have a rowset size of 1
		rc=SQLSetStmtAttr(hstmt, SQL_ATTR_ROW_ARRAY_SIZE, (SQLPOINTER)1, SQL_IS_UINTEGER);
		ERRSHANDLE(SQL_HANDLE_STMT, hstmt, SQL_SUCCESS, rc, szSQLSETDESCFIELD);
		...
		// Fetch all the rows and check the bound value with MakeAData for
		// each column
		for (row=1, rc=SQL_SUCCESS; rc == SQL_SUCCESS; row++)
		{
			rc=SQLFetchScroll(hstmt, SQL_FETCH_NEXT, 1); // Will return SQL_NO_DATA after last row
			...
			if (SQL_SUCCESS == rc)
			{
				...
				ERRSHANDLE(SQL_HANDLE_STMT, hstmt, SQL_NO_DATA, rc, szSQLFETCHSCROLL);
...
void  PASCAL TestEnvAttr()
{
	...
	DEFAULT_ODBC_VERSION = SQL_OV_ODBC2;
	
	if (RC_SUCCESSFUL(rc) && (DEFAULT_ODBC_VERSION != sdwEnvAttr))
	{
		// Default value wrong
		wsprintf(buf, TEXT("Value was not %d!"), DEFAULT_ODBC_VERSION);
		DISPLAYERROR(szSQLGETENVATTR,buf);
	}
...
LPTSTR  PASCAL qtMakeData(int row, int col,FIELDINFO  *rgField, LPTSTR buf)
{
	...
	switch(rgField->wSQLType) {
		...
		case SQL_VARBINARY:
		case SQL_LONGVARBINARY:
			if(rgField->precision < (4*sizeof(TCHAR)) )
		...
		case SQL_TIMESTAMP:
		case SQL_TYPE_TIMESTAMP:
			// precision <= 16 means the timestamp column doesn't have enough precision to hold seconds
			if ( rgField->precision > 16 )
				wsprintf(buf,TEXT("{ts '1994-%02d-%02d 01:%02d:%02d'}"), (row % 12) + 1, (row % 30) + 1, row % 60, row % 60);
			else
				wsprintf(buf,TEXT("{ts '1994-%02d-%02d 01:%02d:%02d'}"), (row % 12) + 1, (row % 30) + 1, row % 60, 0);
			break;

		case SQL_GUID:
			_itot(row, szColNum, 10);
			_tcscpy(tmpbuf, szColNum);
			for (i=0; i<11; i++){
				_tcscat(tmpbuf, szColNum);
			}
			wsprintf(buf, TEXT("%.8s-%.4s-%.4s-%.4s-%.12s"), tmpbuf, tmpbuf, tmpbuf, tmpbuf, tmpbuf);
			break;
...
VOID PASCAL TestCursorName(QTSTRUCT *lpqt)
{
	...
	rc = SQLGetCursorName(hstmt, szCursorName, sizeof(szCursorName)/sizeof(TCHAR), &cb);
...
BOOL CALLBACK AutoTestDesc(UWORD iTest, LPTSTR szName, LPTSTR szDesc)
{
	...
	if (szName)
	_tcscpy(szName,iTestNames[iTest-1]);
...
void CALLBACK AutoTestFunc(lpSERVERINFO pTestSource)
{
	...
	FIELDINFO               *rgFields = 
		(FIELDINFO  *)AllocateMemory(sizeof(FIELDINFO) * MAX_TYPES_SUPPORTED);
	QTSTRUCT                        *lpqt = 
		(QTSTRUCT  *)AllocateMemory(sizeof (QTSTRUCT));
	DSTRUCT                 *lpd = 
		(DSTRUCT  *)AllocateMemory(sizeof (DSTRUCT) * MAX_TYPES_SUPPORTED);
	UWORD                           uDMVer;
	SDWORD                  cbValue=0;
	TCHAR                           szAPI[MEDBUFF];
	UWORD                           i;
