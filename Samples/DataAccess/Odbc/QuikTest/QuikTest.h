...
#define MAX_STRING_SIZE						700		  // Increased from 255 due to overwrite
	...
	switch (fSqlType) {
		...
		case SQL_NUMERIC:
				return  (_tcstod(szTrueValue,&pStop) == 
					_tcstod(rgTestValue[iCol].data,&pStop));
		case SQL_TINYINT:
		case SQL_SMALLINT:
		...
		case SQL_FLOAT:
		case SQL_DOUBLE: 
				return  (_tcstod(szTrueValue,&pStop) == rgTestValue[iCol].sdouble);
