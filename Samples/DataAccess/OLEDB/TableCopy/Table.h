...
public:
	...
	virtual BOOL	GetQuotedID(WCHAR* pwszOutBuff, WCHAR* pwszInBuff);
	virtual HRESULT GetColInfo(DWORD dwInsertOpt);	
	virtual HRESULT GetTypeInfo();
	virtual HRESULT GetLiteralInfo();
	virtual HRESULT GetTypeNameAndParams(ULONG iCol, WCHAR* pwszName);
	...
	virtual HRESULT CopyData(CTable* pCSourceTable, ULONG* pcRowsCopied);
