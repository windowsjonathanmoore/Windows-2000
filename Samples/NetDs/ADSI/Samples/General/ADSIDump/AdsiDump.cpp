...
#define CheckHresult(hr,sz) { HRESULT hr_xyz = (hr);  if (!SUCCEEDED(hr_xyz)) throw CStringException(sz); }
...
FILE * pfile		// output file
	...
	HRESULT hr = piads->Get(sbstr, &svar);
	if (!SUCCEEDED(hr))
		return;
...
FILE * pfile	// file to which we dump
	...
	HRESULT hr = piads->QueryInterface(IID_IADsContainer,
									   (PVOID *) &srpiac);
	if (srpiac)
		{
		CComPtr<IEnumVARIANT> srpiev;
		hr = srpiac->get__NewEnum((IUnknown **) &srpiev);
		if (srpiev)
			{
			while (true)
				{
