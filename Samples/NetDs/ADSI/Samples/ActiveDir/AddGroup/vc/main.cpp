...
HRESULT CreateGroup(IDirectoryObject *pDirObject, LPWSTR pwCommonName,LPWSTR pwSamAcctName,
...
BOOL ParseCommandLine( int argc, wchar_t *argv[ ])
{
    ...
    if (argc ==6)
    {
        ...
        else if (!(_wcsicmp(L"nosecurity",argv[4])==0))
        {
...
*/
HRESULT CreateGroup(IDirectoryObject *pDirObject, LPWSTR pwCommonName,LPWSTR pwSamAcctName,IDirectoryObject ** ppDirObjRet,int iGroupType)
{
    assert(pDirObject);
    if (wcslen(pwSamAcctName) >20)
    {
        MessageBox(NULL,L"SamAccountName CANNOT be bigger than 20 characters",L"Error: CreateSimpleGroup()",MB_ICONSTOP);
        assert(0);
        return E_FAIL;
    }
