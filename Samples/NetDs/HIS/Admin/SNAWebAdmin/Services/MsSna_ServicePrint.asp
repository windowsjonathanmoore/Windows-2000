...
<!-------------------------------------------------------------------------
   ...
   for each SNA Print Service in the local domain.
...
<title>SNA Print Service ASP Query Page!</title>
</head>
...
<%
	...
	if Err = 0 then
		...
		Set PrintServices = ServerClass.Instances_
%>
...
<table BORDER="1">
  <tr>
    ...
    <th>Status</th>
    <th>AlwaysDoNL</th>
    <th>DelayPrintStart</th>
    <th>DoAllFF</th>
    <th>FlushFinalFF</th>
    <th>IgnoreCharsUnder3F</th>
  </tr>
<%
	for each item in PrintServices
%>
  <tr>
    <td> <%=item.Name%></td>
    <td>
    <%
    	...
    	if (Status = "Active") then
    		%>
              <applet code="fphover.class" codebase="../../" width="120" height="24">
                ...
                <param name="url" valuetype="ref" value="StartStopService.asp?Service=MsSna_ServicePrint&Name=<%=Name%>&Method=Stop">
    	...
    	elseif (Status = "Inactive") then
			%>
                ...
                <param name="effect" value="glow">
                <param name="url" valuetype="ref" value="StartStopService.asp?Service=MsSna_ServicePrint&Name=<%=Name%>&Method=Start">
                <param name="target" value="_self">
             	 </applet>
			<%
    	else
   ...
   %>
    ...
    <td> <%=item.AlwaysDoNL%></td>
    <td> <%=item.DelayPrintStart%></td>
    <td> <%=item.DoAllFF%></td>
    <td> <%=item.FlushFinalFF%></td>
    <td> <%=item.IgnoreCharsUnder3F%></td>
  </tr>
<%
		Next
%>
