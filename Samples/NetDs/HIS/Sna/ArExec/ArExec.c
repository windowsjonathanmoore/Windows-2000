 ...
 *  22/12/99   Add timeout to cmallc.
 * [RAID6295]  Support SUPPORTS_SETTING_SECURITY.
 *
 *****************************************************************************/
...
/*****************************************************************************/
/* Routine to timeout asynchronous CPIC verbs                                */
/*****************************************************************************/
CM_INT32 WaitForCpicOrTimeout(unsigned char* conv_id, HANDLE hevent, CM_INT32 rc)
{
	DWORD dwwaitrc;
	CM_INT32	crc = 1;
  
  /***************************************************************************/
  /* If an allocate-timeout has been specified and CPIC returned             */
  /* CM_OPERATION_INCOMPLETE (indicating that it is completing               */
  /* asynchronously), then put a timeout on the wait for CPIC to complete.   */
  /***************************************************************************/
  if ((allocate_timeout > 0) && (rc == CM_OPERATION_INCOMPLETE)) 
  {
    dwwaitrc = WaitForSingleObject(hevent, allocate_timeout);
    
    switch (dwwaitrc)
  	{
      case WAIT_TIMEOUT:
        write_output("\nTimeout expired. Cancel Conversation");
        cmcanc(conv_id, &rc);
        if (rc == CM_OK)
        {
          rc = 99;
      ...
      case WAIT_OBJECT_0:
        cmwait(conv_id, &crc, &rc);
        rc = crc;
        break;
  
      default:
        write_output("\nUnexpected Error!!! Cancel Conversation");
        cmcanc(conv_id, &rc);
        if (rc == CM_OK)
        {
        	rc = 99;
...
char * usage[] = {
    ...
    "  -t tp_name",
    "  -z timeout on allocate (1-1000 seconds)",
    "\tdefault: no timeout", 
...
main( int argc, char *argv[])
{
    ...
    CPICINIT *   cpicinit;
    HANDLE hevent = NULL;                  
    CM_PROCESSING_MODE cm_proc_mod = CM_NON_BLOCKING;
    ...
    /*************************************************************************/
    /* If an allocate timeout has been specified, create an event for        */
    /* asynchronous CPIC completion and link the conversation to the event.  */
    /*************************************************************************/
    if (allocate_timeout > 0)
    {
      hevent = CreateEvent(NULL, FALSE, FALSE, NULL);
      if (hevent != NULL)
      {
        WinCPICSetEvent(cm_conv_id, &hevent, &cm_rc);
        if (cm_rc != CM_OK)
        {
          write_output("Error %l registering CPIC event\n, cm_rc");
          exit(EXIT_FAILURE);
        }
      }
      else
      {
        write_output("Fatal error creating event\n");
        exit(EXIT_FAILURE);
      }
    }
    ...
    cmssl(cm_conv_id,                        /* Set sync level               */
          &sync_level,
          &cm_rc);
    cm_rc = WaitForCpicOrTimeout(cm_conv_id, hevent, cm_rc);
    }

    /*************************************************************************/
    /* If an allocate timeout has been specified set the processing          */
    /* mode to non-blocking                                                  */
    /*************************************************************************/
    if (allocate_timeout > 0)
    {
      cm_proc_mod = CM_NON_BLOCKING;
      cmspm(cm_conv_id, &cm_proc_mod, &cm_rc);
      if (cm_rc != CM_OK) 
      {
        cpicerr_handle_rc(cpicerr, MSG_CMSSL, cm_rc);
      }
    }
    ...
    /*************************************************************************/
    /* Go back to blocking mode for the rest of the conversation.            */
    /*************************************************************************/
    if (allocate_timeout > 0)
    {
      cm_proc_mod = CM_BLOCKING;
      cmspm(cm_conv_id, &cm_proc_mod, &cm_rc);
      if (cm_rc != CM_OK) 
      {
        cpicerr_handle_rc(cpicerr, MSG_CMSSL, cm_rc);
                  ...
                  COMMAND_INFO * command_info)
{
    ...
    while (optind != argc) {
        c = getopt(argc, argv, "?t:m:u:p:z:T:M:U:P:Z:Nn");
        switch (c) {
        case EOF:
        ...
        case 'N':
        case 'n':
            cpicinit_set_security_type(cpicinit, XC_SECURITY_NONE);
            break;
#endif
        case '?':
            show_info(usage);
        ...
        case 'z':
        case 'Z':
              allocate_timeout = atoi(optarg);
              if ((allocate_timeout > 1000) || (allocate_timeout < 0))
              {
                write_output("Invalid timeout, must be in range 0-1000 seconds\n");
                exit(EXIT_FAILURE);
              }
              allocate_timeout *= 1000; // timeout is in milliseconds
              break;
