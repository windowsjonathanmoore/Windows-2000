 ...
 *
 *  21/12/99   Generate different pipe names for every new incoming request so 
 * [RAID6295]  we make sure of avoiding the "Error_Pipe_Busy" return code.
 *             Make sure of terminating the created process if and only if the 
 *             AREXEC client died (ie = we got an APPC conversation failure).
 *
 *****************************************************************************/
                        ...
                        struct error_handler_cpicerr * cpicerr)
{
   ...
   /**************************************************************************/
   /* Create pipe name based on current time.                                */
   /**************************************************************************/
   time(&rand);										
   crand = ctime(&rand);							

   strncpy(pipe, "\\\\.\\pipe\\", 9);		
   strncpy(pipe+9, crand + 8, 2);		
   strncpy(pipe+11, crand + 11, 2);		
   strncpy(pipe+13, crand + 14, 2);		
   strncpy(pipe+15, crand + 17, 2);		
   strncpy(pipe+17, "\0", 1);				
   ...
   /**************************************************************************/
   if ((hReadPipe = CreateNamedPipe(pipe,                       
   ...
   /**************************************************************************/
   if ((hWritePipe = CreateFile(pipe, GENERIC_WRITE, 0,         
   ...
   while (!ProcessDead)
   {
      ...
      if (BytesInPipe != 0)
      {
         ...
         /********************************************************************/
         /* If the send failed, mark the command process as dead.            */
         /********************************************************************/
		   if (cm_rc)														
		   {																	
			   ProcessDead = TRUE;										
   ...
   do
   {
	   ...
	   if (BytesInPipe != 0)
      {
         ...
         /********************************************************************/
         /* If the send failed, kill the process.                            */
         /********************************************************************/
		   if (cm_rc)													
		   {																
			   BytesInPipe = 0;										
