...
/* WaitMode = Use the async mode which reqiures CMWAIT verbs to be issued,   */
/*            as opposed to the message posting mode <default = No>          */
/*                                                                           */
...
BOOL InitialiseWinMain(HINSTANCE hInstance)
{
   ...
   /**************************************************************************/
   xchwnd((wait_mode ? NULL : hWndMain), &return_code);
#ifndef CPICSEND
   /**************************************************************************/
   ...
   if (!TPDead)
   {
      switch (last_verb)
      {
         ...
         case C_INIT:
            ...
            /*****************************************************************/
            {
               CM_INT32 receive_type = CM_NON_BLOCKING;
               CM_INT32 temp_return_code;
...
void ReadConfig()
{
   ...
   wait_mode = FALSE;
   if (ReadString("WaitMode",buffer,200))
   {
      wait_mode = (*(strupr(buffer)) == 'Y');
   }
   NumConversations=1;
