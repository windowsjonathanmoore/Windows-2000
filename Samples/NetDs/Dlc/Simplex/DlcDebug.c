...
//                        : ((islower(*envVar) ? 'a' : 'A') - ('9' + 1)))));
    ...
    if (envVar = getenv(ACSLAN_DUMP_FILE_VAR)) {
        ...
        if ((hDumpFile = fopen(envVar, openMode)) == NULL) {
            MyDbgPrint("Error: can't create dump file\n");
        } else {
...

{
    ...
    if (ccbIsBad) {
        PUT(("*** Error: Bad Address for CCB @ %p ***\n", Ccb));
        return;
    }
    ...
    )
{
    ...
    try {
        ...
        PUT(("*** Error: Bad READ Parameter Table @ %p ***\n", parms));
    }
}
    ...
    )
{
    ...
    try {
        ...
        PUT(("*** Error: Bad Transmit Parameter Table @ %p ***\n", parms));
    }
}
    ...
    )
{
    try {
        ...
        PUT(("*** Error: Bad Transmit Queue/Buffer @ %p ***\n", Buffer));
    }
}
    ...
    )
{
    ...
    try {
        ...
        PUT(("*** Error: Bad received data buffer address %p ***\n", pBuf));
        return;
    }
    ...
    try {
        ...
        PUT(("*** Error: Bad Receive Data Buffer @ %p ***\n", Buffer));
    }
}
