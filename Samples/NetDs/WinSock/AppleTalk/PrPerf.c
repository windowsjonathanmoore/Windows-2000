...
//
   ...
   do
   {
       ...
       nbpname.ObjectNameLen = (char) strlen(gServerName);

       strcpy(nbpname.TypeName, gServerType);
       nbpname.TypeNameLen = (char) strlen(gServerType);

       strcpy(nbpname.ZoneName, "*");
       nbpname.ZoneNameLen = (char) strlen("*");
...
//       printf("Accept Succeeded\n");
                            ...
                            (PTEST_INFO) test, 0, & ThreadId ) )
       {
           fprintf(stderr,"CreateThread NULL 0 PerfThread %d 0 %d 0 %P",
                        test->sock_typ, test->prototyp, (LONG_PTR) & ThreadId);
           exit(1);
        }
...
BOOL DoClient()
{
   ...
   if (fRandom)
       rndbuf = rand() % (int) (strlen(DataString) + 1);
   else
       rndbuf = (int) strlen(DataString);
   ...
   do
   {
       ...
       pLookup->LookupTuple.NbpName.ObjectNameLen = (char) strlen(gServerName);

       strcpy(pLookup->LookupTuple.NbpName.TypeName,  gServerType);
       pLookup->LookupTuple.NbpName.TypeNameLen = (char) strlen(gServerType);

       strcpy(pLookup->LookupTuple.NbpName.ZoneName, gServerZone);
       pLookup->LookupTuple.NbpName.ZoneNameLen = (char) strlen(gServerZone);
...
//       printf("ioctlsocket Succeeded\n");
   ...
   if(DataStartPtr != DataEndPtr)
       memcpy(DataStartPtr, DataString, (int)(DataEndPtr-DataStartPtr-1));
...
//   printf("Base Data sent: %s\n",DataStartPtr);
   ...
   for( i= 0; i < NumTimes; i++)
   {
       ...
       do
       {
           ...
           charcnt = (int) (DataEndPtr - DataStartPtr);
