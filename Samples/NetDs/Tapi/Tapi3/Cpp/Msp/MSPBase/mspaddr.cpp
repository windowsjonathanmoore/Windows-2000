...
--*/
{
    ...
    for (int i = 0; i < m_Terminals.GetSize(); i ++)
    {
        ...
        //
        CBaseTerminal * pCTerminal = dynamic_cast<CBaseTerminal *> (m_Terminals[i]);
...
--*/
{
    ...
    CMSPCallBase * pCall = dynamic_cast<CMSPCallBase *> (pIStreamControl);
...
HRESULT CMSPAddress::IsMonikerInTerminalList(IMoniker* pMoniker)
{
    ...
    for (int i = 0; i < iSize; i++)
    {
        pCSingleFilterStaticTerminal = dynamic_cast<CSingleFilterStaticTerminal *>(m_Terminals[i]);
        ...
        while ((hr = pCatEnum->Next(1, &pMoniker, NULL)) == S_OK)
        {
            if (IsMonikerInTerminalList(pMoniker) == S_FALSE)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    //
                    CBaseTerminal * pCTerminal = dynamic_cast<CBaseTerminal *> (pTerminal);

                    //
                    // Mark this terminal so we don't remove it
                    //

                    pCSingleFilterStaticTerminal = dynamic_cast<CSingleFilterStaticTerminal *>(pTerminal);
    ...
    )
{
    ...
    //
    typedef ITPluggableTerminalClassInfo* ClassPtr; // MS parser sucks
        ...
        )
{
    ...
    typedef ITTerminal * TermPtr; // MS parser sucks
