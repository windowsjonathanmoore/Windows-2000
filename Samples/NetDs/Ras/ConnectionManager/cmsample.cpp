...
//
// Returns:   HRESULT WINAPI    - Error code
//
//+----------------------------------------------------------------------------
HRESULT WINAPI SetProxyUsingTunnelAddress(HWND hWnd, HINSTANCE hInst, 
                                          LPSTR pszArgs, DWORD nShow)
{
    ...
    while (ArgV[i])
    {
		...
		else if (0 == lstrcmpi(ArgV[i], "/TunnelAddress") && ArgV[i+1])
        {
	...
	wsprintf(pszTunnelSettingFilePath, "%s\\%s", pszServiceDir, pszTunnelFile);
    
	// Create the full path to the Proxy file
	wsprintf(pszProxyFilePath, "%s\\%s", pszServiceDir, pszProxyFile);
...
//            them in the provided pointers.  Please note that the buffers 
...
//            LPSTR  *ppszProxyServer - string pointer that will have the Proxy
											...
											LPSTR *ppszProxyServer)
{
	...
	*ppszProxyServer = NULL;
    ...
    do
    {
		...
		HeapFree(hCurrentHeap, 0, *ppszProxyServer);
		*ppszProxyServer = NULL;
