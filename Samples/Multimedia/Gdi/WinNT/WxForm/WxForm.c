                     ...
                     LPSTR lpCmdLine, int nCmdShow)
{
    ...
    hicon = (HICON) GetClassLongPtr (hwndMain, GCLP_HICON);
    SetClassLongPtr (hwndTransform, GCLP_HICON, (LONG_PTR)hicon);
    showTransform = TRUE;
    SendMessage (hwndTransform, WM_PUTUPFLOATS, 0, (LONG_PTR) &ptoRect->xfmChange);
      ...
      if (ptoRect->Mode) {
        ...
        if (showTransform)
          SendMessage (hwndTransform, WM_PUTUPFLOATS, 0,
                       (LONG_PTR) &ptoRect->xfmChange);
      }

      if (showMouse)
        SendMessage (hwndMouse, WM_PUTUPFLOATS, (DWORD_PTR) hwnd, lParam);
    ...
    *   of this window.
    ...
    case WM_SIZE :
      if (wParam == SIZEICONIC)
        ...
        showTransform = TRUE;
        SendMessage (hwnd, WM_PUTUPFLOATS, 0, (LONG_PTR) &ptoRect->xfmChange);
      }
    return FALSE;
    ...
    case WM_COMMAND:
      ...
      *   these values.  Finally, reformat and repaint the entry fields.
      ...
      if (LOWORD(wParam) == IDD_SETXFORM) {
        ...
        doTrackObject (ptoRect, TROB_SETXFORM, hwnd, (LONG_PTR) &xform);
        SendMessage (hwnd, WM_PUTUPFLOATS, 0, (LONG_PTR) &xform);
...
*
\**************************************************************************/
PTrackObject doTrackObject(PTrackObject pto, int msg, HWND hwnd, LONG_PTR lParam)
{
...
*
\**************************************************************************/
VOID MouseMove(PTrackObject pto, int msg, HWND hwnd, LONG_PTR lParam)
{
