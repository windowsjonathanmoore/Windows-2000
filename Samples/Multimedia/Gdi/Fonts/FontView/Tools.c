...
LONG_PTR APIENTRY ToolsProc       (HWND, UINT, WPARAM, LPARAM);
...
BOOL CreateToolBar (HWND hwnd, HANDLE hInstance, int iId)
{
        ...
        if (hwndTmp) {
                SendMessage (hwndTmp, WM_SETFONT, (UINT_PTR)hfontTools, (LONG_PTR)MAKELONG (TRUE, 0));
...
HWND AddToolLabel (HANDLE hInst, int iId, LPSTR szLabel, int iWidth, DWORD dwStyle)
{
        ...
        toolCtrl[cntToolCtrls].hwnd = CreateWindow ("STATIC", szLabel,
                ...
                0, 0, 0, 0,
                hwndTools, (HMENU)(INT_PTR)iId, hInst, NULL);

        if (!toolCtrl[cntToolCtrls].hwnd) return (HWND)0; // CreateWindow failed for some reason

        SendMessage (toolCtrl[cntToolCtrls].hwnd, WM_SETFONT, (UINT_PTR)hfontTools, (LONG_PTR)MAKELONG (TRUE, 0));
...
HWND AddToolCombo (HANDLE hInst, int iId, int iWidth, DWORD dwStyle)
{
        ...
        SendMessage (toolCtrl[cntToolCtrls].hwnd, WM_SETFONT, (UINT_PTR)hfontTools, MAKELONG (TRUE, 0));
...
LONG_PTR APIENTRY ToolsProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
        HDC          hdc;
        PAINTSTRUCT      ps;
        ...
        switch (msg) {
                ...
                case WM_CTLCOLORSCROLLBAR:
                        ...
                        switch (iType) {
                                ...
                                case CTLCOLOR_LISTBOX: //List-box control
                                        lStyle = (LONG)GetWindowLongPtr (hwndCtl, GWL_STYLE);
                ...
                case WM_COMMAND:
                        ...
                        if (GetWindowLongPtr (hwndCtl, GWL_STYLE) & BS_OWNERDRAW) {
