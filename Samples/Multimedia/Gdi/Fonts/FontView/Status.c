...
LONG_PTR APIENTRY StatusProc      (HWND, UINT, WPARAM, LPARAM);
LONG_PTR APIENTRY StatusFieldProc (HWND, UINT, WPARAM, LPARAM);
...
BOOL CreateStatusBar (HWND hwnd, HANDLE hInst, int iId)
{
    ...
    hwndStatus = CreateWindow ("SamplerStatus", "SamplerStatus",
        ...
        0, 0, 0, 0,
        hwnd, (HMENU)(INT_PTR)iId, hInst, NULL);
...
HWND AddStatusField (HANDLE hInst, int iId, int iMin, int iMax, BOOL bNewGroup)
{
    ...
    statusField[cntStatusField].hwnd = CreateWindow ("StatusField", "",
        WS_CHILD | WS_VISIBLE,
        0, 0, 0, 0,
        hwndStatus, (HMENU)(INT_PTR)iId, hInst, NULL);
    ...
    if (bNewGroup) {
        lStyle = (LONG)GetWindowLongPtr (statusField[cntStatusField].hwnd, GWL_STYLE);
        lStyle |= WS_GROUP;
        SetWindowLongPtr (statusField[cntStatusField].hwnd, GWL_STYLE, lStyle);
    }
...
LONG_PTR APIENTRY StatusProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    HDC hdc;
    PAINTSTRUCT ps;
    int x, y, i;
        ...
        if (cntStatusField) {
                ...
                for (i=0; i<cntStatusField; i++) {
                    ...
                    if (statusField[i].iMinWidth) {
                        ...
                        if (GetWindowLongPtr(statusField[i].hwnd, GWL_STYLE) & WS_GROUP) {
...
LONG_PTR APIENTRY StatusFieldProc (HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    HDC     hdc;
    PAINTSTRUCT ps;
    RECT    rect, border;
    ...
    switch (msg) {
        case WM_PAINT:
			...
			len = GetWindowText(hwnd, szText, sizeof (szText));
            if (len) {
