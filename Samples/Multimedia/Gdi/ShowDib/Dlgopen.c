    ...
    LPARAM lParam)
{
    ...
    switch (msg) {
        case WM_INITDIALOG:
            ...
            if (flags & OF_SAVE) {
                LoadString(hInst, IDS_SAVESTR, lpBuffer, sizeof(lpBuffer));
                SetDlgItemText(hwnd, IDOK, lpBuffer);
            }
            if (flags & OF_OPEN) {
                LoadString(hInst, IDS_OPENSTR, lpBuffer, sizeof(lpBuffer));
                SetDlgItemText(hwnd, IDOK, lpBuffer);
            }
