...
LONG CALLBACK PreviewWindowHookProc(HWND, UINT, WPARAM, LPARAM);
						 ...
						 LPSTR lpszTitle)
{
	...
	*lpw++ = (WORD)wClassID;
	...
	nchar = nCopyAnsiToWideChar (lpw, TEXT(lpszTitle));
	lpw += nchar;
...
LPDLGTEMPLATE CreateDialogTemplate()
{
  ...
  pdlgtemplate = p = (PWORD) GlobalAlloc (GPTR, 1000);
  ...
  *p++ = 0;          // Class
  ...
  p = AddItemToTemplate(p, WS_CHILD, 0, 0,1,3,122, IDC_LOCATION, 0x82, "");

  // Add the image display area
  p = AddItemToTemplate(p, WS_VISIBLE | WS_CHILD, WS_EX_STATICEDGE, 4,3,80,82, IDC_IMAGE, 0x82, "");

  // Add filename display area
  p = AddItemToTemplate(p, WS_VISIBLE | WS_CHILD | WS_BORDER | ES_CENTER | ES_READONLY, WS_EX_STATICEDGE, 4,86,80,12, IDC_FILEINFO, 0x81, "");
...
BOOL GetDIBFilename(LPSTR lpstrFilename, LPSTR lpstrDefault, BOOL  bSave)
{
	...
	if (!lpstrDefault) {
		...
		if ((GetFileAttributes(lpstrDefault) & FILE_ATTRIBUTE_DIRECTORY) && (GetFileAttributes(lpstrDefault) !=  0xFFFFFFFF) && lstrlen(lpstrDefault))  
			lstrcpy(szDirName, lpstrDefault);
        else
			GetCurrentDirectory(MAX_PATH, szDirName);
	}    
	...
	if (bSave) {
		...
		ofn.Flags = OFN_ENABLETEMPLATEHANDLE | OFN_ENABLEHOOK | OFN_ENABLESIZING | OFN_HIDEREADONLY |
			        OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST | OFN_EXPLORER;
		ofn.lpfnHook = PreviewWindowHookProc;
...
BOOL DisplayFileToDialog(LPSTR szFile, HWND hWnd, HBITMAP *hbm)
{
	...
	*hbm = (HBITMAP)LoadImage(NULL, 
	...
	if (*hbm) {
		...
		wsprintf(szBuffer, "%lux%lux%lubpp", bm.bmWidth, bm.bmHeight, bm.bmBitsPixel * bm.bmPlanes);
...
LONG CALLBACK PreviewWindowHookProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
