...
LRESULT CALLBACK JuliaWndProc    (HWND, UINT, WPARAM, LPARAM);
LRESULT CALLBACK ViewerWndProc   (HWND, UINT, WPARAM, LPARAM);
LRESULT CALLBACK ViewSurfWndProc (HWND, UINT, WPARAM, LPARAM);
BOOL APIENTRY SuspendDrawThrd (HWND, LPARAM);
BOOL APIENTRY ResumeDrawThrd  (HWND, LPARAM);
							 ...
							 LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case MM_MANDEL: {
			...
			(LOWORD(wParam) == MM_JULIA) ? iJuliaCount++ : iMandelCount++ ;
			LocalUnlock(hInfo);
			return ((LRESULT)hwndChildWindow);
						}
	...
	case WM_COMMAND: {
		...
		case MM_DRAW_SET: {
			...
			PostMessage(hwnd, WM_COMMAND,
				pInfo->bMandel ? (WPARAM)((WORD)MM_CREATE_MANDEL_THREAD) : (WPARAM)((WORD)MM_CREATE_JULIA_THREAD),
				(LPARAM)0L);
...
MDI_ACT_EXIT:
	...
	case WM_WINDOWPOSCHANGING: {
		...
		iCyText = (int) GetWindowLongPtr(hTextWnd, GWLP_USERDATA);
	...
	case WM_SIZE: {
		...
		if (hThrd) {
		  // Tell the thread to commit suicide
		        SetEvent(pInfo->hEventTerm);
			// Wait for .5 seconds for the thread to die
			if (WaitForSingleObject(hThrd, 500) == WAIT_TIMEOUT) {
			  // If the thread did not die after .5 seconds then kill it 
			  TerminateThread(hThrd, (DWORD)0L);
			}
		        ResetEvent(pInfo->hEventTerm);
			/*
			PostMessage(hwnd, WM_COMMAND,
	...
	case WM_CREATE: {
		...
		hJulia = CreateWindow("Julia", (LPSTR) NULL,
			...
			rcl.bottom-rcl.top- (LONG)GetWindowLongPtr(hTextWnd, GWLP_USERDATA),
...
#endif
				SetWindowLongPtr(hwnd, 0, (LONG_PTR) hInfo);
				LocalUnlock(hInfo);
			}
		} 
		else 
	...
	case WM_DESTROY: {
		...
		bReleaseInfoData(hwnd);
		LocalFree((HANDLE) GetWindowLongPtr(hwnd, 0));
		break;
					 }
...
LRESULT CALLBACK ViewSurfWndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
	case WM_CREATE: {
...
LRESULT CALLBACK TextWndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
		{
			...
			SetWindowLongPtr(hwnd, GWLP_USERDATA, lHeight);
    ...
    case WM_LBUTTONDOWN: {
		PostMessage(GetParent(hwnd), WM_USER+0xa, (WPARAM)0L, (LPARAM)0L);
		break;
						 }
...
LRESULT CALLBACK JuliaWndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    //
	...
	case WM_RBUTTONDOWN: {
		...
		if (GetWindowLongPtr(GetParent(hwnd), GWL_STYLE) & WS_MAXIMIZE) {
			GetClientRect(ghwndMain, &rc);
			rc.bottom -= (LONG) GetWindowLongPtr(hTextWnd,GWLP_USERDATA);
		} else {
			GetClientRect(hwnd, &rc);
		}
		...
		if (pInfo->bMandel) {
			...
			if (hJulia) {
				...
				//
				bResetGlobal();
				PostMessage(hJulia, WM_COMMAND, (WPARAM)((WORD) MM_SET_XFORM_ATTR), 0L);
			} else {
...
BOOL APIENTRY SuspendDrawThrd (HWND hwnd, LPARAM lParam) {
	HANDLE      hThrd;
	PINFO       pInfo;
	DWORD       dwSuspend;
	BOOL        bDrawing;
...
BOOL APIENTRY ResumeDrawThrd  (HWND hwnd, LPARAM lParam) {
	HANDLE      hThrd;
	PINFO       pInfo;
	DWORD       dwSuspend;
	BOOL        bDrawing;
	...
	for (n=rc.top; n<=rc.bottom; n+=pInfo->iStep, yCurr+=pInfo->iStep, iPrev = FIRST_PIXEL) {
		...
		for (m=rc.left; m<=rc.right; m++, xCurr++) {
			...
			for (i=1; i<=pInfo->iIteration; i++) {
			  ...
			  // Test to see if we should die
			  if (WaitForSingleObject(pInfo->hEventTerm, 0) != WAIT_TIMEOUT) {
			    goto exit;
			  }
...
PINFO pGetInfoData(HWND hwnd)
{
    ...
    hInfo = (HANDLE) GetWindowLongPtr(hwnd, 0);
    if (hInfo == NULL) {
...
BOOL bInitInfo(PINFO pInfo)
{
    ...
    pInfo->hThrd0       = NULL;
    pInfo->hThrd        = NULL;
    pInfo->hEventTerm   = CreateEvent(NULL, TRUE, FALSE, NULL);
    pInfo->bDrawing     = FALSE;
    pInfo->dwThreadId   = 0;
    pInfo->dwElapsed    = 0L;
    pInfo->c1           = 0.0;
