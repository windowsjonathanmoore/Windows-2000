                ...
                BOOL bOutlineOnly)  // Print as outline or as solid text
{
	...
	for (i=0; i < iNumPts; i++) {
		...
		iTopInd = (lpPoints[i].x * (dwTopPts-1)) / size.cx;

		// What point on the bottom guide does this coorespond to
		iBotInd = (lpPoints[i].x * (dwBotPts-1)) / size.cx;
		
		// Scale the points to their new locations
		lpPoints[i].x = ((lpBot[iBotInd].x * lpPoints[i].x) + (lpTop[iTopInd].x * (size.cx-lpPoints[i].x))) / size.cx;
		lpPoints[i].y = ((lpBot[iBotInd].y * lpPoints[i].y) + (lpTop[iTopInd].y * (size.cy-lpPoints[i].y))) / size.cy;
	}
