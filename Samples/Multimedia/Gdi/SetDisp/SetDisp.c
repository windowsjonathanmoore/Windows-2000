  ...
  int nCmdShow )
{
	...
	DialogBox(hInstance, MAKEINTRESOURCE(IDD_SETDISP), (HWND)NULL, SetDispDlgProc);
...
INT_PTR CALLBACK SetDispDlgProc (
   ...
   LPARAM lParam)
{
	...
	switch (GET_WM_COMMAND_ID(wParam, lParam))
	{
		...
		//
		// Only update if they chose a new setting
		//
  ...
  HWND hwnd)
{
	...
	// Check to make sure the system dmSize is not larger 
	// than the size of DEVMODE at compile time.  If it is
	// and we do not do this check we will overwrite memory.
    //

    structSize = (sizeof(DEVMODE) >= (int) WorkingDM.dmSize ? 
		sizeof(DEVMODE) : (int) WorkingDM.dmSize);

    gpDevModes = (PDEVMODE) malloc ((size_t) (gnModes + 1) * structSize);
             ...
             // Accept 0 or 1 frequency which represent default frequencies
        {
            ...
            if (bAddDM)
            {
                ...
                if (i == 0)
                {
					...
					gpCurrentMode->dmSize=structSize;
                }
                else
                {
                    gpCurrentMode += 1;
    ...
    HWND hwnd)
{
    ...
    else
    {
        ...
        InsertMenu (hWndMenu, 0, MF_POPUP|MF_BYPOSITION, (DWORD_PTR)hMenu, "&Resolutions");
  ...
  DWORD dwTime )
{
 ...
 /****************************************************************************
 *                                                                          
 *  FUNCTION   : VerifyDlgProc (HWND hDlg, UINT uMsg,
 *                                WPARAM wParam, LPARAM lParam)
 *
 *                                                                          
