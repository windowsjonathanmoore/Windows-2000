...
WIA_FORMAT_TABLE_ENTRY g_WIA_FORMAT_TABLE[NUM_WIA_FORMAT_INFO_ENTRIES] = {
	&IMGFMT_UNDEFINED,TEXT("Undefined File")            ,
	&IMGFMT_MEMORYBMP,TEXT("Memory Bitmap File (*.BMP)"),
	&IMGFMT_BMP,      TEXT("Bitmap File (*.BMP)")       ,
	&IMGFMT_EMF,      TEXT("EMF File (*.EMF)")          ,
	&IMGFMT_WMF,      TEXT("WMF File (*.WMF)")          ,
	&IMGFMT_JPEG,     TEXT("JPEG File (*.JPG)")         ,
	&IMGFMT_GIF,      TEXT("GIF File (*.GIF)")          ,
	&IMGFMT_TIFF,     TEXT("Tiff File (*.TIF)")         ,
	&IMGFMT_EXIF,     TEXT("Exif File (*.XIF)")         ,
	&IMGFMT_PHOTOCD,  TEXT("Photo CD File (*.PCD)")     ,
	&IMGFMT_FLASHPIX, TEXT("Flash Pix File (*.FPX)")    ,
