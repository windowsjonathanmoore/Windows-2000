        ...
        if (InitDataTypeComboBox()) {
            ...
            if (InitContrastSlider()) {
                ...
                if (InitBrightnessSlider()) {
                    ...
                    MessageBox(TEXT("Brightness Slider control failed to initialize"));
                    return FALSE;
                }
            } else {
                MessageBox(TEXT("Contrast Slider control failed to initialize"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Data Type combobox failed to initialize"));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Resolution edit boxes failed to initialize"));
        return FALSE;
    }
    return TRUE;
}
...
BOOL CMSQSCANDlg::InitResolutionEditBoxes()
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (FAILED(hr)) {
                MessageBox(TEXT("Application Failed to read x resolution (Min Setting)"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application Failed to read x resolution (Max Setting)"));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Application Failed to read x resolution (Current Setting)"));
        return FALSE;
    }
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            hr = m_WIA.ReadLong(NULL,WIA_IPS_XRES,&lCurrent);
            if (FAILED(hr)) {
                MessageBox(TEXT("Application Failed to read y resolution (Min Setting)"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application Failed to read y resolution (Max Setting)"));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Application Failed to read y resolution (Current Setting)"));
        return FALSE;
    }
...
BOOL CMSQSCANDlg::InitContrastSlider()
{
    ...
    if(SUCCEEDED(hr)){
        ...
        if(SUCCEEDED(hr)) {
            ...
            if(FAILED(hr)){
                MessageBox(TEXT("Application Failed to read contrast (Current Setting)"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application Failed to read contrast (Max Setting)"));
            return FALSE;
        }
    }else {
        MessageBox(TEXT("Application Failed to read contrast (Min Setting)"));
        return FALSE;
    }
...
BOOL CMSQSCANDlg::InitBrightnessSlider()
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (FAILED(hr)) {
                MessageBox(TEXT("Application Failed to read brightness (Current Setting)"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application Failed to read brightness (Max Setting)"));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Application Failed to read brightness (Min Setting)"));
        return FALSE;
    }
...
void CMSQSCANDlg::OnScanButton()
{
    ...
    if(m_DataToFile.GetCheck() == 1) {
        ...
        CFileDialog* pFileDialog = new CFileDialog(FALSE);
        ...
        pFileDialog->m_ofn.lpstrFilter = szFilter;
        if (pFileDialog != NULL) {
            ...
            if (pFileDialog->DoModal() == IDOK) {
                ...
                strcpy(m_DataAcquireInfo.szFileName,pFileDialog->m_ofn.lpstrFile);
                DeleteFile(m_DataAcquireInfo.szFileName);

                //
                // delete File dialog object
                //
            ...
            MessageBox(TEXT("Application failed to allocate memory for file dialog"));
        }
...
BOOL CMSQSCANDlg::WriteScannerSettingsToDevice(BOOL bPreview)
{
    ...
    TableIndex = m_DataTypeComboBox.GetItemData(SelectionIndex);
    ...
    if(SUCCEEDED(hr)){
        ...
        if(SUCCEEDED(hr)){
            ...
            if(FAILED(hr)) {
                MessageBox(TEXT("Application Failed to set Data Type"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application Failed to set Brightness"));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Application Failed to set Data Type"));
    ...
    if(bPreview) {
        ...
        if(SUCCEEDED(hr)) {
            ...
            if(SUCCEEDED(hr)) {
                ...
                hr = m_WIA.WriteGuid(NULL,WIA_IPA_FORMAT,IMGFMT_MEMORYBMP);
                if(FAILED(hr)) {
                    MessageBox(TEXT("Application Failed to set format to Memory BMP"));
                    return FALSE;
                }
            } else {
                MessageBox(TEXT("Application Failed to set Y Resolution (Preview)"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application Failed to set X Resolution (Preview)"));
            return FALSE;
        }
        ...
        if(m_DataAcquireInfo.bTransferToClipboard ) {
            ...
            //
            // Do only Banded transfer, and IMGFMT_MEMORYBMP for clipboard
            ...
            hr = m_WIA.WriteGuid(NULL,WIA_IPA_FORMAT,IMGFMT_MEMORYBMP);
            if(FAILED(hr)) {
                MessageBox(TEXT("Memory BMP could not be set to Device"));
                return FALSE;
            }
        }
        ...
        if(SUCCEEDED(hr)) {
            ...
            if(FAILED(hr)) {
                MessageBox(TEXT("Application failed to read y extent (Max value)"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application failed to read x extent (Max value)"));
            return FALSE;
        }
        ...
        if(SUCCEEDED(hr)) {
            ...
            if(SUCCEEDED(hr)) {
                ...
                if(SUCCEEDED(hr)) {
                    ...
                    if(FAILED(hr)){
                        MessageBox(TEXT("Application failed to set Y Extent"));
                        return FALSE;
                    }
                } else {
                    MessageBox(TEXT("Application failed to set X Extent"));
                    return FALSE;
                }
            }else {
                MessageBox(TEXT("Application failed to set Y Position"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application failed to set X Position"));
            return FALSE;
        }
    }
    return TRUE;
        ...
        if(SUCCEEDED(hr)) {
            ...
            if(hr == S_OK) {
                ...
                if(SUCCEEDED(hr)){
                    ...
                    MessageBox(TEXT("Failed to Set IWiaRootItem Interface in to Global Interface Table"));
                }

            } else {
                MessageBox(TEXT("No Scanner was selected."));
                return;
            }
            ...
            if(!m_PreviewWindow.Create(NULL,
                                   ...
                                   this,
                                   PREVIEW_WND_ID)){
                MessageBox(TEXT("Failed to create preview window"));
                return;
            }
            ...
            MessageBox(TEXT("CoCreateInstance for WIA Device Manager failed"));
            return;
        }
    } else {
        MessageBox(TEXT("CoCreateInstance for Global Interface Table failed"));
        return;
    }
}
    ...
    if(OpenClipboard()){
        if(EmptyClipboard()){
            ...
            if(SetClipboardData(CF_DIB, m_DataAcquireInfo.hClipboardData) == NULL) {
                MessageBox(TEXT("SetClipboardData failed"));
            } else {
            ...
            MessageBox(TEXT("EmptyClipboard failed"));
        }
        if (!CloseClipboard()) {
            MessageBox(TEXT("CloseClipboard failed"));
        }
    } else {
        MessageBox(TEXT("OpenClipboard failed"));
    }
            ...
            if(SUCCEEDED(hr)){
                ...
                if(SUCCEEDED(hr)){
                    ...
                    if(SUCCEEDED(hr)){
                        ...
                        if(FAILED(hr)) {
                            MessageBox(TEXT("Application failed to write y extent"));
                            return FALSE;
                        }
                    } else {
                        MessageBox(TEXT("Application failed to write x extent"));
                        return FALSE;
                    }
                } else {
                    MessageBox(TEXT("Application failed to read y extent"));
                    return FALSE;
                }
            } else {
                MessageBox(TEXT("Application failed to read x extent"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application failed to write y pos"));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Application failed to write x pos"));
        return FALSE;
    }
    return TRUE;
}
            ...
            if(SUCCEEDED(hr)){
                ...
                if(SUCCEEDED(hr)){
                    ...
                    if(SUCCEEDED(hr)){
                        ...
                        if(FAILED(hr)){
                            MessageBox(TEXT("Application failed to read the Document Handling Capacity"));
                            return FALSE;
                        }
                    }  else {
                        MessageBox(TEXT("Application failed to read the Document Handling Status"));
                        return FALSE;
                    }
                }  else {
                    MessageBox(TEXT("Application failed to read the Document Handling Capabilites"));
                    return FALSE;
                }
            } else {
                MessageBox(TEXT("Application failed to read the Document Handling Select Property"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application failed to find the Root Item."));
            return FALSE;
        }
    } else {
        MessageBox(TEXT("Application failed to read ADF settings, because the pointer to the Settings structure is NULL"));
        return FALSE;
    }
    return TRUE;
}
...
BOOL CMSQSCANDlg::WriteADFSettings(ADF_SETTINGS *pADFSettings)
{
    ...
    if(pADFSettings!= NULL) {
        ...
        if(pRootItem != NULL) {
            ...
            if(FAILED(hr)){
                MessageBox(TEXT("Application failed to write ADF settings, because the pointer to the Settings structure is NULL"));
                return FALSE;
            }
        } else {
            MessageBox(TEXT("Application failed to find the Root Item."));
