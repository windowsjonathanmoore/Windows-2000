	...
	: CDialog(CADFDlg::IDD, pParent)
{
    ...
    if(m_MaxPagesAllowed <= 0){
        m_MaxPagesAllowed = 500; // set to a large max value
    }
}
...
void CADFDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_PAGE_ORDER_COMBOBOX, m_PageOrderComboBox);
	...
	DDV_MinMaxUInt(pDX, m_NumberOfPages, 1, m_MaxPagesAllowed);
...
BOOL CADFDlg::OnInitDialog() 
{
    ...
    //
    // Initialize PageOrder ComboBox
    //

    InitPageOrderComboBox();
...
VOID CADFDlg::InitPageOrderComboBox()
{
    //
    // add all choices to combo box
    //
        
    for(ULONG index = WIA_DOC_HANDLING_SELECT_PAGEORDER_INDEX;index < (NUM_WIA_DOC_HANDLING_SELECT_ENTRIES - 1);index++) {
        
        //
        // we found a match, so add it to the combobox along with the flag value
        //
        
        INT InsertIndex = m_PageOrderComboBox.AddString(g_WIA_DOCUMENT_HANDLING_SELECT_TABLE[index].szFlagName);
        m_PageOrderComboBox.SetItemData(InsertIndex, g_WIA_DOCUMENT_HANDLING_SELECT_TABLE[index].lFlagValue);
        ...
        if((m_pADFSettings->lDocumentHandlingSelect &
            ...
            m_PageOrderComboBox.SetCurSel(InsertIndex);
        }
...
void CADFDlg::OnSelchangeAdfModeCombobox() 
{
    ...
    LONG lFlagValue = 0;
    lFlagValue = m_ADFModeComboBox.GetItemData(Index);
    ...
    if((lFlagValue & FLATBED) == FLATBED) {
        m_PageOrderComboBox.EnableWindow(FALSE);
        ...
        m_PageOrderComboBox.EnableWindow(TRUE);
...
void CADFDlg::OnOK() 
{
    ...
    INT Index = m_ADFModeComboBox.GetCurSel();
    lModeflag = m_ADFModeComboBox.GetItemData(Index);

    //
    // get current Page order setting
    //

    LONG lPageOrderflag = 0;
    
    Index = m_PageOrderComboBox.GetCurSel();
    lPageOrderflag = m_PageOrderComboBox.GetItemData(Index);
    ...
    m_pADFSettings->lDocumentHandlingSelect = lModeflag|lPageOrderflag;
