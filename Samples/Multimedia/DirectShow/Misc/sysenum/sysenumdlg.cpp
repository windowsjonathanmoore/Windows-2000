...
void CSysEnumDlg::FillCategoryList(void)
{
    ...
    else
    {
        ...
        // using the names stored in the CATEGORY_INFO_DS array.
...
void CSysEnumDlg::ClearFilterList(void)
{
    ...
    SetNumFilters(0);
    m_StrFilename.SetWindowText("<No filter selected>");
}
...
HRESULT CSysEnumDlg::EnumFilters(IEnumMoniker *pEnumCat)
{
    ...
    if (!pEnumCat)
    {
        m_FilterList.AddString("<< No entries >>");
...
void CSysEnumDlg::ShowFilenameByCLSID(REFCLSID clsid)
{
    ...
    if(SUCCEEDED(hr))
    {
        ...
        if (rc == ERROR_SUCCESS)
        {
            ...
            if (rc == ERROR_SUCCESS)
                m_StrFilename.SetWindowText((char *) szFilename);
