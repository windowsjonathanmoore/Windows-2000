...
UINT CGenerateRC::VerifyDatabaseCodepage()
{
	...
	*pch = '\0';
	...
	int iCodepage = atoi(pchCodepage);
	if (iCodepage != 0) // 0 is language neutral
	{
...
UINT CImportRes::VerifyDatabaseCodepage()
{
	...
	*pch = '\0';
	// convert codepage to int
	unsigned int iCodepage = (unsigned int)atoi(pchCodepage);
	if (iCodepage != 0 && iCodepage != g_uiCodePage) // 0 is language neutral
	{
...
BOOL CImportRes::LoadString(HINSTANCE hModule, const TCHAR* szType, TCHAR* szStringName)
{
	...
	for (int i = 0; i < 16; i++) 
	{
		...
		for (int iKey = 1; iKey <= cKeys; iKey++)
		{
			...
			unsigned int iAdd = (unsigned int)(_tcsclen(TEXT(" AND ")) + _tcsclen(sql) + _tcsclen(TEXT("='")) +_tcsclen(TEXT("'")) + _tcsclen(szKeyValue));
...
BOOL CImportRes::LoadDialog(HINSTANCE hModule, const TCHAR* szType, TCHAR* szDialog)
{
	...
	for (int i = 1; i <= iNumCtrl; i++)
	{
		...
		if (ERROR_SUCCESS != MSI::MsiRecordSetInteger(hRecCtrlUpdate, fRadioButton ? irbiHeight : iciHeight, iCtrlHtDim))
			return m_fError = TRUE, TRUE;
		if (_tcscmp(strOverLimit, szCtrlText) != 0) // don't update if "!! STR OVER LIMIT !!"
		{
...
int CDialogStream::GetInt16()
{
	int i = *(unsigned short*)m_pch;
...
{
	_tprintf(
			TEXT("MSILOC.EXE -- Copyright (C) Microsoft Corporation, 2000.  All rights reserved\n")
...
BOOL SkipValue(TCHAR*& rpch)
{
	...
	{	
		...
		{
			rpch++; // for '"'
			for (; (ch = *rpch) != TEXT('"') && ch != TEXT('/') && ch != TEXT('-') && ch != 0; rpch++)
				;
			ch = *(++rpch);
			break;
		}
