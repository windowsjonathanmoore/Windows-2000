...
// 
// Note : You MUST have the NT version of Process Viewer (PVIEW.EXE) in the 
// directory with this Migration DLL in order for the DLL to work.
//

//
// This Migration DLL can be compiled to migrate the Process Viewer that ships
// with VC5, or the Process Viewer the ships with VC6.  To create a DLL that
// migrates the VC5 version of the Process Viewer, uncomment the line below.
// If VC5 is not defined, the DLL will migrate the Process Viewer for VC6.
// Note : When commenting or uncommenting the line below, make sure to 
// perform a Rebuild All to avoid build errors.
//
//#define VC5
...
#else  // Visual C++ 6.0
   #define DIRECTORYKEY                "SOFTWARE\\Microsoft\\DevStudio\\6.0\\Products\\Microsoft Visual C++"
   #define MESSAGE                                                                                 \
       "PVIEW.EXE has been found in one or more directories outside of your Visual C++ 6.0"        \
       " installation directory. These copies will not be updated with the NT version."            
   #define MESSAGE_SECTION              "Microsoft\\Visual C++ 6.0\\Process Viewer"
   #define DIR_95SPECIFIC               "\\win95"
#endif 

#define VS6_COMMON_DIR              "Common\\Tools"
#define DIRECTORYVALUE              "ProductDir"
#define FILE_PVIEW                  "pview.exe"
#define FILE_PVIEW_95               "pview95.exe"
...
//
#ifdef VC5
   const CHAR  g_ProductID[]                       = {"Process Viewer for Microsoft Visual C++ 5.0"};
#else
   const CHAR  g_ProductID[]                       = {"Process Viewer for Microsoft Visual C++ 6.0"};
#endif
...
VENDORINFO	g_VendorInfo						= {"Microsoft",
												         ...
												         "The Process Viewer Migration DLL has failed."};
...
//
// Comment out the following line to suppress popups
//
#define MYDEBUG
#ifdef  MYDEBUG
#   define INFO(x)     (MessageBoxA(NULL,(x),"PVIEW Sample Migration Dll",MB_OK | MB_ICONINFORMATION | MB_TOPMOST))
#else
#   define INFO(x)
#endif
    ...
    )
{
    ...
    LONG    sizeNeeded;
    PSTR    rString = NULL;
#ifndef VC5 // This DLL Migrates the VC6 PView
    PSTR    pTemp = NULL;
#endif
...
#ifndef VC5 
        // Visual C++ 6.0 is different in that it stores PVIEW95.EXE in the
        // Common\Tools\Win95 directory rather than the VC98\Bin\Win95.
        // Because of this, we have to modify the sizeNeeded to reflect the
        // subtraction of "VC98" and the addition of "Common\Tools"
        sizeNeeded -= lstrlen("VC98");
        sizeNeeded += lstrlen(VS6_COMMON_DIR);
#endif
        //
        // Allocate enough space for the registry path, with the additional
        // subpath to the Win95 specific binaries.
        //
...
#ifndef VC5
    // Find a last backslash in the VC6 product directory.
    // This will be the backslash before the VC98 directory
    pTemp = _mbsrchr(rString, '\\');
    // Set the character following this backslash to a NULL
    // terminating charcter to effectively delete "VC98" from
    // the Visual Studio path.
    *(pTemp + 1) = '\0';
    // Concatenate the VS6 Common Tools directory
    lstrcat(rString, VS6_COMMON_DIR);
#endif
    ...
    )
{
    ...
    //
    // If the key exists, then assume that Microsoft 
    // Visual C++ 5.0/6.0 is installed.
    //
    ...
    )
{
    ...
    // we need processing. We always need to provide the product ID, the 
    // DLL version, and the VENDORINFO structure.
    //
    ...
    *VendorInfo = &g_VendorInfo;
    ...
    if (CheckForInstalledComponents() == ERROR_SUCCESS) {
        ...
        *ExeNamesBuf    = g_ExeNamesBuffer;
        ...
        //
        // Since there is work to do, return ERROR_SUCCESS. This informs Setup
    ...
    )
{
    ...
    //
    // Nothing to do per user, so, return ERROR_NOT_INSTALLED
    //
    ...
    )
{
    ...
    PSTR                OldPViewDir          = NULL;
    PSTR                OldPViewFullPath     = NULL;
    PSTR                NewPViewDir          = NULL;
    PSTR                NewPViewFullPath     = NULL;
        ...
        if (MigInf_FirstInSection(SECTION_MIGRATIONPATHS,&sectionEnum) 
            ...
            do {
                ...
                if (PathIsInPath(sectionEnum.Key,visualCppDirectory)) {
                    ...
                    //
                    // Finally, we need to indicate that the PVIEW95.EXE file is
                    // being renamed to PVIEW.EXE and moved to a new directory.
                    // This is important, because Windows2000 will do the work of
                    // updating any shell links that used to point to PVIEW95.EXE.
                    // Although the above examples use helper functions to add the
                    // section values, this example will show how to use
                    // WritePrivateProfileString() directly to add information to 
                    // the Migrate.INF file.
                    //
                    OldPViewDir = GetPviewDirectory9x();
                    NewPViewDir = GetPviewDirectoryNt();

                    if (OldPViewDir && NewPViewDir)
                    {
                       // Allocate enough room to add the PVIEW95.EXE or PVIEW.EXE
                       // file names as well as an additional backslash,
                       // double quotes, and a NULL terminating character.
                       OldPViewFullPath = LocalAlloc(0, strlen(OldPViewDir) + 
                                                        strlen(FILE_PVIEW_95) + 4);
                       NewPViewFullPath = LocalAlloc(0, strlen(NewPViewDir) +
                                                        strlen(FILE_PVIEW) + 4);

                       sprintf(OldPViewFullPath, "\"%s\\%s\"", 
                          OldPViewDir, FILE_PVIEW_95);

                       sprintf(NewPViewFullPath, "\"%s\\%s\"",
                          NewPViewDir, FILE_PVIEW);

                       // Write private profile string is used to add the [Moved]
                       // section.  The first parameter is the section name (defined
                       // in miginf.h).  The last parameter is the file name.
                       // This method relies on the fact that Migrate.INF is in the
                       // same directory as the Migrate.DLL.  A better way of doing
                       // this is to use a global variable that stores the
                       // WorkingDirectory parameter in Initialize9x().
                       WritePrivateProfileString(SECTION_MOVED, OldPViewFullPath,
                          NewPViewFullPath, ".\\migrate.inf");

                       LocalFree(OldPViewFullPath);
                       LocalFree(NewPViewFullPath);
                    }

                    if (OldPViewDir)
                       LocalFree(OldPViewDir);
                    if (NewPViewDir)
                       LocalFree(NewPViewDir);
                }
                else {
