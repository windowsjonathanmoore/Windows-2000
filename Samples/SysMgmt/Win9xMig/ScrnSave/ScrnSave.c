    ...
    )
{
    ...
    *DllVersion = 1;
	...
	//
	// Provide a VENDORINFO structure.  This structure gives the user
	// information on how to get help in case the Migration DLL fails.
	//

	lstrcpy(g_VendorInfo.CompanyName, COMPANY_NAME);
	lstrcpy(g_VendorInfo.SupportNumber, SUPPORT_NUMBER);
	lstrcpy(g_VendorInfo.SupportUrl, SUPPORT_URL);
	lstrcpy(g_VendorInfo.InstructionsToUser, INSTRUCTIONS);
	*VendorInfo = &g_VendorInfo;
