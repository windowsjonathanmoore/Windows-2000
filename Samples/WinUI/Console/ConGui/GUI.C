...
INT_PTR GetDialogArgs (char ***pargv);
BOOL CenterWindow (HWND hwnd);
LRESULT APIENTRY CLDlgProc (HWND hdlg, UINT msg, WPARAM wParam, LPARAM lParam);


// Use WINHELP to bring up the applicaiton help file
int DoHelp (char *szHelpTopic)
{
    WinHelp (GetFocus(), "ConGUI.HLP", HELP_KEY, (DWORD_PTR)(LPSTR)szHelpTopic);
    return TRUE;
}
...
INT_PTR GetDialogArgs (char ***pargv)
{
    ...
    ret = (int) DialogBoxParam (hinst, "CL", NULL, CLDlgProc, (LPARAM)pargv);
    ...
    switch (msg) {
        case WM_INITDIALOG:
            ...
            while (dlgctrls[i].ctrlId) {
                ...
                index = (int) SendDlgItemMessage (hdlg, iCtrl, CB_ADDSTRING, 0, (DWORD_PTR)(LPSTR)dlgctrls[i].str);
                    ...
                    if (cmdline) {
                        ...
                        while (dlgctrls[i].ctrlId) {
                            if (dlgctrls[i].ctrlId != iCtrl) {
                                iCtrl = dlgctrls[i].ctrlId;
                                index = (int) SendDlgItemMessage(hdlg, iCtrl, CB_GETCURSEL, 0, 0);
                                if (index) {
                                    item = (int) SendDlgItemMessage (hdlg, iCtrl, CB_GETITEMDATA, index, 0);
