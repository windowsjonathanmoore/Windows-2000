...
BOOL myPuts(HANDLE hConsole, PCHAR s)
{
  ...
  bSuccess = WriteConsole(hConsole, s, (DWORD)strlen(s), &cCharsWritten, NULL);
  PERR(bSuccess, "WriteConsole");
  retflag = bSuccess;
  /* put a carriage return & line feed after the string */
  bSuccess = WriteConsole(hConsole, crlf, (DWORD)strlen(crlf), &cCharsWritten, NULL);
  PERR(bSuccess, "WriteConsole");
  if (!bSuccess)
    retflag = FALSE;
  return(retflag);
...
void showConAPIs(HANDLE hConsole)
{
  ...
  for (i = 0; i < sizeof(conAPIs) / sizeof(conAPIs[0]); i++)
    ...
    bSuccess = FillConsoleOutputAttribute(hConsole, FOREGROUND_CYAN |
        FOREGROUND_INTENSITY | BACKGROUND_BLUE, (DWORD)strlen(conAPIs[i]),
