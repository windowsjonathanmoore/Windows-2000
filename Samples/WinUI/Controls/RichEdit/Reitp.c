 ...
 *	Owner:
 *
 *  Copyright 1997 - 2000 Microsoft Coporation
 */
...
#define HinstFromHwnd(_hwnd) ((HINSTANCE) GetWindowLongPtr(_hwnd, GWLP_HINSTANCE))
...
static const TCHAR szClassRE[] = TEXT("RICHEDIT");
...
IRichEditOleCallbackVtbl ITPCALL_Vtbl =
{
	 ...
	 ITPCALL_QueryInsertObject,
	 ITPCALL_DeleteObject,
	 ITPCALL_QueryAcceptData,
	 ITPCALL_ContextSensitiveHelp,
	 ITPCALL_GetClipboardData,
	 ITPCALL_GetDragDropEffect,
	 ITPCALL_GetContextMenu
};
...
INT_PTR CALLBACK MyRead(DWORD dwCookie, LPBYTE pbBuffer, LONG cb, LONG *pcb);
...
WinMain(HINSTANCE hinstCurr, HINSTANCE hinstPrev, LPSTR szCmdLine, int nCmdShow)
{
	...
	SideAssert(predoc = (REDOC *) GetWindowLongPtr(hwndMain, 0));
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	REDOC *predoc = (REDOC *) GetWindowLongPtr(hwnd, 0);
#ifndef NO_OLE
	LPOLEINPLACEACTIVEOBJECT pipaobj;
#endif	// !NO_OLE
	...
	case WM_NCCREATE:
		...
		if(!predoc)
			return 0;
		SetWindowLongPtr(hwnd, 0, (LONG_PTR) predoc);
...
# endif	// MAC, else
	...
	case 3:		// options
	{
		const DWORD eco = (DWORD)SendMessage(predoc->hwndRE, EM_GETOPTIONS, 0, 0);
	...
	case IDM_GOTOCURSOR:
		{
			...
			cr.cpMin = (LONG)SendMessage(predoc->hwndRE, EM_CHARFROMPOS, 0,
									(LPARAM) &pt);
			cr.cpMost = cr.cpMin;
...
INT_PTR CALLBACK MyRead(DWORD dwCookie, LPBYTE pbBuffer, LONG cb, LONG *pcb)
{
	...
	if(hf == HFILE_ERROR)
		return (INT_PTR) E_FAIL;
	*pcb = _lread(hf, pbBuffer, cb);
	return (INT_PTR) (*pcb >= 0 ? NOERROR : (*pcb = 0, E_FAIL));
}
				...
				DWORD dwFormat, BOOL fInsert)
{
	...
	es.dwError = 0;
	es.pfnCallback = (EDITSTREAMCALLBACK)MyRead;

	cch = (LONG)SendMessage(predoc->hwndRE, EM_STREAMIN,
...
LOCAL LRESULT SaveREDoc(REDOC *predoc)
{
	...
	es.dwError = 0;
	es.pfnCallback = (EDITSTREAMCALLBACK)MyWrite;
	cch = (LONG)SendMessage(predoc->hwndRE, EM_STREAMOUT,	(WPARAM) predoc->dwFormat,
			(LPARAM) &es);
	_lclose((HFILE) es.dwCookie);
...
LOCAL LRESULT SaveREDocAs(REDOC *predoc, BOOL fSelect)
{
	...
	es.dwError = 0;
	es.pfnCallback = (EDITSTREAMCALLBACK)MyWrite;
	cch = (LONG)SendMessage(predoc->hwndRE, EM_STREAMOUT,
...
VOID PrintREDoc(REDOC *predoc)
{
	...
	cchText = (LONG)SendMessage(predoc->hwndRE, WM_GETTEXTLENGTH, 0, 0);
	...
	do
	{
		if(StartPage(fr.hdc) <= 0)
			goto abort;
		fr.rc = rc;
		fr.chrg.cpMin = (LONG)SendMessage(predoc->hwndRE, EM_FORMATRANGE,
...
LRESULT CALLBACK PPDlgProc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam)
{
	PRINTPREVIEW * ppp = (PRINTPREVIEW *) GetWindowLongPtr(hdlg, DWLP_USER);
	...
	case WM_INITDIALOG:
		...
		SetWindowLongPtr(hdlg, DWLP_USER, (LONG_PTR) ppp);
/*$$ ::SetWindowLongPtr(hdlg, DWLP_USER, (LONG_PTR) ppp); $$*/
		if(!ppp)
		{
...
#endif	// !NOBLIT
	ppp->fr.hdc = 0;
	ppp->cchText = (LONG)SendMessage(ppp->predoc->hwndRE, WM_GETTEXTLENGTH, 0, 0);
...
doit:
	...
	ppp->fr.rc = ppp->rc;
	ppp->fr.chrg.cpMin = (LONG)SendMessage(ppp->predoc->hwndRE, EM_FORMATRANGE,
...
next:
	cpMatch = (LONG)SendMessage(predoc->hwndRE, EM_FINDTEXTEX,
		(WPARAM) (pfr->Flags),
		(LPARAM) &ft);
	if(cpMatch < 0)
	{
