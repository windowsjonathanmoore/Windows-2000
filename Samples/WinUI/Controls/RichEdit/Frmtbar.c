...
#define	PfbsGetWindowPtr(_hwnd)	\
						((FormatBarState *) GetWindowLongPtr(_hwnd, 0))
#define	SetWindowPtr(_hwnd, _p)	SetWindowLongPtr(_hwnd, 0, (LONG_PTR) _p)
...
LOCAL BOOL FB_OnNcCreate(HWND hwnd, CREATESTRUCT * pcs)
{
    ...
    /*
	pfnWndProcT = (WNDPROC) SetWindowLongPtr(pfbs->hwndToolbar, GWL_WNDPROC,
												(LONG_PTR) LFBToolbarWndProc);
	...
	*/
	...
	pfnWndProcT = (WNDPROC) SetWindowLongPtr(hwndT, GWLP_WNDPROC,
												(LONG_PTR) LFBEditWndProc);
	...
	(WNDPROC) SetWindowLongPtr(hwndT, GWLP_WNDPROC, (LONG_PTR) LFBEditWndProc);
	
	// Lastly the color
	pfnWndProcT = (WNDPROC) SetWindowLongPtr(pfbs->hwndColor, GWLP_WNDPROC,
										  		(LONG_PTR) LFBComboBoxWndProc);
...
LOCAL VOID PaintName(FormatBarState * pfbs)
{
	...
	{
		ilFound = (LONG)SendMessage(pfbs->hwndName, CB_FINDSTRING, 0,
...
LOCAL VOID PaintSize(FormatBarState * pfbs)
{
	...
	ilFound = (LONG)SendMessage(pfbs->hwndSize, CB_FINDSTRINGEXACT, 0,
							(LPARAM) szT);
...
LOCAL VOID PaintColor(FormatBarState * pfbs)
{
	...
	{
		ilFound = (LONG)SendMessage(pfbs->hwndColor, CB_FINDSTRINGEXACT, 0,
								(LPARAM) crTextColor);
		if (ilFound < 0)
		{
			// Not found, add a new color
			ilFound = (LONG)SendMessage(pfbs->hwndColor, CB_ADDSTRING, 0,
								(LPARAM) crTextColor);
		}
	}
...
LOCAL void FB_OnCommand(HWND hwnd, INT nID, HWND hwndCtl, INT nNotify)
{
	...
	case TBI_Name:
		if (pfbs->fExpectChoice)
		{
			...
			iSel = (LONG)SendMessage(pfbs->hwndName, CB_GETCURSEL, 0, 0);
			if (iSel < 0)
			{
		//DebugStr("\pBefore GetWindowText in Frmtbar.c");
			...
			else
			{
				...
				dw = (DWORD)ComboBox_GetItemData(pfbs->hwndName, iSel);
				dw >>= 16;
	...
	case TBI_Size:
		if (pfbs->fExpectChoice)
		{
			...
			// Get the new selection
			iSel = (LONG)SendMessage(pfbs->hwndSize, CB_GETCURSEL, 0, 0);
			if (iSel < 0)
			{
	...
	case TBI_Color:
		if (pfbs->fExpectChoice)
		{
			LONG ilSel = (LONG)SendMessage(pfbs->hwndColor, CB_GETCURSEL, 0, 0);
									...
									LPARAM lParam)
{
	...
	case WM_ENABLE:
		if (pfbs)
		{
			...
			EnableWindow(pfbs->hwndToolbar, (int)wParam);
			EnableWindow(pfbs->hwndName, (int)wParam);
			EnableWindow(pfbs->hwndSize, (int)wParam);
			EnableWindow(pfbs->hwndColor, (int)wParam);
	...
	case WM_RETURN:
		...
		pfbs->fExpectChoice = TRUE;
		pfbs->fGiveUpFocus = TRUE;
		FB_OnCommand(hwnd, (int)wParam, (HWND) lParam, CBN_SELCHANGE);
		return 0;
