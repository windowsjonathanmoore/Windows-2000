 ...
 * or draw it.
 *
 * Copyright 1992 - 2000 Microsoft Corporation, All Right Reserved
 */
...
STDAPI_(UINT) OleUIMetafilePictExtractLabel(HGLOBAL hMetaPict, LPOLESTR lpszLabel
   ...
   EnumMetaFile(hDC, pMF->hMF, (MFENUMPROC)EnumMetafileExtractLabel, (LONG_PTR)(LPLABELEXTRACT)&le);
...
int CALLBACK EXPORT EnumMetafileExtractLabel(HDC hDC, HANDLETABLE FAR *phTable
   ...
   if (META_EXTTEXTOUT==pMFR->rdFunction)
      ...
      pLE->u.cch = (UINT)OLESTRLEN (pLE->lpsz);
...
STDAPI_(HICON) OleUIMetafilePictExtractIcon(HGLOBAL hMetaPict)
   ...
   // EnumMetafileExtractLabel proc, we do not need to use
   // MakeProcInstance)
   EnumMetaFile(hDC, pMF->hMF, (MFENUMPROC)EnumMetafileExtractIcon, (LONG_PTR)(LPICONEXTRACT)&ie);
...
STDAPI_(BOOL) OleUIMetafilePictExtractIconSource(HGLOBAL hMetaPict
   ...
   EnumMetaFile(hDC, pMF->hMF, (MFENUMPROC)EnumMetafileExtractIconSource, (LONG_PTR)(LPLABELEXTRACT)&le);
