...
BOOL CALLBACK MyEnumProcedure( HANDLE  hModule, LPCTSTR  lpszType, LPTSTR  lpszName, LPARAM  lParam );
...
BOOL CALLBACK MyEnumProcedure( HANDLE  hModule, LPCTSTR  lpszType, LPTSTR  lpszName, LPARAM  lParam )	
{
    ...
    {
        wsprintf( szBuffer, "Icon [%d]", (DWORD_PTR)lpszName );
        lpID = lpszName;
    }
    else
    {
...
LPTSTR ChooseIconFromEXEFile( LPEXEDLLICONINFO lpEDII )
{
    ...
    if( DialogBoxParam( hInst, MAKEINTRESOURCE(IDD_EXTRACTDLG), hWndMain, (DLGPROC)ExtractDlgProc, (LPARAM)(lpEDII) ) )
    {
        // User chose 'Ok'
        return lpEDII->lpID;
    }
...
BOOL DIBToIconImage( LPICONIMAGE lpii, LPBYTE lpDIB, BOOL bStretch )
{
    ...
    lpii->lpXOR = (PBYTE)FindDIBBits( (LPSTR)(lpii->lpBits) );
...
BOOL CreateBlankNewFormatIcon( LPICONIMAGE lpii )
{
    ...
    bmih.biBitCount = (WORD)lpii->Colors;
    bmih.biClrUsed = 0;
    ...
    lpii->lpbi->bmiHeader.biBitCount = (WORD)lpii->Colors; 
