...
LRESULT CALLBACK IconChildWndProc( HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( Msg )
    {
        ...
        case WM_LBUTTONDOWN:
        {
            ...
            if( (lpcwd = (LPCHILDWINDOWDATA)GetWindowLongPtr( hWnd, GWLP_USERDATA )) == NULL )
                break;
            ...
            if( MK_CONTROL & wParam )
            {
                ...
                if( lpcwd->lpIR != NULL )
                {
                    ...
                    if( (nIndex = (DWORD)SendMessage( lpcwd->hWndFormatListBox, CB_GETCURSEL, 0, 0 )) != CB_ERR )
                    {
                        // Where is it drawn
        ...
        case WM_CLOSE:
        {
            ...
            lpcwd = (LPCHILDWINDOWDATA)GetWindowLongPtr( hWnd, GWLP_USERDATA );
        ...
        case WM_CREATE:
        {
            ...
            SetWindowLongPtr( hWnd, GWLP_USERDATA, (LONG_PTR)lpcwd );
            lpcwd->lpIR = NULL;
        ...
        case WM_COMMAND:
            ...
            switch( LOWORD(wParam) )
            {
                ...
                case ID_EDIT_ADDFORMAT:
                {
                    ...
                    if( DialogBoxParam( hInst, MAKEINTRESOURCE(IDD_ADDFORMATDLG), hWndMain, (DLGPROC)AddFormatDlgProc, (LPARAM)(&lpii) ) )
                    {
                        LPCHILDWINDOWDATA	lpcwd;
                        ...
                        if( (lpcwd != NULL ) && (lpcwd->lpIR != NULL) && (lpcwd->hWndFormatListBox!= NULL) )
                        {
                            ...
                            nIndex = (ULONG)SendMessage( lpcwd->hWndFormatListBox, CB_ADDSTRING, 0, (LPARAM)szBuffer );
                            // Select the new one
                            SendMessage( lpcwd->hWndFormatListBox, 	CB_SETCURSEL, (WPARAM)i, (LPARAM)0 );
                            // clean up
                            free( lpii );
                ...
                case ID_EDIT_REMOVEFORMAT:
                {
                    ...
                    {
                        // Which image is currently selected?
                        if( (nIndex = (ULONG)SendMessage( lpcwd->hWndFormatListBox, CB_GETCURSEL, 0, 0 )) != CB_ERR )
                        {
                ...
                case IDOK:
                {
                    ...
                    if( ((LPICONIMAGE)lInitParam) != NULL )
                    {
                        ...
                        else
                        {
                            ...
                            lpii->Width = (UINT)SendDlgItemMessage( hWnd, ID_WIDTHSLIDER, TBM_GETPOS, 0, 0 );
                            lpii->Height = (UINT)SendDlgItemMessage( hWnd, ID_HEIGHTSLIDER, TBM_GETPOS, 0, 0 );
