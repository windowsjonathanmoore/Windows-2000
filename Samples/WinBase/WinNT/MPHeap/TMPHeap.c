...
LPVOID Malloc(DWORD_PTR dwBytes);
    ...
    )
{
    ...
    DWORD_PTR BufferSize;
    DWORD Delay;
    DWORD i,j;
    DWORD Fill;
    PULONG_PTR Last = NULL;
    BOOL AllocAgain;
    DWORD_PTR MaxSize=0;
    DWORD_PTR CurrentSize=0;
    ...
    while (!Suicide) {
        ...
        if (AllocAgain) {
            BufferSize = (rand() & 0xfff) + sizeof(PUCHAR) + sizeof(ULONG_PTR);
            ...
            for (j=0; j<BufferSize/sizeof(ULONG_PTR); j++) {
                Buffer[j] = Fill;
            }
            Buffer[0] = (ULONG_PTR)Last;
            Buffer[1] = BufferSize;
            Last = Buffer;
        } else if (Buffer != NULL) {
            Last = (PULONG_PTR)Buffer[0];
