    ...
    )
{
    ...
    do {
        ...
        if ((Index != NO_LOOKASIDE) &&
            ...
            if ((Header = (PMP_HEADER)InterlockedExchangePointer((PVOID)&Entry->Lookaside[Index].Entry,
                                                          (PVOID)NULL)) != NULL) {
                //
    ...
    )
{
    ...
    UNREFERENCED_PARAMETER(hMpHeap);
    ...
    )
{
    ...
    if (Header->LookasideIndex != NO_LOOKASIDE) {
        ...
        //
        if (InterlockedCompareExchangePointer((PVOID *)&HeapEntry->Lookaside[Header->LookasideIndex],
                                       (PVOID)Header,
                                       NULL) == NULL) {
            //
    ...
    )
{
    ...
    //
    FreeList = (PSINGLE_LIST_ENTRY)InterlockedExchangePointer((PVOID)&HeapEntry->DelayedFreeList, (PVOID)NULL);
    while (FreeList != NULL) {
        Next = FreeList->Next;
