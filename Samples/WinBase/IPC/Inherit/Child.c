...
void myError(HANDLE h, char *api, int line)
{
  ...
  sprintf(buf, "%s: Error %d from %s on line %d\n", __FILE__, GetLastError(),
      api, line);
  fSuccess = WriteFile(h, buf, (DWORD)strlen(buf), &cBytesWritten, NULL);
  if (!fSuccess)
    {
    ...
    WriteFile(hConout, buf, (DWORD)strlen(buf), &cBytesWritten, NULL);
    CloseHandle(hConout);
    }
  return;
}
...
int main()
{
  ...
  if (hStdOut == INVALID_HANDLE_VALUE)
    myError(hConout, "GetStdHandle", __LINE__);
  fSuccess = WriteFile(hStdErr, stdErrMsg, (DWORD)strlen(stdErrMsg),
      &cBytesWritten, NULL);
  if (!fSuccess)
    myError(hConout, "WriteFile", __LINE__);
  fSuccess = WriteFile(hStdOut, stdOutMsg, (DWORD)strlen(stdOutMsg),
      &cBytesWritten, NULL);
  if (!fSuccess)
    myError(hConout, "WriteFile", __LINE__);
  CloseHandle(hConout);
