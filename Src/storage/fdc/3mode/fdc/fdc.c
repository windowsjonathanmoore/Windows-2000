...
COMMAND_TABLE CommandTable[] = {
    ...
    { 0x01, 4, 0, 0,  FALSE, FALSE, FDC_NO_DATA    }    // Density Mode...0x1D
#else
    { 0xAD, 5, 1, 7,  TRUE,  TRUE,  FDC_WRITE_DATA }    // Format And Write
#endif
};
...
//        deviceCapabilities->D3Latency;
    ...
    case IRP_MN_QUERY_ID:
        ...
        case BusQueryHardwareIDs: {
            ...
            ansiId.Length = ansiId.MaximumLength = strlen( idString );
...
--*/
{
    ...
    if ( fdoExtension->FdcEnablerSupported ) {
        ...
        //  it.
        //
        FdcDump( FDCSHOW, ("AllocatedResources = %p\n",irpSp->Parameters.StartDevice.AllocatedResources));
        FdcDump( FDCSHOW, ("AllocatedResourcesTranslated = %p\n",irpSp->Parameters.StartDevice.AllocatedResourcesTranslated));
    ...
    )
{
    ...
    if ( NT_SUCCESS(ntStatus) ) {
        ...
        do {
            //
            // If nameIndex is greater than 100 then we'll have a buffer overflow
            //
            ASSERT(nameIndex < 100);
...
--*/
{
    ...
    } // Create device name unicode string
...
#ifdef TOSHIBAJ
// 20-Sep-1994 RKADP003
// J-3100PV2/PVN use FDC command when driver change FDD rotation for 3mode.
// It's MODE command.
// 14-Nov-1994 RKADP00G-1  For Daytona RC1's DDK.
//
//  < To 360rpm >
//           7   6   5   4   3   2   1   0
//         +---+---+---+---+---+---+---+---+
...
//         +---+---+---+---+---+---+---+---+
//  Byte 1 | 0 | 0 | 0 | 0 | 0 | 0 | 1 | 0 | 02h
//         +---+---+---+---+---+---+---+---+
//  Byte 2 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 00h
//         +---+---+---+---+---+---+---+---+
//  Byte 3 | 0 | 0 | 0 | 0 | 1 | 0 | 0 | 0 | 08h
//         +---+---+---+---+---+---+---+---+ ^^^
//  Byte 4 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 00h
//         +---+---+---+---+---+---+---+---+
//  < To 300rpm >
//           7   6   5   4   3   2   1   0
//         +---+---+---+---+---+---+---+---+
...
//
// Parameter fields passed to the MODE command.
//
#define TOSHIBA_COMMND_MODE                       0x1D
#define TOSHIBA_COMMND_MODE_BYTE1                 0x02
#define TOSHIBA_COMMND_MODE_BYTE2                 0x00
#define TOSHIBA_COMMND_MODE_DENSEL_HIGH           0x08      // To 360rpm
#define TOSHIBA_COMMND_MODE_DENSEL_LOW            0x48      // To 300rpm
#define TOSHIBA_COMMND_MODE_BYTE4                 0x00
...
{   NTSTATUS    ntStatus;
    ...
    FdcDump(FDCSHOW,("Parameters...%d %d %d\n",
...
//      return  STATUS_SUCCESS;
    ...
    motorSpeed = (param->Enable3Mode)? RPM_360: RPM_300;
    ...
    //
    // FDD rotatin change from 300rpm(360rpm) to 360rpm(300rpm).
    //
    // 17-Oct-1994 RKADP00A  Output speciral register for DynaBookV486FV.
    // 19-Oct-1994 RKADP00C  Output speciral register for ASKAs PC.
    // 18-Nov-1994 RKADP00J  New Toshiba standerd 3mode feature updated from ASKA 3mode Feature.
    //
    //   FDD CONTROL Register
    //      Port 0E5h, Key 0E4h, Index 074h
    //          Bit 7 : Drive 0  Bit 6 : Drive 1
    //              1: 2.0MB(300RPM) mode   0: 1.6MB(360RPM) mode
    //
    //  note: When you output this bit, you must be "1" to bit2.
    //          Bit 0 :
    //              1: 2DD media        0: 2HD media
    //

    if (motorSpeed == RPM_360) {
        FdoExtension->FifoBuffer[1] = TOSHIBA_COMMND_MODE_BYTE1;
        FdoExtension->FifoBuffer[2] = TOSHIBA_COMMND_MODE_BYTE2;
        FdoExtension->FifoBuffer[3] = TOSHIBA_COMMND_MODE_DENSEL_HIGH;
        FdoExtension->FifoBuffer[4] = TOSHIBA_COMMND_MODE_BYTE4;
        FdcDump(FDCSHOW,
        ...
        FdoExtension->FifoBuffer[1] = TOSHIBA_COMMND_MODE_BYTE1;
        FdoExtension->FifoBuffer[2] = TOSHIBA_COMMND_MODE_BYTE2;
        FdoExtension->FifoBuffer[3] = TOSHIBA_COMMND_MODE_DENSEL_LOW;
        FdoExtension->FifoBuffer[4] = TOSHIBA_COMMND_MODE_BYTE4;
        FdcDump(FDCSHOW,
    ...
    // Send MODE command to FDC.
    FdoExtension->FifoBuffer[0] = TOSHIBA_COMMND_MODE;
        ntStatus = FcIssueCommand( FdoExtension,
            FdoExtension->FifoBuffer,
                        FdoExtension->FifoBuffer,
                        NULL,
    ...
    // Sep.20.1996 KIADP006 Ignore if mode command didn't raise interrupt
...
{   PIO_STACK_LOCATION irpSp;
    PUCHAR          modeControlRegisterValue;
    NTSTATUS    ntStatus;
    ...
    if (!NT_SUCCESS( ntStatus ) )
    {   FdoExtension->Available3Mode = FALSE;
        ntStatus = STATUS_UNSUCCESSFUL;
    }
