...

{
    ...
    if (RtlCompareMemory(InquiryData->VendorId,"Elms    ",8) == 8) {
        if (RtlCompareMemory(InquiryData->ProductId,"DVL",3) == 3) {
...
--*/
{
    ...
    switch (senseBuffer->SenseKey) {
        ...
        case SCSI_SENSE_HARDWARE_ERROR:
            if ((senseBuffer->AdditionalSenseCode == SCSI_ADSENSE_DIAGNOSTIC_FAILURE) &&
                (senseBuffer->AdditionalSenseCodeQualifier == SCSI_SENSEQ_ELMS_UNIQUE)) {
...

{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (srb->DataTransferLength != length) {
            DebugPrint((1,
                       "Invalid length returned srb length %x, length %x\n",
                       srb->DataTransferLength,
                       length));
