...

{
    ...
    if (RtlCompareMemory(InquiryData->ProductId,"EXB-440",7) == 7) {

        return TRUE;
    } else if (RtlCompareMemory(InquiryData->ProductId,"EXB-480",7) == 7) {

        return TRUE;
    } else if (RtlCompareMemory(InquiryData->ProductId,"EXB-210",7) == 7) {

        return TRUE;
    } else if (RtlCompareMemory(InquiryData->ProductId,"EXB-220",7) == 7) {

        return TRUE;
    } else if (RtlCompareMemory(InquiryData->ProductId,"EXB-10e",7) == 7) {
        ...
        do {
            ...
            for (i = 0; i < typeCount && remainingElements > 0; i++, remainingElements--) {
                ...
                elementStatus->Element.ElementAddress =
                    elementDescriptor->EXA_FULL_ELEMENT_DESCRIPTOR.ElementAddress[1];
                elementStatus->Element.ElementAddress |=
                    (elementDescriptor->EXA_FULL_ELEMENT_DESCRIPTOR.ElementAddress[0] << 8);
...
{
    UCHAR asq = ElementDescriptor->EXA_FULL_ELEMENT_DESCRIPTOR.AddSenseCodeQualifier;
    ULONG exceptionCode;
