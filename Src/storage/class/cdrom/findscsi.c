   ...
   for (controllerIndex = 0; TRUE; controllerIndex++) {
      ...
      if (!NT_SUCCESS(status)) {
         ...
         if (status != STATUS_NO_MORE_ENTRIES) {
            DebugPrint ((2, "FindScsiAdapter: Error %lx enumerating key\n", status));
            return(status);
         }
      ...
      if (!_wcsicmp(pControllerKeyInformation->Name, L"ScsiAdapter")) {
         ...
         if (!NT_SUCCESS(status)) {
            DebugPrint ((2, "FindScsiAdapter:  ZwOpenKey got status = %lx\n", status));
            return (status);
         }
         ...
         for (controllerNumberIndex = 0; TRUE; controllerNumberIndex++) {
            ...
            if (!NT_SUCCESS(status)) {
               ...
               if (status != STATUS_NO_MORE_ENTRIES) {
                  DebugPrint ((2, "FindScsiAdapter: Status %lx enumerating key\n", status));
                  ZwClose(controllerHandle);
                  return (status);
               }
            ...
            if (validControllerNumber) {
               ...
               if (!NT_SUCCESS(status)) {
                  DebugPrint ((2, "FindScsiAdapter: Status %lx opening controller number key\n", status));
                  ZwClose(controllerHandle);
                  return (status);
               }
