...

{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        if(status == STATUS_OBJECT_NAME_COLLISION) {
            ...
            DebugPrint((0, "TapeClassInitialize: Error %x allocating driver extension.\n",
                            status));
    ...
    if (!NT_SUCCESS(status)) {
        DebugPrint((0, "TapeClassInitialize: Error %x from classinit\n", status));
...
Routine Description:
    ...
    This routine is the dispatcher for WMI requests. It looks at the IRP_MN code and calls the appropriate tape device
    routine if data collection is needed or carries out the request for admin-type functions.
...

{
    PIO_STACK_LOCATION              irpStack = IoGetCurrentIrpStackLocation(Irp);
    PFUNCTIONAL_DEVICE_EXTENSION    fdoExtension = DeviceObject->DeviceExtension;
    PTAPE_INIT_DATA_EX              tapeInitData = (PTAPE_INIT_DATA_EX) (fdoExtension->CommonExtension.DriverData);
    ...
    //
    // Determine if this WMI request is ours.
    //
    ...
    switch (irpStack->MinorFunction) {
        ...
        //
        // These are handled at this level and don't need to be sent to the miniclass driver.
        //
    ...
    for (callNumber = 0; ;callNumber++) {
        ...
        tapeStatus = tapeInitData->TapeWMIOperations(minitapeExtension,
                                                     commandExtension,
                                                     Irp->AssociatedIrp.SystemBuffer,
                                                     &srb,
                                                     callNumber,
    ...
    Irp->IoStatus.Information = sizeof(TAPE_STATISTICS);
    Irp->IoStatus.Status = status;
    ClassReleaseRemoveLock(DeviceObject, Irp);
    ClassCompleteRequest(DeviceObject, Irp, IO_NO_INCREMENT);
