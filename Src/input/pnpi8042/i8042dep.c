...
#if defined(NEC_98)
        //
        // There is nothing in i8251 output buffer.
        // We regard this case as overrun.
        //
        *Byte = (UCHAR)0xff;
#endif // defined(NEC_98)
        return;
...
--*/
{
...
#ifdef NEC_98
    Globals.KeyboardExtension->KeyboardTypeNEC = -1;
#else
    //
    ...
    I8xInitializeHardware(KeyboardStatus, MouseStatus, TRUE);

#ifndef NEC_98
    if ((NT_SUCCESS(*KeyboardStatus) || NT_SUCCESS(*MouseStatus)) &&
...

{
    ...
    if (keyboardExtension) {
        kbOutstandingPowerIrp = keyboardExtension->OutstandingPowerIrp;
        keyboardExtension->OutstandingPowerIrp = NULL;
#ifdef NEC_98
...
#ifndef NEC_98
    if (mouseExtension) {
        mouOutstandingPowerIrp = mouseExtension->OutstandingPowerIrp;
        mouseExtension->OutstandingPowerIrp = NULL;
    }
    ...
    I8xInitializeHardware(&keyboardStatus, &mouseStatus, FALSE);

    //
    // Reset PoweredDevices so that we can keep track of the powered device
    //  the next time the machine is power managed off.
    //
    Globals.PoweredUpDevices = 0;
    CLEAR_POWERUP_FLAGS();

    //
    // Devices might have come and gone, reset started devices and increment
    // it back up below
    //
    Globals.StartedDevices = 0;
    ...
    if (NT_SUCCESS(mouseStatus) || mouseStatus == STATUS_IO_TIMEOUT) { 
        Globals.StartedDevices++;
    ...
    if (NT_SUCCESS(keyboardStatus)) {
#ifdef NEC_98
        I8xKeyboardConnectInterrupt(keyboardExtension);
#endif
        Globals.StartedDevices++;
...

{
    ...
    BOOLEAN     canTouchKeyboard, canTouchMouse;
    ...
    if (FirstInit) {
        ...
        canTouchKeyboard = KEYBOARD_POWERED_UP_SUCCESS() ? TRUE : FALSE;
        canTouchMouse = MOUSE_POWERED_UP_SUCCESS() ? TRUE : FALSE;
    }
   ...
   *KeyboardStatus =
    ...
    if (!NT_SUCCESS(*KeyboardStatus)) {
        ...
        I8xManuallyRemoveDevice((PCOMMON_DATA) Globals.KeyboardExtension);
        }
#endif
    ...
    if (KEYBOARD_PRESENT() && FirstInit == FALSE && canTouchKeyboard &&
        Globals.KeyboardExtension->ShutdownType  == PowerActionSleep) {
    ...
    *MouseStatus =
    ...
    if (!NT_SUCCESS(*MouseStatus)) {
        ...
        I8xManuallyRemoveDevice((PCOMMON_DATA) Globals.MouseExtension);
    }
#endif
    ...
    if (MOUSE_PRESENT() && FirstInit == FALSE && canTouchMouse &&
        Globals.MouseExtension->ShutdownType  == PowerActionSleep) {
        I8xPutBytePolled((CCHAR) CommandPort,
                         NO_WAIT_FOR_ACKNOWLEDGE,
                         (CCHAR) UndefinedDeviceType,
                         (UCHAR) I8042_DISABLE_MOUSE_DEVICE
    ...
    if (KEYBOARD_PRESENT() && canTouchKeyboard) { 
        ASSERT(Globals.KeyboardExtension);

        *KeyboardStatus = I8xInitializeKeyboard();
        if (!NT_SUCCESS(*KeyboardStatus)) {
            Print(DBG_SS_ERROR,
...
#else
            I8xManuallyRemoveDevice((PCOMMON_DATA) Globals.KeyboardExtension);
#endif // defiend(WINDOWS_FE) && defined(_X86_)
    ...
    if (MOUSE_PRESENT() && canTouchMouse) {
        ASSERT(Globals.MouseExtension);

        *MouseStatus = I8xInitializeMouse();
        Print(DBG_SS_NOISE, ("I8xInitializeMouse:  status = 0x%x, FirstInit = %d\n",
                             *MouseStatus, (ULONG) FirstInit));

        if (!NT_SUCCESS(*MouseStatus) && FirstInit) {
            Print(DBG_SS_ERROR,
    ...
    if (KEYBOARD_PRESENT() && NT_SUCCESS(*KeyboardStatus) && !NT_SUCCESS(*MouseStatus) &&
        canTouchKeyboard && FirstInit) {
        NTSTATUS status;
        ...
        if ((status = I8xPutBytePolled(
            ...
            status = I8xPutBytePolled(
                                 ...
                                 (CCHAR) KeyboardDeviceType,
#if defined(NEC_98)
                                 (UCHAR) (Globals.KeyboardExtension->KeyboardIndicators.LedFlags | 0x0070)
#else // defined(NEC_98)
                                 (UCHAR) Globals.KeyboardExtension->KeyboardIndicators.LedFlags
#endif // defined(NEC_98)
                                 );
...

{
...
#if defined(NEC_98)
    while ((i++ < (ULONG)Globals.ControllerData->Configuration.PollingIterations) &&
           (I8X_GET_STATUS_BYTE(Globals.ControllerData->DeviceRegisters[CommandPort]) & PC98_8251_BUFFER_EMPTY)
            != PC98_8251_BUFFER_EMPTY) {
        ...
        Print(DBG_BUFIO_NOISE,
             ("I8xPutByteAsynchronous: wait for NEC98 8251 BUFFER EMPTY\n"
             ));
    ...
    if (PortType == CommandPort) {
        ...
        if (status != STATUS_SUCCESS){
            Print(DBG_BUFIO_INFO | DBG_BUFIO_ERROR,
                 ("I8xPutByteAsynchronous: KeyboardCommand Time out\n"
                 ));
        }
#else // defined(NEC_98)
...

{
    ...
    for (j=0;j < (ULONG)Globals.ControllerData->Configuration.ResendIterations;j++) {
        ...
        if (PortType == CommandPort) {
            ...
            status = NEC98_KeyboardCommandByte(Byte);
            if (status != STATUS_SUCCESS){
                Print(DBG_BUFIO_ERROR | DBG_BUFIO_INFO,
                     ("I8xPutBytePolled: KeyboardCommand Time out exit\n"
                     ));
