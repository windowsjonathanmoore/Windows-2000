        ...
        if (byte == RESEND) {
            ...
            if (deviceExtension->CurrentOutput.CurrentByte <
                ...
                IoRequestDpc(
                    ...
                    (PVOID) IsrDpcCauseMouseWriteComplete 
                    );
            }
...
SwitchOnInputResetSubState:
            ...
            case EnableWheel:
                ...
                deviceExtension->PostSamplesState = Enable5Buttons;
                break;
            ...
            case ExpectingGetDeviceIdValue: 
                ...
                if  (1) {
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID) (ULONG) I8042_BOGUS_MOUSE_RESET
                                     );
            ...
            case ExpectingSetResolutionACK:
                ...
                if (byte == (UCHAR) ACKNOWLEDGE) {
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID) (ULONG) I8042_SET_RESOLUTION_FAILED
                                     );
            ...
            case ExpectingSetResolutionValueACK:
                ...
                if (byte == (UCHAR) ACKNOWLEDGE) {
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID)(ULONG) I8042_SET_SAMPLE_RATE_FAILED
                                     );
            ...
            case ExpectingSetScaling1to1ACK3:
                ...
                if (byte == (UCHAR) ACKNOWLEDGE) {
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID)(ULONG) I8042_ERROR_DURING_BUTTONS_DETECT
                                     );
            ...
            case ExpectingGetDeviceId2ACK: 
                ...
                if (byte == (UCHAR) ACKNOWLEDGE) {
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID)(ULONG) I8042_GET_DEVICE_ID_FAILED
                                     );
            ...
            case ExpectingGetDeviceId2Value:
                ...
                default:
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID) (ULONG) I8042_MOU_RESET_RESPONSE_FAILED
                                     );
            ...
            case ExpectingEnableACK: 
                ...
                if (byte == (UCHAR) ACKNOWLEDGE) {
                    ...
                    IoRequestDpc(DeviceObject,
                                 // DeviceObject->CurrentIrp,
                                 deviceExtension->ResetIrp,
                                 (PVOID) IsrDpcCauseMouseResetComplete
                                 );
                }
                else if (byte == (UCHAR) RESEND) {
                    ...
                    KeInsertQueueDpc(&deviceExtension->ErrorLogDpc,
                                     (PIRP) NULL,
                                     (PVOID) (ULONG) I8042_ENABLE_FAILED
                                     );
...
#define DUMP_COUNT 4
    ...
    //
    deviceExtension = Globals.MouseExtension;
    ...
    for (i = 0; i < I8X_POLL_ITERATIONS_MAX / 2; i++) {
...
#define DUMP_COUNT 4
    ...
    // UCHAR       pnpID[] = { 0x32, 0x1f, 0x23, 0x0b, 0x0b, 0x0b, 0x03, 0x00 };
    ULONG                   dumpCount = 0;
    ULONG                   dumpData[DUMP_COUNT];
    ULONG                   i;
    ULONG                   idCount;
    ...
    if (MouseExtension->EnableWheelDetection == 1) {
        ...
        //
        while (pnpCommands[commandCount] != 0) {

            status = I8xPutBytePolled(
                (CCHAR) DataPort,
                WAIT_FOR_ACKNOWLEDGE,
                (CCHAR) MouseDeviceType,
                pnpCommands[commandCount]
                );
            ...
            if (!NT_SUCCESS(status)) {
                ...
                dumpData[1] = DataPort;
                dumpData[2] = I8042_WRITE_TO_AUXILIARY_DEVICE;
                dumpData[3] = pnpCommands[commandCount];
                dumpCount = 4;
...
InitializeWheel:
    commandCount = 0;
    while(enableCommands[commandCount] != 0) {

        //
        // Put the byte to the mouse
        //
        status = I8xPutBytePolled(
            (CCHAR) DataPort,
            WAIT_FOR_ACKNOWLEDGE,
            (CCHAR) MouseDeviceType,
            enableCommands[commandCount]
            );
        ...
        if(!NT_SUCCESS(status)) {
            ...
            dumpData[1] = DataPort;
            dumpData[2] = I8042_WRITE_TO_AUXILIARY_DEVICE;
            dumpData[3] = enableCommands[commandCount];
            dumpCount = 4;
    ...
    if ((!NT_SUCCESS(status)) ||
       ((byte != MOUSE_ID_BYTE) && (byte != WHEELMOUSE_ID_BYTE))) {
        ...
        Print(DBG_SS_INFO,
              ("%s, wheel mouse attached - running in wheel mode.\n",
              pFncFindWheelMouse
              ));
...
--*/
{
    ...
    // the counts there.
    //
    //MouseExtension->ResetCount = 0;
    //MouseExtension->ResendCount = 0;

    //
    // Make sure the next packet is started, irregardless if the reset IRP
    // was present or not
    //
...
--*/
{
    PDEVICE_OBJECT     self = MouseExtension->Self;
    ...
    //
    if (!MouseExtension->ResetIrp) {
        InterlockedExchangePointer(&MouseExtension->ResetIrp,
                                   IoAllocateIrp(self->StackSize, FALSE)
                                   );

        if (!MouseExtension->ResetIrp) {
            //
            // Couldn't allocate the irp, just send off the reset and hope it
            // doesn't conflict with a kb set IOCTL
            //
            I8xSendResetCommand(MouseExtension);
            return STATUS_SUCCESS;
        }

        // IoInitializeIrp(MouseExtension->ResetIrp,
        //                 IoSizeOfIrp(self->StackSize), 
        //                 self->StackSize
        //                 );
        // IoResetIrp(MouseExtension->ResetIrp, STATUS_SUCCESS);
        MouseExtension->ResetIrp->Cancel = FALSE ;
        MouseExtension->ResetIrp->PendingReturned = FALSE ;
        MouseExtension->ResetIrp->IoStatus.Status = STATUS_SUCCESS;
    
        stack = IoGetNextIrpStackLocation(MouseExtension->ResetIrp);
        stack->MajorFunction = IRP_MJ_INTERNAL_DEVICE_CONTROL;
        stack->Parameters.DeviceIoControl.IoControlCode = IOCTL_INTERNAL_MOUSE_RESET; 
        IoSetNextIrpStackLocation(MouseExtension->ResetIrp);

        IoStartPacket(self,
                      MouseExtension->ResetIrp,
                      (PULONG) NULL,
                      NULL
                      );
    }
...
--*/
{
    PWSTR       str = NULL;
    ULONG       length;
    WCHAR       szDefaultIDs[] = { L"MSH0002\0MSH0005\0MSH0010\0\0" };
    ...
    Destination->MaximumLength = Destination->Length = 
        Source->MaximumLength + sizeof(szDefaultIDs) / sizeof(WCHAR) + 1;
    Destination->Buffer =
        ExAllocatePool(NonPagedPool, Destination->MaximumLength *sizeof(WCHAR));

    if (Destination->Buffer) {
        //
        // copy the default IDs first
        //
        RtlZeroMemory(Destination->Buffer, Destination->MaximumLength);
        str = szDefaultIDs; 
        while ((length = wcslen(str)) != 0) {
            length++;
            RtlCopyMemory(Destination->Buffer + (str-szDefaultIDs),
                          str,
                          length * sizeof(WCHAR)
                          );
            str += length;
