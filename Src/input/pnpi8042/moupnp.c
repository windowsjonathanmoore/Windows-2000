...
Arguments:
    ...
    SyncConnectContext -  Structure to be filled in if synchronization needs to 
                          take place between this interrupt and the keyboard
                          interrupt. 
...
--*/
{
    ...
    if (NT_SUCCESS(keyboardStatus)) { 
        ...
        ASSERT( KEYBOARD_INITIALIZED() && !MANUALLY_REMOVED(KeyboardExtension));
    ...
    if (NT_SUCCESS(mouseStatus)) {
        ...
        //
        mouseStatus = I8xMouseConnectInterruptAndEnable(MouseExtension);
    }
...
--*/
{
    ...
    if (NT_SUCCESS(status) && MOUSE_PRESENT()) {
        ...
        //
        // Since we are at passive, no need to queue a dpc, just do the work here
        //
        ...
        //
        // IoFreeController, IoStartNextPacket must be called at dispatch
        //
...
--*/
{
    ...
    while ((length = wcslen(currentString)) != 0) {
