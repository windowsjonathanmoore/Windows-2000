...
--*/
{
    ...
    if (NT_SUCCESS(mouseStatus)) {
        ...
        ASSERT( MOUSE_INITIALIZED() && !MANUALLY_REMOVED(MouseExtension));

        //
        // Any errors will be logged by I8xMouseConnectInterruptAndEnable
        //
        status = I8xMouseConnectInterruptAndEnable(MouseExtension);
...
--*/
{
    ...
    //
    KeyboardExtension->Dump1Keys = 0;
    KeyboardExtension->Dump2Key = 0;
    KeyboardExtension->DumpFlags = 0;
    ...
    // the "Crash Dump" key
    //
    if (KeyboardExtension->Dump1Keys == 0x0) {
        //
