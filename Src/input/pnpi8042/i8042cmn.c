...
// #if defined(_X86_) && defined(FE_SB)
#pragma alloc_text(PAGE, I8xServiceCrashDump)
// #endif // defined(_X86_) && defined(FE_SB)
...
--*/
{
    ...
    case IOCTL_INTERNAL_MOUSE_RESET:
        ...
        //
        // No need to complete the irp b/c we sent it to ourself
        //
...
--*/
{
    ...
    case IOCTL_KEYBOARD_QUERY_INDICATORS:
        ...
        if (irpSp->Parameters.DeviceIoControl.OutputBufferLength <
            ...
            *(PKEYBOARD_INDICATOR_PARAMETERS) Irp->AssociatedIrp.SystemBuffer =
                kbExtension->KeyboardIndicators;
#if defined(FE_SB) && !defined(NEC_98)
...
#if defined(NEC_98)
        if ((GET_COMMON_DATA(DeviceObject->DeviceExtension)->PowerState) != PowerDeviceD0) {
            Print(DBG_POWER_INFO,
                  ("I8xInternalDeviceControl: PowerState is not D0, the request is ignored.\n"
                  ));
            status = STATUS_SUCCESS;
            break;
        }
        //
        // Katakana keyboard indicator support on PC9800 keyboard
        //
#else  // defined(NEC_98)
        //
...
--*/
{
    ...
    //
    // All of the write requests (including the mice ones) go through the kb
    // StartIo routine (if it exists)
    //
    IoFreeController(Globals.ControllerData->ControllerObject);
    IoStartNextPacket(DeviceObject, FALSE);
...
#if defined(NEC_98)
        ic.Bytes[1] &= (KEYBOARD_CAPS_LOCK_ON | KEYBOARD_KANA_LOCK_ON
                         | KEYBOARD_SCROLL_LOCK_ON | KEYBOARD_NUM_LOCK_ON);
        ic.Bytes[1] |= 0x70;

        KeSynchronizeExecution(
                commonData->InterruptObject,
                (PKSYNCHRONIZE_ROUTINE) NEC98_SetIndicators,
                (PVOID) &ic 
                );

#if DBG
        ic.Bytes[0] = ic.Bytes[1] = (UCHAR) 0x0;
        Globals.ControllerData->CurrentIoControlCode = 0x0;
#endif
...
--*/
{
    ...
    //
    // If the time out is on a mouse IOCTL, then the irp actuall resides
    // in the kbd's irp queue
    //
    IoAcquireCancelSpinLock(&cancelIrql);
...

{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (!NT_SUCCESS(status)) {
            Print(DBG_SS_NOISE,
                 ("I8xServiceCrashDump: RtlQueryRegistryValues failed with 0x%x\n",
                 status
                 ));
        }
    }
    ...
    Print(DBG_SS_INFO,
         ("I8xServiceCrashDump: Dump1Keys = %d\n",
         DeviceExtension->Dump1Keys
         ));
    Print(DBG_SS_INFO,
         ("I8xServiceCrashDump: Dump2Key = %d\n",
         DeviceExtension->Dump2Key
         ));
        ...
        if (Globals.RegistersMapped) {
            ...
            for (i=0; i < Globals.ControllerData->Configuration.PortListCount; i++) {
                Globals.ControllerData->DeviceRegisters[i] = (PUCHAR)
                    MmMapIoSpace(
                        Globals.ControllerData->Configuration.PortList[i].u.Port.Start,
                        Globals.ControllerData->Configuration.PortList[i].u.Port.Length,
                        MmNonCached
                        );
            }
        }
            ...
            for (i=0; i < Globals.ControllerData->Configuration.PortListCount; i++) {
                Globals.ControllerData->DeviceRegisters[i] = (PUCHAR)
                    Globals.ControllerData->Configuration.PortList[i].u.Port.Start.LowPart;
            }
        }
    }
...
Routine Description:
    ...
    This routine is called from StartIo synchronously.
    It sets up the new indicator status and set it to kb hardare.
...
--*/
{
    PPORT_KEYBOARD_EXTENSION kbExtension = InitiateContext->DeviceObject->DeviceExtension;
    PUCHAR                   bytes = InitiateContext->Bytes;

    Print(DBG_CALL_TRACE, ("NEC98_SetIndicators: enter\n"));
    ...
    //
    // NOTE:NUM key on Laptop and NOTE keyboard.
    //      Keyboard driver must be read NUM key status before send led set commands.
    //      Laptop and NOTE keyboard have NUM key lock status on LED status
    //      that is cleared by LED set commands. keyboard driver cannot know when
    //      NUM key ware hit, because PC9800 laptop keyboard NUM key don't make scancode.
    //

    if (kbExtension->KeyboardAttributes.KeyboardIdentifier.Subtype == (UCHAR)4) {
        UCHAR LED_Status;

        //
        // Get the keyboard indicator lights.  Ignore errors.
        //
        if (I8xPutBytePolled(
                (CCHAR) DataPort,
            ...
            Print(DBG_SS_ERROR | DBG_SS_INFO,
                  ("NEC98_SetIndicators: could not send SET LEDS cmd\n"
                  ));
        } else if (I8xPutBytePolled(
                       (CCHAR) DataPort,
                       WAIT_FOR_ACKNOWLEDGE,
                       (CCHAR) KeyboardDeviceType,
                       (UCHAR) GET_KEYBOARD_INDICATORS
                       ) != STATUS_SUCCESS) {
            Print(DBG_SS_ERROR | DBG_SS_INFO,
                  ("NEC98_SetIndicators: could not send GET LEDS param.\n"
                  ));
        }
        else {
            Print(DBG_SS_INFO, ("NEC98_SetIndicators: waiting for LED status.\n"));
        ...
        bytes[1] &= 0xfc;
        bytes[1] |= (LED_Status & KEYBOARD_PC98_LAPTOP_NUMKEY_ON);
    }

    //
    // Set the keyboard indicator lights.
    //
    if (I8xPutBytePolled(
            (CCHAR) DataPort,
            WAIT_FOR_ACKNOWLEDGE,
        ...
        Print(DBG_SS_ERROR | DBG_SS_INFO,
              ("NEC98_SetIndicators: could not send LEDS param.\n"
              ));
    }

    KeStallExecutionProcessor(3000);

    Print(DBG_CALL_TRACE, ("NEC98_SetIndicators: exit\n"));
}
#endif // defined(NEC_98)
...
#if defined(NEC_98)
    if (commonData->CurrentOutput.Bytes[0] == SET_KEYBOARD_TYPEMATIC) {
        Print(DBG_SS_INFO,
              ("I8xInitiateOutputWrapper: Set PC-9800 Keyrepeat Stop\n"
              ));
        commonData->CurrentOutput.Bytes[1] = (UCHAR) PC98_STOP_AUTO_REPEAT;
    }
#endif // defined(NEC_98)
