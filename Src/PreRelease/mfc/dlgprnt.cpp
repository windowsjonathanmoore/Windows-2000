...
void CPageSetupDialog::Dump(CDumpContext& dc) const
{
	...
	dc << "m_psd.hwndOwner = " << (UINT_PTR)m_psd.hwndOwner;
	dc << "\nm_psd.Flags = " << (LPVOID)m_psd.Flags;

	RECT rc;
	memcpy(&rc, &m_psd.rtMinMargin, sizeof(RECT));  // Take care of alignment fixup
	dc << "\nm_psd.ptPaperSize = " << m_psd.ptPaperSize;
	dc << "\nm_psd.rtMinMargin = " << rc;
	dc << "\nm_psd.rtMinMargin = " << rc;
...
void CPrintDialog::Dump(CDumpContext& dc) const
{
	...
	dc << "m_pd.hwndOwner = " << (UINT_PTR)m_pd.hwndOwner;

	if (m_pd.hDC != NULL)
		dc << "\nm_pd.hDC = " << CDC::FromHandle(m_pd.hDC);

	dc << "\nm_pd.Flags = " << m_pd.Flags;
