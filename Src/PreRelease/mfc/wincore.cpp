		...
		if (pWndInit != NULL)
		{
				...
				(dwFlags = AfxCallWndProc(pWndInit, hWnd, WM_QUERY3DCONTROLS)) != 0)
			{
				...
				BOOL bAfxWndProc = ((WNDPROC)
					GetWindowLongPtr(hWnd, GWLP_WNDPROC) == afxWndProc);
			...
			if (oldWndProc != NULL && GetProp(hWnd, _afxOldWndProc) == NULL)
			{
				...
				if ((WNDPROC)GetProp(hWnd, _afxOldWndProc) == oldWndProc)
				{
					...
					SetWindowLongPtr(hWnd, GWLP_WNDPROC,
						INT_PTR(pThreadState->m_bDlgCreate ?
...
#ifdef _WIN64
		wsprintf(lpszName, _T("Afx:%p:%x:%p:%p:%p"), hInst, nClassStyle,
...
void CWnd::WinHelp(DWORD_PTR dwData, UINT nCmd)
{
   ...
   //IA64: WINBUG: last parameter to WinHelp should be 64-bit
	if (!::WinHelp(pWnd->m_hWnd, pApp->m_pszHelpFilePath, nCmd, DWORD(dwData)))
...
LDispatch:
	...
	case AfxSig_vwww:
		(this->*mmf.pfn_vwww)((UINT)wParam, LOWORD(lParam), HIWORD(lParam));
		break;

	case AfxSig_vwii:
		(this->*mmf.pfn_vwii)((UINT)wParam, LOWORD(lParam), HIWORD(lParam));
		break;
	...
	case AfxSig_vwW:
		(this->*mmf.pfn_vwW)((UINT)wParam, CWnd::FromHandle((HWND)lParam));
		break;
...
BOOL CWnd::SubclassWindow(HWND hWnd)
{
	...
	else if (*lplpfn != oldWndProc)
	{
		...
		TRACE3("\thWnd = $%08X (nIDC=$%08X) is not a %hs.\n", (UINT)(UINT_PTR)hWnd,
