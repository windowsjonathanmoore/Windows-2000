...
#ifdef _IA64_
		memset(pStack, 0, nSizeArgs+_SCRATCH_SIZE);  // Zero out to prevent NaTVals on IA64
#endif
		...
		for (pb = pbParams; *pb != '\0'; ++pb)
		{
			...
			if (*pb == IT_RAW)
			{
				...
				else
				{
					...
					pStack = StoreRawStackParameter(pStack, IT_PSTR, pbParam, sizeof(LPTSTR));
					pStack = StoreRawStackParameter(pStack, IT_I4,
						(BYTE*) &(pCtxt->m_dwBytesReceived), sizeof(DWORD));
#else
