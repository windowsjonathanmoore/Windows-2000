	...
	CDBVariant& varValue, short nFieldType)
{
	...
	LONG_PTR nActualSize = GetData(m_pDatabase, m_hstmt, nIndex,
...
void CRecordset::GetFieldValue(short nIndex, CString& strValue)
{
	...
	INT_PTR nLen = GetTextLen(m_rgODBCFieldInfos[nIndex - 1].m_nSQLType,
			m_rgODBCFieldInfos[nIndex - 1].m_nPrecision);
...
#endif  // _WIN64
	void* pvData = strData.GetBufferSetLength((int)nLen);
...
LONG_PTR CRecordset::GetLBFetchSize(LONG_PTR lOldSize)
{
	// Make it twice as big
	return lOldSize << 1;
}

LONG_PTR CRecordset::GetLBReallocSize(LONG_PTR lOldSize)
{
...
void* PASCAL CRecordset::GetDataBuffer(CDBVariant& varValue,
	short nFieldType, INT_PTR* pnLen, short nSQLType, SQLULEN nPrecision)
{
	void* pvData = NULL;
	...
	case SQL_C_CHAR:
		...
		*pnLen = GetTextLen(nSQLType, nPrecision);
		...
		if (*pnLen > INT_MAX)
			AfxThrowMemoryException();
#endif
		pvData = varValue.m_pstring->GetBufferSetLength((int)*pnLen);
		break;
...
INT_PTR PASCAL CRecordset::GetTextLen(short nSQLType, SQLULEN nPrecision)
{
	INT_PTR nLen;
...
LONG_PTR PASCAL CRecordset::GetData(CDatabase* pdb, HSTMT hstmt,
	short nFieldIndex, short nFieldType, LPVOID pvData, INT_PTR nLen,
	short nSQLType)
{
	UNUSED(nSQLType);
...
void PASCAL CRecordset::GetLongCharDataAndCleanup(CDatabase* pdb,
	HSTMT hstmt, short nFieldIndex, LONG_PTR nActualSize, LPVOID* ppvData,
	INT_PTR nLen, CString& strValue, short nSQLType)
{
	RETCODE nRetCode;
...
void PASCAL CRecordset::GetLongBinaryDataAndCleanup(CDatabase* pdb, HSTMT hstmt,
	short nFieldIndex, LONG_PTR nActualSize, LPVOID* ppvData, INT_PTR nLen,
