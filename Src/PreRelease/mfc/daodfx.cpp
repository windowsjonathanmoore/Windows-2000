...
void AFX_CDECL AllocLongBinary(CLongBinary& lb, DWORD_PTR dwDataLength);
	...
	case AFX_RFX_LONGBINARY:
		{
			...
			memcpy(pbDest, pbSrc, (size_t)pLongBinarySrc->m_dwDataLength);
	...
	case AFX_RFX_BINARY:
		{
			...
			else
			{
				...
				if (memcmp(&pByteArrayDest[0], &pByteArraySrc[0], (size_t)nSize) != 0)
					bDirty = TRUE;
			}
		}
		break;
