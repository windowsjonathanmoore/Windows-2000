...
void CFieldExchange::Default(LPCTSTR szName,
	void* pv, LONG_PTR* plLength, int nCType, UINT_PTR cbValue, UINT_PTR cbPrecision)
{
	RETCODE nRetCode;
	...
	case BindParam:
		...
		m_prs->m_mapParamIndex.SetAt(pv, (void*)(INT_PTR)nField);
		return;
	...
	case BindFieldToColumn:
		...
		m_prs->m_mapFieldIndex.SetAt(pv, (void*)(INT_PTR)nField);
		return;
	...
	case CFieldExchange::BindParam:
		{
			...
			*plLength = pFX->m_prs->IsParamStatusNull(nField - 1) ?
			...
			// Add the member address to the param map
			pFX->m_prs->m_mapParamIndex.SetAt(value, (void*)(INT_PTR)nField);
		}
		return;
	...
	case CFieldExchange::BindFieldToColumn:
		{
			...
			UINT_PTR cbColumn = pODBCInfo->m_nPrecision;
			...
			if (cbColumn > (UINT_PTR)nMaxLength || cbColumn < 256)
				cbColumn = nMaxLength;
			...
			// Add the member address to the field map
			pFX->m_prs->m_mapFieldIndex.SetAt(value, (void*)(INT_PTR)nField);
		}
		return;
	...
	case CFieldExchange::BindFieldToColumn:
		{
			...
			AFX_SQL_SYNC(::SQLBindCol(pFX->m_prs->m_hstmt, (UWORD)nField,
				SQL_C_BINARY, &value[0], cbColumn, plLength));
			if (!pFX->m_prs->Check(nRetCode))
				pFX->m_prs->ThrowDBException(nRetCode);
...
LONG_PTR CFieldExchange::GetLongBinarySize(int nField)
{
	RETCODE nRetCode;
	int nDummy;
	LONG_PTR lSize;
...
{
	RETCODE nRetCode;
	LONG_PTR lActualDataSize = 0;
	LONG_PTR lChunkDataSize;
	LONG_PTR lReallocSize;
	...
	do
	{
		...
		lpLongBinary = ReallocLongBinary(lb,
			...
			lb.m_dwDataLength + lReallocSize);
...
BYTE* CFieldExchange::ReallocLongBinary(CLongBinary& lb, LONG_PTR lSizeRequired,
	LONG_PTR lReallocSize)
{
...
void AFXAPI AfxRFXBulkDefault(CFieldExchange* pFX, LPCTSTR szName,
	void* pv, LONG_PTR* rgLengths, int nCType, UINT_PTR cbValue)
{
	RETCODE nRetCode;
