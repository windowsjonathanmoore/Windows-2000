	...
	DBCOLUMNID* pcidDst, DBCOLUMNID const *pcidSrc)
{
	...
	case DBCOLKIND_GUID_NAME:
		...
		// fall through
	case DBCOLKIND_NAME:
		pcidDst->lpdbsz = (LPDBSTR) ::CoTaskMemAlloc(sizeof(DBCHAR) * UINT((ldbstrlen(pcidSrc->lpdbsz) + 1)));
		if (!pcidDst->lpdbsz)
			return FALSE;
...
void CDataSourceControl::BindColumns()
{
	...
	{
		m_nBindings += (int)m_pMetaRowData[nCol].m_pClientList->GetCount();
	}
	if (m_nBindings > 0)
			...
			while (pos)
			{
				...
				if (pSite->m_bIsDirty)
				{
					...
					case DBTYPE_WSTR:
						...
						wcsncpy((wchar_t*)m_pDynamicAccessor->GetValue(nCol), var.bstrVal, (size_t)m_pDynamicAccessor->m_pColumnInfo[nCol].ulColumnSize);
						break;
					case DBTYPE_BOOL:
					...
					case DBTYPE_STR:
						...
						WideCharToMultiByte(CP_ACP, 0, var.bstrVal, -1, (char*)m_pDynamicAccessor->GetValue(nCol),
											(ULONG)m_pDynamicAccessor->m_pColumnInfo[nCol].ulColumnSize, NULL, NULL);
						break;
					case DBTYPE_DBTIMESTAMP:
						DBTIMESTAMP ts;
						SYSTEMTIME	sysTime;
