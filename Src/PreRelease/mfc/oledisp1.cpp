...
void CCmdTarget::EnableAutomation()
{
	...
	// sizeof(COleDispatchImpl) should be just a DWORD_PTR (vtable pointer)
...
CCmdTarget* PASCAL CCmdTarget::FromIDispatch(LPDISPATCH lpDispatch)
{
	...
	ASSERT(*(DWORD_PTR*)&dispatch != 0);    // null vtable ptr?
	if (*(DWORD_PTR*)lpDispatch != *(DWORD_PTR*)&dispatch)
...
#ifdef _WIN64
		case VT_I8|VT_BYREF:
			*(_STACK_PTR*)pStack = (_STACK_PTR)pArg->pllVal;
			pStack += sizeof(_STACK_PTR);
			break;
#endif
		case VT_R4|VT_BYREF:
	...
	VARIANT* pvarResult, DISPPARAMS* pDispParams, UINT* puArgErr)
{
	...
	BYTE* pStack = (BYTE*)_alloca(nSizeArgs + _SCRATCH_SIZE + _ALIGN_STACKPOINTER);
	if (pStack == NULL)
	{
		TRACE0("Error: stack overflow in IDispatch::Invoke!\n");
		return E_OUTOFMEMORY;
	}
	pStack = (BYTE*)((((DWORD_PTR)pStack)+(_ALIGN_STACKPOINTER-1))&~(_ALIGN_STACKPOINTER-1));
#ifdef _IA64_
	memset(pStack, 0, nSizeArgs+_SCRATCH_SIZE);  // Zero out stack to prevent NaTVals on IA64
#endif
...
#ifdef _WIN64
		case VT_I8:
			pvarResult->llVal = (LONGLONG)dwResult;
			break;
#endif
		case VT_R4:
