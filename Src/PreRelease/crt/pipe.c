        ...
        )
{
        ...
        if ((phandles[0] = _alloc_osfhnd()) != -1) {
                ...
                if ((phandles[1] = _alloc_osfhnd()) != -1) {
                        ...
                        _set_osfhnd(phandles[0], (intptr_t)ReadHandle);
                        _set_osfhnd(phandles[1], (intptr_t)WriteHandle);
                        errno = 0;
#ifdef _MT
                        lock1_flag = 1;     /* unlock handle */
#endif  /* _MT */
                }
                else {
        ...
        __finally {
                if ( lock1_flag )
                        _unlock_fh( phandles[1] );

                if ( lock0_flag )
                        _unlock_fh( phandles[0] );
        }
#endif  /* _MT */
