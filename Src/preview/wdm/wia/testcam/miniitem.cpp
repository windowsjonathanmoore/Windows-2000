    ...
    PMINIDRV_TRANSFER_CONTEXT  pDataTransferContext)
{
   ...
   //
   // for tymed file or hglobal, only IMGFMT_MEMORYBMP is allowed
   //

   if ((pDataTransferContext->tymed == TYMED_FILE) ||
       (pDataTransferContext->tymed == TYMED_HGLOBAL)) {

       if (pDataTransferContext->guidFormatID != IMGFMT_BMP && pDataTransferContext->guidFormatID != AUDFMT_WAV) {
   ...
   //
   // for tymed CALLBACK, only IMGFMT_MEMORYBMP is allowed
   //

   if (pDataTransferContext->tymed == TYMED_CALLBACK) {

       if ((pDataTransferContext->guidFormatID != IMGFMT_BMP) &&
           (pDataTransferContext->guidFormatID != IMGFMT_MEMORYBMP)) {
    ...
    PMINIDRV_TRANSFER_CONTEXT   pTranCtx)
{
    ...
    if (pTranCtx->guidFormatID == IMGFMT_MEMORYBMP) {
...
// #define TEST_PER_USER_DATA 1
    ...
    *plDevErrVal = 0;
    ...
    if (!IsEqualGUID (pTranCtx->guidFormatID, AUDFMT_WAV) )
    {
