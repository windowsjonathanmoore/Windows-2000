...
*
*  DESCRIPTION:
*   Implementation of a WIA test camera.
*
*******************************************************************************/
...
* CamLoadPicture
*
*    Load a bmp from disk and copy it to application. A real driver would
    ...
    PLONG                       plCamErrVal)
{
    ...
    if (pDataTransCtx->guidFormatID != IMGFMT_BMP && pDataTransCtx->guidFormatID != AUDFMT_WAV) {
        return (E_NOTIMPL);
    }
    ...
    if (pDataTransCtx->guidFormatID == IMGFMT_BMP)
    {
...
*    A real driver would read the image data from the device.
*
* Arguments:
*
    ...
    PLONG                      plCamErrVal)
{
    ...
    if ((pTransCtx->guidFormatID != IMGFMT_BMP) &&
        (pTransCtx->guidFormatID != IMGFMT_MEMORYBMP)) {
        return (E_NOTIMPL);
    }
    ...
    //
    // Flip the image vertically if IMGFMT_MEMORYBMP is requested
    //

    if (pTransCtx->guidFormatID == IMGFMT_MEMORYBMP) {
        VerticalFlip(
    ...
    //
    // Flip the image back if IMGFMT_MEMORYBMP is requested
    //
    ...
    LONG                  *pBITMAPINFOSize)
{
    ...
    //
    // Changed on 5/18/98 to BITMAPINFOHEADER
    //
