...
#define NUM_PROPS_TO_SET 3
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            WIAS_LERROR(g_pIWiaLog,WIALOG_NO_RESOURCE_ID,("SetItemSize, NULL item context"));
        }
    } else {
    ...
    LONG                        *plDevErrVal)
{
    ...
    *plDevErrVal = 0;
    ...
    //  For a typical scanner, here is where the property values would be
    //  retrieved from the property streams and written to the hardware.
    //
    //  For example:
    //
    //  HRESULT hr = S_OK;
    ...
    //
    //  //
    //  //  Indicate which properties we're interested in.  For this example,
    //  //  we're going to get some properties related to selection area:
    ...
    //   };
    //
    //  PROPVARIANT pv[NUM_PROPS];
    //
    //  //
    //  // Read item properties.  Note that we're currently on the child item.
    //  // If we wanted to read properties on the root item, we'd have to 
    //  // navigate to the root item using wiasGetRootItem(...)
    //  //
    //
    //  hr = wiasReadMultiple(pWiasContext, NUM_PROPS, ps, pv, NULL);
    //  if (hr == S_OK) {
    //      //
    //      //  After our call to wiasReadMultiple(...):
    //      //  - pv[0].lVal now contains current value for WIA_IPS_XPOS i.e. 
    //      //  the horizontal start position.
    //      //  - pv[1].lVal now contains current value for WIA_IPS_YPOS i.e.
    //      //  the vertical start position.
    //      //  - pv[2].lVal now contains current value for WIA_IPS_XEXTENT i.e.
    //      //  the horizontal extent value.
    //      //  - pv[3].lVal now contains current value for WIA_IPS_YEXTENT i.e.
    //      //  the vertical extent value.
    //      //  
    //      
    ...
    //      
    //      //
    //      //  Now, let's call a function which writes the selection area to 
    //      //  the hardware.
    //      //
    //      hr = WriteSelectionAreaToScanner(StartX, StartY, Width, Height);
    //      
    //      // And so on...
    //      .
    //      .
    ...
    LONG                        *plDevErrVal)
{
    ...
    *plDevErrVal = 0;
    ...
    //
    // Use the pStiDevice handed to us at drvInitializeWia time to
    // lock the device.
    //
    ...
    LONG                        *plDevErrVal)
{
    ...
    *plDevErrVal = 0;
    ...
    //
    //  See the WIA documentation or email winimage@microsoft.com for
    //  information on how to implement this method.
    //
    ...
    LONG                        *plDevErrVal)
{
    ...
    *plDevErrVal = 0;
    ...
    if (pContext) {
        ...
        // Note: The WIA service allocates and frees the memory for us!
        //       We should only release any resources held by this context
        //       e.g. if we had used this context to store a handle to a file,
        //       this is where we'd close the handle.
        //
    }
    ...
    LONG                        *plDevErrVal)
{
    ...
    *ppIUnknownInner = NULL;
    ...
    //
    //  Build the driver item tree, if it hasn't been built yet.
    //
...
HRESULT _stdcall TestUsdDevice::BuildItemTree(void)
{
    ...
    if (bstrRoot) {
        ...
        //
        //  This will call create a driver item.  Since we want this to be the
        //  root of our driver item tree, we specify it in the paramters.
        //  In this case, the flags indicate that this driver item is:
        //  1.  A folder item
        //  2.  A device item
        //  3.  The root item
        //
    ...
    for (INT i = 0; i < NUM_DEVICE_ITEM; i++) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                //
                //  This will call create another driver item.  Since we want 
                //  this to be the child, notice that the flags indicate it is:
                //  1.  A file item (i.e. not a foilder, since it wont have
                //      any children)
                //  2.  An image item (i.e. requesting a data transfer is valid
                //      and will return image data)
                //  3.  A device item
                //
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    //  m_pIDrvItemRoot).
                    //  NOTE:  This is how the driver item tree is built!
                    //
