...
//print job info callback
	...
	virtual bool PrintPage(UINT nPage, HDC hDC)
	{
		...
		int n = (LONG)SendMessage(m_view, EM_FORMATRANGE, TRUE, (LPARAM)&fr);
		SendMessage(m_view, EM_DISPLAYBAND, 0, (LPARAM)&rcOutput);

		//Cleanup cache in richedit
		SendMessage(m_view, EM_FORMATRANGE, FALSE, 0);
		return true;
	}
	...
	bool LayoutPages()
	{
		...
		while (1)
		{
			...
			LONG lRet = (LONG)m_view.SendMessage(EM_FORMATRANGE, FALSE, (LPARAM)&fr);
		...
		m_view.SendMessage(EM_FORMATRANGE, FALSE, 0);
