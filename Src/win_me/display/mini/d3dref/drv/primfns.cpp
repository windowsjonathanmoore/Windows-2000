...
ReferenceRasterizer::Dp2SetTransform(LPD3DHAL_DP2COMMAND pCmd)
{
    ...
    LPD3DHAL_DP2SETTRANSFORM pSetXfrm = (LPD3DHAL_DP2SETTRANSFORM)(pCmd + 1);
...
HRESULT ReferenceRasterizer::Dp2SetRenderTarget(LPD3DHAL_DP2COMMAND pCmd)
{
    ...
    SRTDataOld.lpDDSLcl = GetDDSurfaceLocal(m_pDDLcl, pSRTData->hRenderTarget, &bIsNew);
    if( pSRTData->hZBuffer )
        SRTDataOld.lpDDSZLcl = GetDDSurfaceLocal(m_pDDLcl, pSRTData->hZBuffer, &bIsNew);
    else
