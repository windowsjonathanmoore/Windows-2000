...

{
    ...
    if( initializationStatus != NO_ERROR) 
    {
        ...
        hwInitData.HwInitDataSize = SIZE_NT4_VIDEO_HW_INITIALIZATION_DATA;
...

{
         ...
         (ConfigInfo->Length < sizeof(VIDEO_PORT_CONFIG_INFO)) ) 
    {
        ...
        return (ERROR_INVALID_PARAMETER);
         ...
         (ConfigInfo->Length < SIZE_NT4_VIDEO_PORT_CONFIG_INFO) ) 
    {
...

{
    ...
    case IOCTL_VIDEO_QUERY_LINE_DMA_BUFFER:
        ...
        else
        {
            ...
            if (pDMAIn->size >= sizeof(LINE_DMA_BUFFER))
            {
                if (hwDeviceExtension->ulLineDMABufferUsage == 0)
                {
                    *pDMAOut = *pDMAIn;
...

{
    ...
    DEBUG_PRINT((3, "PERM2: Reference Clock speed set to %dHz\n",ulChipClk));
