...
--*/
{
    ...
    else if ( *(USHORT *)pvROMAddress != 0xAA55)
    {
        ...
        DEBUG_PRINT((2, "CopyROMinitializationTable: ROM Signature 0x%x is invalid\n", 
                     *(USHORT *)pvROMAddress ));
        return;
    ...
    while(ul--)
    {
        ...
        ulReg = *pul++;
        ulRegData = *pul++;
        DEBUG_PRINT((0, "CopyROMInitializationTable: initializing register %08.8Xh with %08.8Xh\n",
                         ulReg, ulRegData));
    }
...

{
    ...
    while(cul--)
    {
        ...
        if( ulRegData != VideoPortReadRegisterUlong(pulReg) )
        {
            ...
            DEBUG_PRINT((0, "VerifyBiosSettings: Bios failed to set some registers correctly. \n"));
