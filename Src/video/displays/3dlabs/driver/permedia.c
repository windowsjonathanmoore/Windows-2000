...
VOID vFree(P2DMA *pP2dma)
{
    ...
    {
        if (pP2dma->ICB.pDMABufferStart!=NULL)
        {
            if (pP2dma->bDMAEmulation)
            {
                ENGFREEMEM( pP2dma->ICB.pDMABufferStart);
            } else
            {
                FreeDMABuffer( pP2dma->hDriver, pP2dma->ICB.pDMABufferStart);
            }
                     ...
                     )
{
    ...
    {
        // already initialized...but check also new acceleration level
        // if the user changed the acceleration level, we free or 
        // reallocate the DMA buffer 
        //

        if (dwAccelLevel==0)
        {
            // user wants full acceleration level!

            // if we were in emulation, we better free the old buffer
            // and try to reinitialize the DMA again
            if (pP2dma->bDMAEmulation && 
                pP2dma->ICB.pDMABufferStart!=NULL)
            {
                ENGFREEMEM( pP2dma->ICB.pDMABufferStart);
            ...
            // otherwise everything remains the same, nothing to do!

        } else
        {
            //
            // free continous DMA Buffer if user selects to 
            // lower acceleration level
            //
            if (pP2dma->ICB.pDMABufferStart!=NULL)
            {
                if (pP2dma->bDMAEmulation)
            ...
            //
            // maybe we have to switch to DMA emulation mode...
            //
            goto InitializeDMA;
        }
...
InitializeDMA:
    ...
    if (bUseDMA) 
    {
        ...
        LONG lDMABufferSize=DMACMDSIZE;
...
#if defined(_ALPHA_)
            ASSERTDD(lDMABufferSize<=0x2000,
                "DMA Buffer too big for alpha, fix constants!");
#endif
            if (lDMABufferSize >= DMACMDMINSIZE)
            {
                // we always do "ULONG" arithmetics in the DMA routines
                pP2dma->lDMABufferSize=lDMABufferSize/sizeof(ULONG);
                ...
                } else
                {
                    ...
                    DISPDBG((0,"allocated %ld bytes for DMA, interrupts ok", 
                        pP2dma->lDMABufferSize*4));
                }
            } else // buffer too small
            {
                DISPDBG((0,"allocated %ld (%ld) bytes for DMA, not enough! No DMA!", 
                    pP2dma->lDMABufferSize*4, lDMABufferSize));
    ...
    if (!bUseDMA)
    {
        ...
        pP2dma->lDMABufferSize=DMACMDMINSIZE;
...
VOID vBlockLoadInputFifo( P2DMA *pP2dma, ULONG uiTag, ULONG *pImage, LONG lWords)
{
    ...
    while (lWords>0)
    {
        ...
        LONG lBufferEntries=min( lWords, GetFreeEntries(pP2dma)-1);
...
{   
    ...
    return min(MAXBLKSIZE,
        (LONG)(pP2dma->ICB.pDMAWriteEnd-pP2dma->ICB.pDMAWritePos));
}
#endif
