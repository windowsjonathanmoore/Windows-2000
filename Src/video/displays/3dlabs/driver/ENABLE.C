                ...
                DRVENABLEDATA* pded)
{
    ...
    DBG_GDI((SETUP_LOG_LEVEL, "DrvEnableDriver: iEgnVersion = %ld",
            iEngineVersion, cj, pded));

    // Set up g_bOnNT40 based on the HIWORD value in iEngineVersion
    if(HIWORD(iEngineVersion) >= 0x3)
        g_bOnNT40 = FALSE;
