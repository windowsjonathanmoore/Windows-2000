...
__inline ULONG __MCD_CEIL(MCDFLOAT x)
{
    if ( x - (LONG)FABS(x) > 0.0)
        return (LONG)FABS(x) + 1;
    else
        return (LONG)FABS(x);
}

__inline MCDFLOAT __MCD_WRAP(MCDFLOAT x)
{
    if ( x > __MCDONE){
        while (x > __MCDONE) x-= __MCDONE;
        return x;
    }else if (x < __MCDZERO){
        while (x < __MCDZERO) x+= __MCDONE;
        return ((LONG)x - x + __MCDONE);
    }else
        return (x);
}
