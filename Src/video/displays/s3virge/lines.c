...
BOOL bIntegerLine3D(PDEV*, ULONG, ULONG, ULONG, ULONG, ULONG);

/******************************Public*Routine******************************\
* BOOL bLines3D(ppdev, pptfxFirst, pptfxBuf, cptfx, pls,
...
PFNSTRIP3D  apfn[],     // Array of strip functions
FLONG       flStart,    // Flags for each line
ULONG       ulHwMix)    // ROP3 to use (won't require a source)
{
...
\***********************************************************************/
    ...
    // Check for non-clipped, non-styled integer endpoint lines
        ...
        if ((fl & (FL_CLIP | FL_STYLED)) == 0)
        {
            ...
            {
                if (bIntegerLine3D(ppdev, M0, N0, dM, dN, ulHwMix))
                {
                    goto Next_Line;
                }
            }
