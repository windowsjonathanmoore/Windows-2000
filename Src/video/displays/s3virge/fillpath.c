...
FLONG       flOptions)
{
    ...
    if (ulHwForeMix & MIX_NEEDSPATTERN)  // Only do if we need a pattern
    {
        // Prevent 24bpp and transparent from calling vMmFastPatRealize.
        // This supports s3 968 workaround.

        bRealizeTransparent = (ulHwForeMix != ulHwBackMix);
...
//            // hardware patterns.
//
//            if (!(ppdev->ulCaps & CAPS_HW_PATTERNS))
//                goto SkipFastFill;
//
...
//                (ppdev->ulCaps & CAPS_RE_REALIZE_PATTERN))
//            {
