...
*       File Name:  LPB.C
*
*       Contains most of the DirectDraw VPE stuff.
*
*       This source file is shared between the Win95 driver (where it's
*       called 'LPB.C') and the WindowsNT driver (where it's called
*       'VPETR.C').
*
*       If you're reading this code from the NT DDK, you'll notice that
*       this file uses the Win95 structure definitions (e.g., Win95's
*       LPDDRAWI_DDRAWSURFACE_LCL instead of NT's PDD_SURFACE_LOCAL).
*       This is done so that this code can be shared between Win95 and
*       NT.  The header file 'dx95type.h' contains the defines
*       necessary to convert between the two types.
*
...
/*
** The virge supports 8 and 16 bit video port connections using external
** lines to supply the HSYNC and VSYNC information.
*/
const GUID *guids[] =
{
...
//  DDVPCAPS_SYNCMASTER     |   // LPB doesn't support genlocking
//  DDVPCAPS_VBISURFACE     |   // GX2 places VBI data in same surface
//  DDVPCAPS_COLORCONTROL   |   // LPB doesn't support this. SP does.
    0,
//  DDVPFX_CROPTOPDATA      |   // We support full cropping - not limited cropping
    DDVPFX_CROPX            |
    DDVPFX_CROPY            |
    DDVPFX_INTERLEAVE       |
...
/*
** The LPB connection has the following properties:
** -  Can invert the field polarities (treat odd fields like even and
**     versa)
** -  Does not could lines that occur during the VREF period
** -  Counts half lines written by the decoder
** -  Can understand an interlaced signal
** -  Can perform cropping using a VACT line
*/
...
const DWORD DH_16_YUV[] = {
    0x7fff7fff, 0x7777ffff, 0x777777ff, 0x77777777,
    0x57775777, 0x55557777, 0x55575557, 0x55555555,
    0x55555511, 0x11115555, 0x11115151, 0x88888888,
    0x01110111, 0x00001111, 0x00010001, 0x00000000
};
...
void  Bandwidth_SetSecondary_NT(PPDEV ppdev, PBAND pb, WORD device, SHORT bpp,
	...
	#define Bandwidth_SetSecondary(pb, device, bpp, xSrc, ySrc, xTrg, yTrg, xMax, yMax) \
	Bandwidth_SetSecondary_NT(sData, pb,device,bpp,xSrc,ySrc,xTrg,yTrg,xMax,yMax)
#else
 ...
 * LPB_DriverInfo
 *
 * Called when DriectDraw is initialized.  It fills in the HALINFO table
 * with those flags required by VPE and intializes some state information.
 */
DWORD LPB_DriverInit(LPS3THKINFO sData)
{
    ...
    sData->ddHALInfo.ddCaps.dwCaps2 |=
                                      DDCAPS2_VIDEOPORT         |
//                                      DDCAPS2_CANBOBINTERLEAVED |
    ...
    switch(id)
    {
        ...
        default:
            pLPB->jFamily = 3;
            pLPB->ddVideoPortCaps.dwNumAutoFlipSurfaces = 2;
            break;
    }
...
DWORD PASCAL LPB_Create(LPDDHAL_CREATEVPORTDATA lpInput)
{
    ...
    pLPB->jGUIDIndex = (UCHAR)(IndexFromGUID(&(lpInput->lpVideoPort->ddvpDesc.VideoPortType.guidTypeID)));
...
DWORD PASCAL LPB_Flip(LPDDHAL_FLIPVPORTDATA lpInput)
{
     ...
     */
    dwBase = (DWORD)(SCREENOFFSET(sData, lpInput->lpSurfTarg->lpGbl->fpVidMem));
...
DWORD PASCAL LPB_GetBandwidth(LPDDHAL_GETVPORTBANDWIDTHDATA lpInput)
{
    ...
    if (!(lpInput->dwFlags & DDVPB_TYPE))
    {
        ...
        x = (SHORT)pDesc->dwFieldWidth;
        y = (SHORT)pDesc->dwFieldHeight;
        ...
        */
...

        {
            ...
            */
            ...
            // HACK because their bandwidth doesn't correctly determine
            // Y interpolation values (in fact, they disable the interpolator)
            lpOutput->dwYInterpolate = TempBand.HStretch;
            lpOutput->dwYInterpAndColorkey = TempBand.HStretch;
        }
    }
    ...
    if (lpInput->lpConnect != NULL)
    {
        ...
        for (i=0;i <= 1; i++)   // do twice.. 8 bit and 16 bit connections
        {
            for (j=0;j < MAX_VP_TYPES; j++)
            {
                ...
                pEntry->dwSize = sizeof(DDVIDEOPORTCONNECT); // Fix for DCT 250
...
DWORD PASCAL LPB_Update(LPDDHAL_UPDATEVPORTDATA lpInput)
{
    ...
    */
    ...
    //bchernis: alligment fix is same as for Stream Processor's stride
    dwStride = (dwStride + HEAP_X_ALIGNMENT - 1) & ~(HEAP_X_ALIGNMENT - 1);
        ...
        lpInput->dwFlags == DDRAWI_VPORTSTART )
    {
        ...
        */
        dwTemp = (DWORD)(SCREENOFFSET(sData, lpSrcLcl->lpGbl->fpVidMem));
        ...
        if (lpInput->lpVideoInfo->dwVPFlags & DDVP_AUTOFLIP)
        {
            ...
            ** for second buffer
            */
            sData->AutoflipOffsets[0] = (DWORD)(SCREENOFFSET(sData, lpSrcLcl->lpGbl->fpVidMem));
            TRACE("Using hardware double buffer");
            dwTemp = (DWORD)(SCREENOFFSET(sData, lpInput->lplpDDSurface[1]->lpLcl->lpGbl->fpVidMem));
            sData->AutoflipOffsets[1] = dwTemp;
            dwTemp += pLPB->dwBaseOffset;
        ...
        */
        ...
        dwOrgSrcPixelWidth = dwSrcPixelWidth;

        if ( sData->wChipId == 0x8c01)
        { //only for M5
                if (dwSrcHeight > 240)
                  { //PAL DVD
                        dwSrcPixelWidth = RoundDown(dwSrcPixelWidth, 64);
                        dwSrcHeight--;             
                        sData->LPBData.VPType = 2;
                  }
                  else
                  { //NTSC DVD
                    /* Input width given to hardware must be aligned */
                        dwSrcPixelWidth = RoundUp(dwSrcPixelWidth, 64);
                        sData->LPBData.VPType = 1;
                  }
        }
        ...
        {
           dwHOffset =  dwOrgSrcPixelWidth - dwSrcPixelWidth;
           if (dwHOffset >= 2 )
               dwHOffset -= 2;
           else
               dwHOffset = 0;
        ...
        if (lpInput->lpVideoInfo->dwVPFlags & DDVP_PRESCALE)
        {
            ...
            if ( (dwSrcHeight > 240) && ( sData->wChipId == 0x8c01) )
            { //PAL DVD
               dwDstHeight--;
            }
...
DWORD GetTimeInMilliseconds()
{
    ...
    liTime = (liTime & 0xFFFFFFFF00000000) / liFrequency * 10000000 +
            (liTime & 0xFFFFFFFF) * 10000000 / liFrequency;
    liTime /= 10000;
...
DWORD PASCAL LPB_WaitForSync(LPDDHAL_WAITFORVPORTSYNCDATA lpInput)
{
    ...
    /*
     * Convert the timout value from ms to 100 nanosecond intervals
     */
    dwEndTime = lpInput->dwTimeOut;
    if( dwEndTime == 0 )
    {
        dwEndTime = 20;     // Default to 20 ms timeout
    }
    dwTempTime = GetTimeInMilliseconds();
    dwEndTime += dwTempTime;
        ...
        (lpInput->dwFlags == DDVPWAIT_END))
    {
         ...
         *
         * We have to check a timout value because the video can be turned
         * off or unplugged and we don't want to hang forever.
         */
        LPB_RMW(sData, LPB_INTERRUPT_FLAGS, 0, LPB_INT_VSYNC);         // clear the interrupt bit .. by writing 1, go figure..
...
DWORD PASCAL LPB_GetSignalStatus(LPDDHAL_GETVPORTSIGNALDATA lpInput)
{
    ...
    /*
    ** First, check to see that End of Line Interrupts are happening.
    ** If it takes longer than 2 ms for 10 lines to occur, we don't
    ** have a good signal.
    */
    dwTempTime = GetTimeInMilliseconds();
    dwEndTime = dwTempTime + 2;

    for (i=0; (i < 10) && (dwTempTime < dwEndTime); i++)
    {
        LPB_RMW(sData, LPB_INTERRUPT_FLAGS, 0, LPB_INT_HSYNC);
        while(!(LPB_Get(sData, LPB_INTERRUPT_FLAGS) & LPB_INT_HSYNC) &&
            (dwTempTime < dwEndTime))
 ...
 * LPB_ColorControl
 *
 * This function is only required if the video port can 
 * adjust the color of the picture (brightness, contrast, etc.)
 */
 ...
 * SyncVideoPortData32
 *
 *  This function is called everytime the video port information changes
 *  (e.g. UpdateVideoPort is called, etc.).  It allows the driver to massage
 *  the data before it is given to the Kernel Mode Video Transport
 *  (DxApi) for use.  This allows the driver to change variables and fill
 *  in driver reserved fields to make life easier for the miniport driver.
 *
 *  The following members must be filled in by this call:
 *
 *      dwOriginOffset
 *          Value indicating the byte offset from the top-left corner of the
 *          surface to the top-left pixel written by the video port.
 *
 *  The following members can be modified by the driver, but default values
 *  are provided:
 *
 *      dwHeight
 *          Height of the video region written to memory (double the
 *          field height if interleaved).
 *      dwVBIHeight
 *          Height of the VBI region
 *
 *  The following members can be modified by the driver, but default values
 *  are not provided:
 *
 ...
 *  SyncSurfaceData32
 *
 *  This function is called everytime the surface information changes
 *  (e.g. UpdateOverlay is called, etc.).  It allows the driver to massage
 *  the data before it is given to the Kernel Mode Video Transport
 *  (DxApi) for use.  This allows the driver to change variables and fill
 *  in driver reserved fields to make life easier for the miniport driver.
 *
 ...
 *
 *      dwOverlayOffset
 *          Value indicating the byte offset of the top-left corner of the
 *          overlay relative to the top-left corner of the surface.
 *
 *  The following members can be modified by the driver, but default values
 *  are provided:
 *
 *      dwSurfaceOffset
 *          Contains the frame buffer offset of the top-left corner
 *          of the surface.
 *      fpLockPtr
 *          Contains a PCI address that can be used to access the top-left
 *          corner of the surface.
 *      lPitch
 *          Contains the pitch of the surface.
 *
 *  The following members can be modified by the driver, but default values
 *  are not provided:
 *
...
DWORD WINAPI SyncSurfaceData32(LPDDHAL_SYNCSURFACEDATA lpInput)
{
    ...
    DPF("rcOverlaySrc={%d, %d, %d, %d}",
        lpInput->lpDDSurface->rcOverlaySrc.left,
        lpInput->lpDDSurface->rcOverlaySrc.top,
        lpInput->lpDDSurface->rcOverlaySrc.right,
        lpInput->lpDDSurface->rcOverlaySrc.bottom);

    lpInput->dwSurfaceOffset
        = (DWORD)(SCREENOFFSET(sData, lpInput->lpDDSurface->lpGbl->fpVidMem));
    lpInput->dwOverlayOffset
        = lpInput->dwSurfaceOffset + UPDATEOVERLAYFLIPOFFSET(sData);

    /*
     * For now, the S3 driver only uses one of the reserved fields.  The
     * miniport keeps an array of surface polarities, but it needs a way
     * to assign each surface an index.
     */
    lpInput->dwDriverReserved1 = dwCnt;
    if( ++dwCnt >= 10 )
    {
        dwCnt = 0;
    }
