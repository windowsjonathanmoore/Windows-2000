...
* S3 Virge MCD driver, including culling and face computations.  Note that
...
{                                                                           \
    ...
    MCDVERTEX *temp;                                                        \
                                                                            ...
                                                                            \
    reversed = 0;                                                           \
    if (__MCD_VERTEX_COMPARE((c)->windowCoord.y, <=, (b)->windowCoord.y)) {  \
        if (__MCD_VERTEX_COMPARE((b)->windowCoord.y, <=, (a)->windowCoord.y)) {  \
            /* Already sorted */                                            \
        } else {                                                            \
            if (__MCD_VERTEX_COMPARE((c)->windowCoord.y, <=, (a)->windowCoord.y)) {\
                temp=(b); (b)=(a); (a)=temp;                                \
                reversed = 1;                                               \
            } else {                                                        \
...
VOID FASTCALL __MCDCalcZSlope(DEVRC *pRc, MCDVERTEX *a, MCDVERTEX *b, MCDVERTEX *c)
{
    ...
    pRc->dzdx = (dzAC * pRc->dyBC - dzBC * pRc->dyAC)*oneOverArea;
    pRc->dzdy = (dzBC * pRc->dxAC - dzAC * pRc->dxBC)*oneOverArea;
}
                                     ...
                                     MCDVERTEX *c)
{
    ...
    if ((clipNum = pRc->pEnumClip->c) > 1) {
        ...
        (*ppdev->hwTriSetupClipRect)(pRc, pClip++);  
    }
    ...
    if (pRc->privateEnables & __MCDENABLE_SMOOTH) {
        ...
        if (polygonMode == GL_FILL) {
            ...
            COPY_COLOR(tempA, a->colors[0]);
            COPY_COLOR(tempB, b->colors[0]);
            COPY_COLOR(tempC, c->colors[0]);
    ...
    if (pRc->privateEnables & __MCDENABLE_SMOOTH) {
        ...
        if (polygonMode == GL_FILL) {
            ...
            COPY_COLOR(a->colors[0],tempA);
            COPY_COLOR(b->colors[0],tempB);
            COPY_COLOR(c->colors[0],tempC);
        }
    }
}
                                      ...
                                      MCDVERTEX *c)
{
    ...
    MCDBG_PRINT(DBG_PRIM,"__MCDRenderFlatTriangle");
...
////////////////////////////////////////////////////////////////////////
//
// VOID FASTCALL __MCDRenderFlatFogTriangle(DEVRC *pRc, MCDVERTEX *pv1,
//                                          MCDVERTEX *pv2, MCDVERTEX *pv3)
//
// This is the top-level flat triangle renderer with cheap fogging
//
////////////////////////////////////////////////////////////////////////
