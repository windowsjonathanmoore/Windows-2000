...
*
* To inform the miniport that the display driver is going to update
* the CRTC registers.
*
\**************************************************************************/
...
VOID AcquireMiniPortCrtc(PDEV * ppdev)
{
   ...
   if(ppdev->ulCaps2 & CAPS2_HW_CURSOR_WKAROUND)
    EngDeviceIoControl(
        ...
        IOCTL_VIDEO_S3_ACQUIRE_CRTC, // dwIoControlCode
        NULL,                        // lpInBuffer
...
*
* TO inform the miniport that the display driver has finished to update
* the CRTC registers.
*
\**************************************************************************/
...
VOID ReleaseMiniPortCrtc(PDEV * ppdev)
{
   ...
   if(ppdev->ulCaps2 & CAPS2_HW_CURSOR_WKAROUND)
    EngDeviceIoControl(
        ppdev->hDriver,              // hDevice
        IOCTL_VIDEO_S3_RELEASE_CRTC, // dwIoControlCode
        NULL,                        // lpInBuffer
        0,                           // DWORD nInBufferSize
        NULL,                        // lpOutBuffer
        0,                           // DWORD nOutBufferSize
