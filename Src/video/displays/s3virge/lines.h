...
#define FL_DONT_DO_HALF_FLIP    0x00004000L     // .1.. .... .... ....
...
typedef VOID (*PFNSTRIP3D)(PDEV*, STRIP*, ULONG);

// Strip drawers:

VOID vrlSolidHorizontal3D(PDEV* ppdev, STRIP *pStrip, ULONG ulHwForeMix);
VOID vrlSolidVertical3D(PDEV* ppdev, STRIP *pStrip, ULONG ulHwForeMix);
VOID vrlSolidDiagonalHorizontal3D(PDEV* ppdev, STRIP *pStrip, ULONG ulHwForeMix);
VOID vrlSolidDiagonalVertical3D(PDEV* ppdev, STRIP *pStrip, ULONG ulHwForeMix);


// External calls:

BOOL bLines3D(PDEV*, POINTFIX*, POINTFIX*, RUN* prun, ULONG,
            LINESTATE*, RECTL*, PFNSTRIP3D*, FLONG, ULONG);
