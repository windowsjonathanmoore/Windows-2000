...
#define INIT_PIN( pin, pinptr, category, name, index )      \
    ...
    if (AdapterCommon->GetPinConfig (PINC_SURROUND_PRESENT))
    {
        ...
        {
            INIT_PIN (PIN_VIRT_CENTER,
                      CurrentPin,
                      &KSNODETYPE_ANALOG_CONNECTOR,
                      &ICHFNAME_CENTER,
                      Index);
    
            INIT_PIN (PIN_VIRT_LFE,
                      CurrentPin,
                      &KSNODETYPE_ANALOG_CONNECTOR,
                      &ICHFNAME_LFE,
                      Index);
        ...
        // Note: this master out and the one in the else path differs only
        // in the name!
...
#define INIT_NODE( node, nodeptr, type, name, automation, index )   \
    ...
    if (NodeDescriptors)
    {
        ...
        if (AdapterCommon->GetPinConfig (PINC_SURROUND_PRESENT))
        {
            // Add the mono master out volume.
...
#define INIT_NF_CONN( cptr, fnode, fpin, tpin )         \
    ...
    if (ConnectionDescriptors)
    {
        ...
        if (AdapterCommon->GetPinConfig (PINC_SURROUND_PRESENT))
        {
            ...
            INIT_FN_CONN (CurrentConnection, PIN_VIRT_SURROUND, NODE_SURROUND_VOLUME, 1);
            ...
            INIT_NN_CONN (CurrentConnection, NODE_SURROUND_MUTE, 0, NODE_MAIN_MIX, 9);

            // check also for the center and LFE volumes
            if (AdapterCommon->GetPinConfig (PINC_CENTER_LFE_PRESENT))
            {
                // PIN_VIRT_CENTER -> NODE_CENTER_VOLUME
                INIT_FN_CONN (CurrentConnection, PIN_VIRT_CENTER, NODE_CENTER_VOLUME, 1);
                ...
                INIT_NN_CONN (CurrentConnection, NODE_CENTER_MUTE, 0, NODE_MAIN_MIX, 10);
            
                // PIN_VIRT_LFE -> NODE_LFE_VOLUME
                INIT_FN_CONN (CurrentConnection, PIN_VIRT_LFE, NODE_LFE_VOLUME, 1);
    
                // NODE_LFE_VOLUME -> NODE_LFE_MUTE
                INIT_NN_CONN (CurrentConnection, NODE_LFE_VOLUME, 0, NODE_LFE_MUTE, 1);
    
                // NODE_LFE_MUTE -> NODE_MAIN_MIX
                INIT_NN_CONN (CurrentConnection, NODE_LFE_MUTE, 0, NODE_MAIN_MIX, 11);
            }
        }
