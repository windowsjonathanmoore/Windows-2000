        ...
        case WM_DEVICECHANGE:
            ...
            switch ((UINT)wParam) {
                ...
                case DBT_DEVICEREMOVECOMPLETE:
                    ...
                    if (SharedData->PlatformId == VER_PLATFORM_WIN32_NT) {
                ...
                case DBT_DEVNODES_CHANGED:
                    ...
                    if (SharedData->PlatformId == VER_PLATFORM_WIN32_WINDOWS) {
...
//            ShowWindow(SharedData->g_hWnd, SW_SHOWNORMAL);
    ...
    )
{
    ...
    DeviceData = &SharedData->deviceData;
    DeviceData->numDevices = 0;

    // get a handle to 1394 test class using guid.
    hDevInfo = SetupDiGetClassDevs( &t1394DiagGUID,
                                    NULL,
                                    NULL,
    ...
    if (!hDevInfo) {
        ...
        while (TRUE) {
            ...
            if (SetupDiEnumDeviceInterfaces(hDevInfo, 0, &t1394DiagGUID, i, &deviceInterfaceData)) {
                ...
                if (!bReturn) {
                    ...
                    lstrcpy(DeviceData->deviceList[i].DeviceName, DeviceInterfaceDetailData->DevicePath);
                    LocalFree(DeviceInterfaceDetailData);
                }
            }
            else {
    ...
    TRACE(TL_TRACE, (hWnd, "DeviceData->numDevices = 0x%x\r\n", DeviceData->numDevices));
    ...
    )
{
    ...
    BusResetThreadParams->szDeviceName = SharedData->deviceData.deviceList[0].DeviceName;
    ...
    )
{
    ...
    if (bAll) {
        ...
        ULONG           i = 0;
        UCHAR           ch;
        UCHAR           BusName[] = "\\\\.\\1394BUS ";

        while (i<10) {

            _itoa(i, &ch, 10);
            strncpy(&BusName[11], &ch, 1);
            ...
            if (hDevice != INVALID_HANDLE_VALUE) {
                ...
                dwRet = DeviceIoControl( hDevice,
                                         (bMode) ? IOCTL_1394_TOGGLE_ENUM_TEST_ON : IOCTL_1394_TOGGLE_ENUM_TEST_OFF,
                                         NULL,
                                         0,
                                         NULL,
                                         0,
    ...
    )
{
    ...
    CopyMemory( DeviceData, &SharedData->deviceData, sizeof(DEVICE_DATA));
