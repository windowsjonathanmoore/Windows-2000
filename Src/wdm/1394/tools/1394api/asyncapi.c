    ...
    )
{
    ...
    if (hDevice != INVALID_HANDLE_VALUE) {
        ...
        if (dwRet) {
            ...
            TRACE(TL_TRACE, (hWnd, "hAddressRange = 0x%x\r\n", pAllocateAddressRange->hAddressRange));
...
{
    PASYNC_LOOPBACK_PARAMS          pLoopbackParams;
    PASYNC_READ                     pAsyncRead;
    PASYNC_WRITE                    pAsyncWrite;
//    PVOID                           pAsyncLock;
...
//    ULONG                           ulLockSize;
    ...
    if (hDevice != INVALID_HANDLE_VALUE) {
        ...
        // we need to figure out the block size by reading the max_rec field
        // out of the bus info block, this is offset 0xf0000408
        pAsyncRead = LocalAlloc(LPTR, sizeof(ASYNC_READ)+sizeof(ULONG));

        pAsyncRead->bRawMode = FALSE;
        pAsyncRead->bGetGeneration = TRUE;
        pAsyncRead->DestinationAddress.IA_Destination_Offset.Off_High = 0xffff;
        pAsyncRead->DestinationAddress.IA_Destination_Offset.Off_Low = 0xf0000408;
        pAsyncRead->nNumberOfBytesToRead = 4;
        pAsyncRead->nBlockSize = 0;
        pAsyncRead->fulFlags = 0;

        dwRet = AsyncRead( pLoopbackParams->hWnd,
                           pLoopbackParams->szDeviceName,
                           pAsyncRead,
                           FALSE
                           );

        MaxRec = bswap(*(PULONG)pAsyncRead->Data) >> 12;
        nBlockSize = 2 << (MaxRec);

        TRACE(TL_TRACE, (pLoopbackParams->hWnd, "nBlockSize = %d\r\n", nBlockSize));
        ...
        if (pLoopbackParams->ulLoopFlag & ASYNC_LOOPBACK_WRITE) {
            ...
            pAsyncWrite->nBlockSize = nBlockSize;
        ...
        if (pLoopbackParams->ulLoopFlag & ASYNC_LOOPBACK_READ) {
            ...
            pAsyncRead->DestinationAddress.IA_Destination_Offset.Off_Low = 
                pLoopbackParams->AddressOffset.Off_Low;
            pAsyncRead->nBlockSize = nBlockSize;
            pAsyncRead->fulFlags = 0;
        ...
        while (TRUE) {
            ...
            if ((!pLoopbackParams->bKill) && (pLoopbackParams->ulLoopFlag & ACCESS_FLAGS_TYPE_WRITE)) {
                ...
                if (!dwRet) {
                    ...
                    TRACE(TL_ERROR, (pLoopbackParams->hWnd, "pAsyncWrite = 0x%x\r\n", pAsyncWrite));

                    // see if this is a generation error, if so, we'll assume
                    // a bus reset and continue
                    if (dwRet != ERROR_MR_MID_NOT_FOUND) {

                        // we had a failure. set the kill flag to abort the loopback...
            ...
            if ((!bFailed) && (!pLoopbackParams->bKill) && (pLoopbackParams->ulLoopFlag & ACCESS_FLAGS_TYPE_READ)) {
                ...
                if (!dwRet) {
                    ...
                    TRACE(TL_ERROR, (pLoopbackParams->hWnd, "pAsyncRead = 0x%x\r\n", pAsyncRead));
