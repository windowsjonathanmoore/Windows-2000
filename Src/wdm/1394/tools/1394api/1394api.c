...
/*
            ...
            // in this case, we have to get the buffer size
            ...
            TRACE(TL_TRACE, (hWnd, "pGetLocalHostInfo = 0x%x\r\n", pGetLocalHostInfo));

            pGetLocalHostInfo->nLevel = getLocalHostInfo->nLevel;
            ...
            LocalHostInfo6->CsrBaseAddress.Off_Low = INITIAL_REGISTER_SPACE_LO + SPEED_MAP_LOCATION;
            ...
            if (dwRet) {
                ...
                TRACE(TL_TRACE, (hWnd, "CsrBaseAddress.Off_High needed = 0x%x\r\n", LocalHostInfo6->CsrBaseAddress.Off_High));
                TRACE(TL_TRACE, (hWnd, "CsrBaseAddress.Off_Low needed = 0x%x\r\n", LocalHostInfo6->CsrBaseAddress.Off_Low));
                TRACE(TL_TRACE, (hWnd, "CsrDataLength needed = 0x%x\r\n", LocalHostInfo6->CsrDataLength));

                // recalcute the buffer needed to complete this request
                ulBufferSize = sizeof(GET_LOCAL_HOST_INFO6) + LocalHostInfo6->CsrDataLength;

                // free the buffer we passed down...
