...
--*/
{
    ...
    *pDescriptor = NULL;
    ...
    __try {
        ...
        if((Pass != 0) && !NT_SUCCESS(Status) && (pLocalDescriptor != NULL)) {
...
--*/
{
    ...
    SCSISCAN_KdPrint(MAX_TRACE,("Release Queue \n"));
    ...
    if (pIrp != NULL ) {
        ...
        SCSISCAN_KdPrint(MAX_TRACE,("ScsiScanner Couldn't allocate IRP \n"));
...
--*/
{
    ...
    pEvent = ExAllocatePool(NonPagedPool, sizeof(KEVENT));
    if(pEvent == NULL) {
        SCSISCAN_KdPrint(MAX_TRACE,("ClassGetInfo: Unable to allocate event\n"));
        Status = STATUS_INSUFFICIENT_RESOURCES;
        goto Cleanup;
    }

    pLocalInfo = ExAllocatePool(NonPagedPool, sizeof(SCSI_ADDRESS));
    if(pLocalInfo == NULL) {
        SCSISCAN_KdPrint(MAX_TRACE,("ClassGetInfo: Unable to allocate local buffer\n"));
        Status = STATUS_INSUFFICIENT_RESOURCES;
        goto Cleanup;
    }
    ...
    __try {
        ...
        if (pIrp == NULL) {
            SCSISCAN_KdPrint(MAX_TRACE,("ClassGetInfo: unable to allocate irp\n"));
            Status = STATUS_INSUFFICIENT_RESOURCES;
            __leave;
        }
