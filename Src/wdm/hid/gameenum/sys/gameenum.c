...
--*/
{
    ...
    if (NT_SUCCESS (status)) {
        status = Game_SendIrpSynchronously (data->TopOfStack, Irp, TRUE, TRUE);
    }
...
Game_NoCustomAccessors:
        ...
        Parameters->PortContext = PdoData;
    }
    else {
        //
...
#define GAME_INVALID_PDO(pdo) ((pdo)->Removed || (pdo)->SurpriseRemoved || !(pdo)->Attached || !(pdo)->Started)
    ...
    )
{
...
#if DBG
    if (PdoData->Removed) {
        Game_KdPrint(PdoData, GAME_DBG_ACQUIRE_ERROR,
                     ("Acquire failed!  Pdo (0x%x) was removed....\n", PdoData->Self));
        return STATUS_NO_SUCH_DEVICE;
    }
    else if (PdoData->SurpriseRemoved)  {
        Game_KdPrint(PdoData, GAME_DBG_ACQUIRE_ERROR,
                     ("Acquire failed!  Pdo (0x%x) was surprise removed....\n", PdoData->Self));
        return STATUS_NO_SUCH_DEVICE;
    }
    else if (!PdoData->Attached) {
        Game_KdPrint(PdoData, GAME_DBG_ACQUIRE_ERROR,
                     ("Acquire failed!  Pdo (0x%x) was not attached....\n", PdoData->Self));
        return STATUS_NO_SUCH_DEVICE;
    }
    else if (!PdoData->Started) {
        Game_KdPrint(PdoData, GAME_DBG_ACQUIRE_ERROR,
                     ("Acquire failed!  Pdo (0x%x) was not started....\n", PdoData->Self));
        return STATUS_NO_SUCH_DEVICE;
    }
#else
    if (GAME_INVALID_PDO(PdoData)) {
        return STATUS_NO_SUCH_DEVICE;
    }
#endif
    ...
    if (InterlockedCompareExchange(&fdoData->Acquired, TRUE, FALSE)) {
        Game_KdPrint(PdoData, GAME_DBG_ACQUIRE_ERROR,
                     ("Acquire failed!  Pdo (0x%x) was already acquired....\n", PdoData->Self));
    ...
    )
{
    ...
    ASSERT(!PdoData->SurpriseRemoved);
