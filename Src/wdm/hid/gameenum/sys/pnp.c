...
//    length = (ExposeSize - sizeof (GAMEENUM_EXPOSE_SIBLING))/sizeof(WCHAR);
    ...
    if (ExposeSibling->HardwareIDs) {
        ...
        length = (ULONG) (buffer - ExposeSibling->HardwareIDs);
         
        if ((L'\0' != ExposeSibling->HardwareIDs[length - 1]) ||
            (L'\0' != ExposeSibling->HardwareIDs[length - 2])) {
            return STATUS_INVALID_PARAMETER;
        }
    }
    ...
    //
    // Check to see if the multi sz has any contents, ie more than just two 
    // null chars
    //
    if (length) {
        //
...
Parameters
    ...
    RemoveHandle - if NULL, then remove all the items in the list, otherwise
