    ...
    case WM_INITDIALOG:
        {
            ...
            LPDIPROPSTRING pDIPropStr = new (DIPROPSTRING);
            assert (pDIPropStr);
            ...
            pDIPropStr->diph.dwObj          = 0;
            pDIPropStr->wsz[0]              = L'\0';
            ...
            // Another method of doing this can be found in DICPUTIL.CPP and the call would look like this:
            //DIUtilGetJoystickDisplayName(pdiCpl->GetID(), pdiJoyCfg, lptszBuf, MAX_PATH);
            if( SUCCEEDED(pdiDevice2->GetProperty(DIPROP_PRODUCTNAME, &pDIPropStr->diph)) )
            {
#ifdef _UNICODE
                SendDlgItemMessageW(hWnd, IDC_DISPNAME, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)pDIPropStr->wsz);
#else
                SendDlgItemMessageA(hWnd, IDC_DISPNAME, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)W2A(pDIPropStr->wsz));
#endif
            }
            ...
            {
#ifdef _UNICODE
                SendDlgItemMessageW(hWnd, IDC_TYPENAME, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)pDIPropStr->wsz);
#else
                SendDlgItemMessageW(hWnd, IDC_TYPENAME, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)W2A(pDIPropStr->wsz));
#endif
            }
...
INT_PTR CALLBACK CPLSVR1_Page4_DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        {
            // perform any required initialization... 
            lpTmpBuff = new (TCHAR[STR_LEN_64]);
            assert (lpTmpBuff);
        ...
        case IDC_STOP:
            {
                ...
                if( LoadString(ghInst, IDS_UNACQUIRED, lpTmpBuff, STR_LEN_64) )
                    SetDlgItemText(hWnd, IDC_DEVSTATUS, lpTmpBuff);
    ...
    case WM_NOTIFY:
        {
            ...
            case PSN_SETACTIVE:
                ...
                {
                    if( LoadString(ghInst, IDS_ACQUIRED, lpTmpBuff, STR_LEN_64) )
                        // display the acquisition status
                        SetDlgItemText(hWnd, IDC_DEVSTATUS, lpTmpBuff);
            ...
            case PSN_KILLACTIVE:
                ...
                if( LoadString(ghInst, IDS_UNACQUIRED, lpTmpBuff, STR_LEN_64) )
                {
                    // Above, the acquisition status was set via SetDltItemText...
                    // Here's another way that's more effecient...
                    ::SendMessage(hWnd, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)lpTmpBuff);
                }
