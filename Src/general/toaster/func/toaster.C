...
#pragma alloc_text (PAGE, ToasterCreateClose)
...
--*/
{
    ...
    DriverObject->MajorFunction[IRP_MJ_CLOSE]          = ToasterCreateClose;
...
--*/
{
    ...
    if (!NT_SUCCESS (status)) {
        ...
        ToasterWmiRegistration(fdoData);
...

{
    ...
    if (HoldRequests == fdoData->QueueState) {
        ...
        status = ToasterQueueRequest(fdoData, Irp);
        return status;
    }
...
--*/
{
    ...
    while(!IsListEmpty(&cancelledIrpList))
    {
        ...
        cancelledIrp = CONTAINING_RECORD(listEntry, IRP,
                                Tail.Overlay.ListEntry);
