...
)
{
    ...
    pPort = (PPORTDIALOG) GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
)
{
    ...
    SetWindowLongPtr(hwnd, GWLP_USERDATA, (LONG_PTR) pPort);
    SendDlgItemMessage (hwnd, IDD_PN_EF_PORTNAME, EM_LIMITTEXT, MAX_PATH-2, 0);
...
)
{
    ...
    GetDlgItemText( hwnd, IDD_PN_EF_PORTNAME, string, MAX_PATH );
