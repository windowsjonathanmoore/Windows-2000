...
BOOLEAN DoSend(IrDevice *thisDev, PNDIS_PACKET packetToSend)
{
	...
	if (convertedPacket){
		...
		LOG("Send conversion complete; bytes:", thisDev->portInfo.writeBufLen);
		...
		LOG("DoSend - staging complete; POLL bit:", (UINT)(thisDev->portInfo.writeBuf[1] & (UCHAR)0x10));
	}
	else {
