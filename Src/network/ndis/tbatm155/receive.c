...
--*/
{
   ...
   Offset = DmaAlignmentRequired -
               (ULONG)((ULONG_PTR)pRecvPool->VirtualAddress % DmaAlignmentRequired);
...
--*/
{
   ...
   //
   Offset = DmaAlignmentRequired -
               (ULONG)((ULONG_PTR)VirtualAddress % DmaAlignmentRequired);
...
--*/
{
   ...
   pRecvPool->ReceiveHeaderSize =
       (((sizeof(RECV_BUFFER_HEADER) + sizeof(ALLOCATION_INFO) +
        DmaAlignmentRequired - 1) / DmaAlignmentRequired) *
...
--*/
{
   ...
   do
   {
       ...
       pRecvPool->ReceiveBufferSize = 
			(((SizeOfReceiveBuffer + DmaAlignmentRequired - 1) /
			DmaAlignmentRequired) * DmaAlignmentRequired);
       ...
       //
       pRecvPool->ReceiveHeaderSize =
               (((sizeof(RECV_BUFFER_HEADER) + DmaAlignmentRequired - 1) /
               DmaAlignmentRequired) * DmaAlignmentRequired);
...
--*/
{
        ...
        i = (i == pHwInfo->MaxIdxOfRxReportQ)? 0 : ++i)
   {
       ...
       {
           NdisDprReleaseSpinLock(&pRecvDmaQ->lock);
           tbAtm155ProcessReturnPacket(pAdapter, Packet, Reserved->RecvHeader);
           NdisDprAcquireSpinLock(&pRecvDmaQ->lock);
       }
...
//???   ASSERT(RECV_BUFFER_HEADER_SIG == pRecvHeader->Signature);

	tbAtm155ProcessReturnPacket(pAdapter, Packet, pRecvHeader);
   ...
   )
{
   ...
   //
   //	If the we failed somwhere then we need to cleanup the receive
   //	pool.
   //
   if ((NDIS_STATUS_SUCCESS != Status) && (NDIS_STATUS_PENDING != Status))
   {
...
--*/
{
   ...
   for (RxSlotNum = 0; *pTmpFreeSlots > 0; (*pTmpFreeSlots)--)
   {
       ...
       if (0 == pRecvHeader->TagPosted)
       {
           ...
           ASSERT(*pSlotTagOfBufQ > pHwInfo->NumOfVCs);
