...
--*/
{
   ...
   {
       // 
       // If fiail to get network address from registry, set
...
--*/
{
   ...
   do
   {
       ...
       {
           //
           //	Determine the total number of map registers.
           //
           Status = NdisQueryMapRegisterCount(NdisInterfacePci, &MapRegisters);
           if (NDIS_STATUS_SUCCESS == Status)
           {
               //
               //	We have a max that we can handle and we don't want to take
               //	all of the systems map registers...
               //
               MapRegisters = MapRegisters / 8;
           }
           else
           {
       ...
       for (c = 0; c < MapRegisters; c++, pMapRegisterBuffers++)
       {
           ...
           //
           pMapRegisterBuffers->MapRegisterIndex = c;
   ...
   *OpenErrorStatus = NDIS_STATUS_SUCCESS;
   ...
   do
   {
       ...
       *SelectedMediumIndex = c;
       ...
       //	Assign the PCI resources.
       //
       Status = tbAtm155ReadPciConfiguration(pAdapter);
       if (NDIS_STATUS_SUCCESS != Status)
       {
           DBGPRINT(DBG_COMP_INIT, DBG_LEVEL_ERR,
