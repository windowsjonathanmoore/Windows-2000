...
--*/
{
   ...
   if ((pAtmMediaParms->ConnectionId.Vci < MIN_VCS) ||
       (pAtmMediaParms->ConnectionId.Vci > (ULONG)(pHwInfo->NumOfVCs - 1)))
   {
		DBGPRINT(DBG_COMP_VC, DBG_LEVEL_ERR,
...
--*/
{
   ...
   if (VC_TEST_FLAG(pVc, fVC_RESET_IN_PROGRESS))
   {
		DBGPRINT(DBG_COMP_RESET, DBG_LEVEL_ERR,
			("tbAtm155DeactivateVcComplete: Adapter reset in progress (VC:0x%lx)\n",
           pVc->VpiVci.Vci));
