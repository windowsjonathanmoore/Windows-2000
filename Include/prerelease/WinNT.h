...
//
// FIELD_SIZE is used elsewhere in the tree, so we prefix RTL_ to not
// clash. The definitions of FIELD_SIZE vary between
//  (sizeof(((type *)0)->field))
//  (sizeof(((type *)0L)->field))
//   sizeof(((type *)0)->field)
//
...
// Using the name NUMBER_OF would break builds.
//
...
// This synonym is for initializing strings that actually are not const.
#define RTL_INIT_STRING(s) RTL_CONSTANT_STRING(s)
...
typedef struct tagACTIVATION_CONTEXT_DETAILED_INFORMATION {
    DWORD dwFlags;
...
typedef const struct tagACTIVATION_CONTEXT_DETAILED_INFORMATION *PCACTIVATION_CONTEXT_DETAILED_INFORMATION;
