...
#else
            *pcbUsed = SpEventSerializeSize(this, sizeof(*pSerEvent));
#endif
        }
        return hr;
    }
                             ...
                             CComPtr<ISpPhoneConverter> cpPhoneConv = NULL)
{
    ...
    dsText.TrimBoth();
    WCHAR *pwszStart = dsText;

    CSpDynamicString dsPhoneId(wcslen(pszOriginalText)+1);
    ...
    p = pwszStart;
    for (ULONG i = 0; SUCCEEDED(hr) && i < cWords; i++)
    {
        // skip over the \0's following the word
        for(; *p == L'\0'; p++)
            ;
        ...
        pPhraseElement[i].bDisplayAttributes = SPAF_ONE_TRAILING_SPACE;
