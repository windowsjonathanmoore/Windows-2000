...
//  of the message property that is in the buffer was copied.
//
...
// MessageText:
//
//  The RPC server cannot impersonate the client application, so the security
...
// MessageText:
//
//  The label is too long. It should be less than or equal to MQ_MAX_MSG_LABEL_LEN in length.
//
...
// MessageText:
//
//  Signature of received message is not valid.
//
...
// MessageText:
//
//  The operation is not supported by the remote Message Queuing service. For example, MQReceiveMessageByLookupId is not supported by Message Queuing 1.0/2.0.
//
...
// MessageText:
//
//  The object that its properties are being retreived from Active Directoy, does not belng to the requested class. 
//
...
// MessageText:
//
//  The cCol value in MQSORTSET can not be greater than one, Active Directory supports only a single sort key.
//
